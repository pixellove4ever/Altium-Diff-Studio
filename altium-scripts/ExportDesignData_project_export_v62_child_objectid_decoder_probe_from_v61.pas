{==============================================================================}
{ Altium Diff Studio - Exporter Script v62                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v62';
    SCHEMA_VERSION = 'ads-json-v62';
    SCHEMATIC_SCHEMA_VERSION = 'v62';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndChildObjectIdDecoderProbeV62",');
    JsonList.Add('      "graphicsNotes": "V62 keeps validated textRender and switches native symbol discovery away from direct ObjectId 44 geometry access toward a compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v62",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe child ObjectId decoder probe for native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v62 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v62  : active réellement Component.SchIterator_Create sur le composant courant, sans géométrie,
...
function V62DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V62ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V62IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V62DecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V62AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V62DecodeChildObjectId(ObjectId);
...
function V62InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V62CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V62ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V62AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V62IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V62TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V62_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V62 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V62_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V62_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V62_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V62_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V62_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V62ChildObjectIdDecoderProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V62IsTarget(Oid) then
        begin
            TargetRole := V62DecisionForObjectId(Oid);
            ExpectedKind := V62ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V62InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V62CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ...
            if CurrentComp <> nil then
                V62ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V62TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v62:' + NativeStatus
                    else
...
procedure ExportSchV62ChildObjectIdDecoderProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v62 child ObjectId decoder probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v62 child ObjectId decoder probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v62RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;childObjectIdDecoderProbe_v62;childIteratorObjectCount_v62;childIteratorObjectIds_v62;childIteratorDecodedRoles_v62;childIteratorTargetOrCandidateHits_v62;routeResultPlan_v62;recommendedNextAction');
        Summary.Add('Altium schematic v62 child ObjectId decoder probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v62 compile-safe route activation after v60: stop probing unknown direct/Variant geometry properties and map ObjectId 44 through component cluster, LibRef grouping, and child ObjectId decoder route without touching risky geometry accessors.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference/Location, typed ISch_Line cast-only confirmation; no direct generic/Variant geometry property is executed.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V62ChildObjectIdDecoderProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V62_CHILD_OBJECTID_DECODER_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 rows are included and childIteratorDecodedRoles_v62 decodes recurrent child ObjectIds 37, 27, 8, 18, 22 and 5 for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v62 actively inventories and decodes Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v62: use decoded child ObjectId patterns to choose the next typed cast batch for 37/27/8/18/22/5, or promote 44/4 correlations into symbolGraphics fallback improvements.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v62_child_objectid_decoder_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v62_child_objectid_decoder_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v62 child ObjectId decoder probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v62 child ObjectId decoder probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v62_child_objectid_decoder_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v62_child_objectid_decoder_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV62ChildObjectIdDecoderProbeReport;
    end
