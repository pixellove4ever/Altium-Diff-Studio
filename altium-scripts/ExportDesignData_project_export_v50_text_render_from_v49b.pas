{==============================================================================}
{ Altium Diff Studio - Exporter Script v50                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v50';
    SCHEMA_VERSION = 'ads-json-v50';
    SCHEMATIC_SCHEMA_VERSION = 'v50';
...
{ V50: typed, compile-visible helpers for component display text geometry.
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderV50",');
    JsonList.Add('      "graphicsNotes": "V50 keeps validated displayObjects and adds a viewer-ready textRender array for native designator/comment rendering. Text position and orientation come from typed component Designator/Comment display objects; symbol bodies still use pin-bounds fallback until native primitives are validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v50",');
    JsonList.Add('        "focus": "viewer-ready component text rendering for designator/comment plus primary display candidate",');
...
{==============================================================================}
{ 3f. TARGETED DISPLAY TEXT RENDER PROBE - v50 SAFE                    }
{==============================================================================}
{ Purpose:
...
function V50DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V50ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V50IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
procedure V50TextRenderDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V50IsTarget(Oid) then
        begin
            TargetRole := V50DecisionForObjectId(Oid);
            ExpectedKind := V50ExpectedTextKindForObjectId(Oid);
...
procedure ExportSchV50TextRenderReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v50 text render skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v50 text render skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v50RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('Altium schematic v50 display text render probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=export viewer-ready text render metadata for high-confidence display objects 25/45/46 using typed component Designator/Comment display accessors');
        ...
        Summary.Add('CsvColumns=sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v50RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V50TextRenderDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V50_TEXT_RENDER_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('Next step v51: probe native symbol body primitives 44/4 and integrate them into symbolGraphics.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v50_text_render.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v50_text_render_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v50 text render CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v50 text render summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v50_text_render.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v50_text_render_summary.txt'));
