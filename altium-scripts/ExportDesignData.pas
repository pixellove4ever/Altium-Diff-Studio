...
begin
    ...
    begin
        ...
        if Project <> nil then
            Result := ExtractFilePath(Project.DM_FullPath);
    end;
    if Result = '' then
        Result := 'C:\';
end;
...
begin
    ...
    begin
        ShowMessage('Schematic editor is not running.');
        Exit;
    end;
    ...
    if SchDoc = nil then
    begin
        ShowMessage('Please open a Schematic Document (.SchDoc) first.');
        Exit;
    end;
    ...
    try
        ...
        ShowMessage('Schematic exported successfully to:' + #13 + FilePath);
    finally
        JsonList.Free;
    end;
end;
...
begin
    ...
    begin
        ShowMessage('PCB editor is not running.');
        Exit;
    end;
    ...
    if Board = nil then
    begin
        ShowMessage('Please open a PCB Document (.PcbDoc) first.');
        Exit;
    end;
    ...
    try
        ...
        while Track <> nil do
        begin
            // Export tracks on copper layers primarily
            if Track.Layer in [eTopLayer, eBottomLayer, eMid1Layer..eMid30Layer] then
            begin
                if Count > 0 then JsonList.Add('    ,');
                JsonList.Add('    {');
        ...
        JsonList.SaveToFile(FilePath);
        ShowMessage('PCB exported successfully to:' + #13 + FilePath);
    finally
        JsonList.Free;
    end;
end;
...
begin
    ...
    if Workspace = nil then
    begin
        ShowMessage('Workspace Manager not found.');
        Exit;
    end;
    ...
    if Project = nil then
    begin
        ShowMessage('No focused project found in Altium.');
        Exit;
    end;

    FilePath := GetProjectPath + ChangeFileExt(ExtractFileName(Project.DM_FullPath), '_bom.json');
    JsonList := TStringList.Create;
    ...
    try
        ...
        // Iterate through all logical documents in the project
        for i := 0 to Project.DM_LogicalDocumentCount - 1 do
        begin
            Document := Project.DM_LogicalDocuments(i);
            Ext := AnsiUpperCase(ExtractFileExt(Document.DM_FullPath));

            // Load and iterate Schematic Documents (.SchDoc)
            if Ext = '.SCHDOC' then
            begin
                // Request server to open the schematic sheet if not already open
                ...
                begin
                    ...
                    while Component <> nil do
                    begin
                        // Exclude graphical annotations / non-components
                        ...
                        begin
                            ...
                            // Iterate component custom parameters (Manufacturer, MPN, etc.)
                            ParamCount := 0;
        ...
        JsonList.SaveToFile(FilePath);
        ShowMessage('Project BOM exported successfully to:' + #13 + FilePath);
    finally
        JsonList.Free;
    end;
end;
