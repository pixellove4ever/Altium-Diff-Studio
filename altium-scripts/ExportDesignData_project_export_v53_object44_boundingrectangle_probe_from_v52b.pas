{==============================================================================}
{ Altium Diff Studio - Exporter Script v53                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v53';
    SCHEMA_VERSION = 'ads-json-v53';
    SCHEMATIC_SCHEMA_VERSION = 'v53';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndObject44BoundingRectangleProbeV53",');
    JsonList.Add('      "graphicsNotes": "V53 keeps validated textRender and adds a one-property native geometry probe on ObjectId 44 using Obj.BoundingRectangle. Symbol bodies still use pin-bounds fallback in the JSON until ObjectId 44 rectangle geometry is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v53",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v53 OBJECT44 BOUNDINGRECTANGLE                    }
{==============================================================================}
{ Purpose:
...
function V53DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V53ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V53IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V53TryObject44BoundingRectangle(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X1 := 0;
    Y1 := 0;
    ...
    { V53 intentionally tests only one native geometry property on ObjectId 44:
      Obj.BoundingRectangle. If this compiles and runs, it should tell us
      whether ObjectId 44 exposes a usable native rectangle for symbol graphics. }
    try
        X1 := Obj.BoundingRectangle.Left;
        Y1 := Obj.BoundingRectangle.Bottom;
        X2 := Obj.BoundingRectangle.Right;
        Y2 := Obj.BoundingRectangle.Top;
        Status := 'Obj.BoundingRectangle_OK_LeftBottomRightTop';
        Result := True;
    except
        Status := 'Obj.BoundingRectangle_RUNTIME_ERROR';
        Result := False;
    end;
end;

procedure V53Object44BoundingRectangleProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V53IsTarget(Oid) then
        begin
            TargetRole := V53DecisionForObjectId(Oid);
            ExpectedKind := V53ExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V53TryObject44BoundingRectangle(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
...
procedure ExportSchV53Object44BoundingRectangleProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v53 ObjectId 44 BoundingRectangle probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v53 ObjectId 44 BoundingRectangle probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v53RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeBoundsX1;nativeBoundsY1;nativeBoundsX2;nativeBoundsY2;nativeBoundsStatus;nativeBoundsSource');
        Summary.Add('Altium schematic v53 ObjectId 44 BoundingRectangle probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=ObjectId 44 candidate probe testing one geometry property: Obj.BoundingRectangle.Left/Bottom/Right/Top');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 additionally probes Obj.BoundingRectangle inside try/except; text geometry read only through typed ISch_Component.Designator/Comment Location accessors');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V53Object44BoundingRectangleProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V53_OBJECT44_BOUNDINGRECTANGLE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 BoundingRectangle is the single native geometry property tested by V53.');
        Summary.Add('Next step v54: if BoundingRectangle works, integrate it as native symbol bounds; otherwise test a different ObjectId 44 geometry property.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53_object44_boundingrectangle_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53_object44_boundingrectangle_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v53 ObjectId 44 BoundingRectangle probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v53 ObjectId 44 BoundingRectangle probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53_object44_boundingrectangle_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53_object44_boundingrectangle_probe_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV53Object44BoundingRectangleProbeReport;
    end
