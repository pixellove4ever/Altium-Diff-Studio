{==============================================================================}
{ Altium Diff Studio - Exporter Script v52                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v52';
    SCHEMA_VERSION = 'ads-json-v52';
    SCHEMATIC_SCHEMA_VERSION = 'v52';
...
{ V52: typed, compile-visible helpers for component display text geometry.
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndObject44LocationProbeV52",');
    JsonList.Add('      "graphicsNotes": "V52 keeps validated textRender and adds a one-property native geometry probe on ObjectId 44 using late-bound Obj.Location.X/Y. Symbol bodies still use pin-bounds fallback in the JSON until ObjectId 44 Location geometry is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v52",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v52 SAFE                    }
{==============================================================================}
{ Purpose:
...
function V52DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V52ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V52IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V52TryObjectLocationXY(Obj : Variant; var X : Integer; var Y : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X := 0;
    Y := 0;
    ...
    try
        X := Obj.Location.X;
        Y := Obj.Location.Y;
        Status := 'Obj.Location.XY_OK';
        Result := True;
    except
        Status := 'Obj.Location.XY_NOT_AVAILABLE';
    end;
end;

procedure V52Object44LocationProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V52IsTarget(Oid) then
        begin
            TargetRole := V52DecisionForObjectId(Oid);
            ExpectedKind := V52ExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V52TryObjectLocationXY(Obj, NativeX, NativeY, NativeStatus);
...
procedure ExportSchV52Object44LocationProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v52 ObjectId 44 Location probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v52 ObjectId 44 Location probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v52RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeLocationX;nativeLocationY;nativeLocationStatus;nativeLocationSource');
        Summary.Add('Altium schematic v52 ObjectId 44 Location probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=probe one native geometry property on ObjectId 44: late-bound Obj.Location.X/Y, while preserving validated textRender 45/46');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V52Object44LocationProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V52_OBJECT44_LOCATION_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 is the only ObjectId 44 Location candidate with a new geometry access attempt in V52: Obj.Location.X/Y via Variant late binding and try/except.');
        Summary.Add('Next step v53: if ObjectId 44 Location succeeds, use it to classify its role; otherwise probe a second single property such as BoundingRectangle or Location variants.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52_object44_location_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52_object44_location_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v52 ObjectId 44 Location probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v52 ObjectId 44 Location probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52_object44_location_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52_object44_location_probe_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV52Object44LocationProbeReport;
    end
