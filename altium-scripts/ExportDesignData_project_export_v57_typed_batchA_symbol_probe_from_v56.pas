{==============================================================================}
{ Altium Diff Studio - Exporter Script v57                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v57';
    SCHEMA_VERSION = 'ads-json-v57';
    SCHEMATIC_SCHEMA_VERSION = 'v57';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndTypedBatchASymbolProbeV57",');
    JsonList.Add('      "graphicsNotes": "V57 keeps validated textRender and accelerates native symbol discovery with a compile-safe typed batch-A matrix for ObjectId 44/4/25: component context, LibRef grouping, fallback scoring, and typed-interface candidate batches. Symbol bodies still use pin-bounds fallback in the JSON until one typed primitive interface is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v57",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v57 INTERFACE MATRIX COMPILE-SAFE             }
{==============================================================================}
{ Purpose:
...
function V57DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V57ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V57IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V57InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V57CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V57TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'TYPED_BATCH_A_NO_MATCH';

    { V57 is intentionally more ambitious than v52-v56:
      - it stops trying unsupported generic Obj.Location / Obj.X1 properties;
      - it tests typed schematic interfaces on ObjectId 44 only;
      - each candidate is isolated in try/except so one failed cast does not stop the run.

      Geometry mapping used here:
      ISch_Line/ISch_Rectangle : Location -> X1/Y1, Corner -> X2/Y2
      ISch_Arc                : Location -> X1/Y1, Radius -> X2, Y2 reserved

      If one property name is still rejected by the compiler on the user's Altium build,
      the next micro-fix should disable only that specific typed branch, not the whole probe. }

    try
        LineObj := Obj;
        if LineObj <> nil then
        begin
            X1 := LineObj.Location.X;
            Y1 := LineObj.Location.Y;
            X2 := LineObj.Corner.X;
            Y2 := LineObj.Corner.Y;
            Status := 'ISch_Line_LOCATION_CORNER_OK';
            Result := True;
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_OR_LOCATION_CORNER_FAILED';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            X1 := RectObj.Location.X;
            Y1 := RectObj.Location.Y;
            X2 := RectObj.Corner.X;
            Y2 := RectObj.Corner.Y;
            Status := 'ISch_Rectangle_LOCATION_CORNER_OK';
            Result := True;
            Exit;
        end;
    except
        if Status = 'TYPED_BATCH_A_NO_MATCH' then
            Status := 'ISch_Rectangle_CAST_OR_LOCATION_CORNER_FAILED'
        else
            Status := Status + '|ISch_Rectangle_CAST_OR_LOCATION_CORNER_FAILED';
    end;
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            X1 := ArcObj.Location.X;
            Y1 := ArcObj.Location.Y;
            X2 := ArcObj.Radius;
            Y2 := 0;
            Status := 'ISch_Arc_LOCATION_RADIUS_OK';
            Result := True;
            Exit;
        end;
    except
        if Status = 'TYPED_BATCH_A_NO_MATCH' then
            Status := 'ISch_Arc_CAST_OR_LOCATION_RADIUS_FAILED'
        else
            Status := Status + '|ISch_Arc_CAST_OR_LOCATION_RADIUS_FAILED';
    end;
end;


procedure V57TypedBatchASymbolProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V57IsTarget(Oid) then
        begin
            TargetRole := V57DecisionForObjectId(Oid);
            ExpectedKind := V57ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V57InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V57CompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V57TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and ((NativeStatus = 'ISch_Line_LOCATION_CORNER_OK') or (NativeStatus = 'ISch_Rectangle_LOCATION_CORNER_OK') or (NativeStatus = 'ISch_Arc_LOCATION_RADIUS_OK')) then
                    begin
                        DisplayX := NativeX1;
                        DisplayY := NativeY1;
                        GeometrySource := 'typedBatchA:' + NativeStatus;
                    end
                    else
...
procedure ExportSchV57TypedBatchASymbolProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v57 typed batch-A symbol primitive probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v57 typed batch-A symbol primitive probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v57RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;typedBatchA_v57;batchResultPlan_v57;recommendedNextAction');
        Summary.Add('Altium schematic v57 typed batch-A symbol primitive probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=cleaned v57 accelerated typed batch-A probe after generic Location/BoundingRectangle/X1/Y1/X2/Y2 failures; ObjectId 44 is tested through typed ISch_Line/ISch_Rectangle/ISch_Arc interfaces rather than generic properties.');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 generic properties remain disabled; v57 actively tests typed batch A interfaces only: ISch_Line, ISch_Rectangle and ISch_Arc.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V57TypedBatchASymbolProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V57_TYPED_BATCHA_SYMBOL_PRIMITIVE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 generic geometry remains disabled; v57 actively tries typed batch A: ISch_Line / ISch_Rectangle / ISch_Arc.');
        Summary.Add('Next step after v57: if batch A validates, integrate matching primitives into symbolGraphics; otherwise disable only the failing typed branch and move to batch B or child-iterator route.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57_typed_batchA_symbol_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57_typed_batchA_symbol_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v57 typed batch-A symbol primitive probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v57 typed batch-A symbol primitive probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57_typed_batchA_symbol_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57_typed_batchA_symbol_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV57TypedBatchASymbolProbeReport;
    end
