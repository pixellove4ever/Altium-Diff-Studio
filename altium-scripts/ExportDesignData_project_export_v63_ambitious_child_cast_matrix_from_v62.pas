{==============================================================================}
{ Altium Diff Studio - Exporter Script v63                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v63';
    SCHEMA_VERSION = 'ads-json-v63';
    SCHEMATIC_SCHEMA_VERSION = 'v63';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndAmbitiousChildCastMatrixProbeV63",');
    JsonList.Add('      "graphicsNotes": "V63 keeps validated textRender and switches native symbol discovery away from direct ObjectId 44 geometry access toward a compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v63",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe ambitious child cast matrix probe for native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v63 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v63  : active réellement Component.SchIterator_Create sur le composant courant, sans géométrie,
          pour inventorier les ObjectId enfants et vérifier si 44/4 apparaissent dans le symbole.
 ...
 - on active un inventaire enfant objectId-only via Component.SchIterator_Create pour accélérer.
======================================================================================}
...
function V63DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
    if ObjectId = 4 then Result := 'componentLocalPrimitiveOrAnchor_candidate'
    else if ObjectId = 5 then Result := 'childCandidate_5_castMatrixTarget'
    else if ObjectId = 8 then Result := 'childCandidate_8_castMatrixTarget'
    else if ObjectId = 18 then Result := 'childCandidate_18_castMatrixTarget'
    else if ObjectId = 22 then Result := 'childCandidate_22_castMatrixTarget'
    else if ObjectId = 25 then Result := 'componentPrimaryDisplayObject_candidateDesignatorOrPrimaryText'
    else if ObjectId = 27 then Result := 'childCandidate_27_castMatrixTarget'
    else if ObjectId = 37 then Result := 'childCandidate_37_castMatrixTarget'
    else if ObjectId = 44 then Result := 'componentNativeSymbolGraphic_candidate_castMatrixHighPriority'
...
function V63ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 4 then Result := 'nativePrimitiveOrAnchor_noTextExpected'
    else if (ObjectId = 5) or (ObjectId = 8) or (ObjectId = 18) or (ObjectId = 22) or (ObjectId = 27) or (ObjectId = 37) then Result := 'childCandidate_castOnly_noTextExpectedYet'
...
function V63IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
    if (ObjectId = 4) or (ObjectId = 5) or (ObjectId = 8) or (ObjectId = 18) or
       (ObjectId = 22) or (ObjectId = 25) or (ObjectId = 27) or (ObjectId = 37) or
       (ObjectId = 44) or (ObjectId = 45) or (ObjectId = 46) then
        Result := True;
end;
...
function V63DecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V63AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V63DecodeChildObjectId(ObjectId);
...
function V63InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
        Result := 'activeCastMatrix:ISch_Line/ISch_Rectangle/ISch_Arc;plannedOnly:ISch_Ellipse/ISch_Polygon/ISch_Bezier;priority=highest'
    else if ObjectId = 4 then
        Result := 'activeCastMatrix:ISch_Line/ISch_Rectangle/ISch_Arc;priority=componentLocalPrimitiveOrAnchor'
    else if (ObjectId = 5) or (ObjectId = 8) or (ObjectId = 18) or (ObjectId = 22) or (ObjectId = 27) or (ObjectId = 37) then
        Result := 'activeCastMatrix:ISch_Line/ISch_Rectangle/ISch_Arc;priority=decodedChildCandidate'
    else if ObjectId = 25 then
        Result := 'activeCastMatrix:ISch_Line/ISch_Rectangle/ISch_Arc;alsoKnownAsPrimaryDisplayCandidate'
    else if (ObjectId = 45) or (ObjectId = 46) then
        Result := 'textDisplayObject_anchorOnly_castMatrixLoggedForControl';
end;

function V63CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
    Result := 'castOnlySafeNoGeometryRead';
    if ObjectId = 44 then
        Result := 'useCastMatrixResultToChooseNextSingleGeometryAccessor;doNotUseGenericObjProperties'
    else if ObjectId = 4 then
        Result := 'keepInMatrixButLowerPriorityThan44'
    else if (ObjectId = 5) or (ObjectId = 8) or (ObjectId = 18) or (ObjectId = 22) or (ObjectId = 27) or (ObjectId = 37) then
        Result := 'decodedChildCandidate_castOnlySafe_nextDecideByInterfaceHit'
    else if ObjectId = 25 then
        Result := 'excludeFromNativeBodyUnlessInterfaceCastProvesGraphic'
    else if (ObjectId = 45) or (ObjectId = 46) then
        Result := 'textAnchorValidated_controlRows';
end;


function V63ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V63AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V63IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V63AppendStatus(const ExistingStatus : String; const AddStatus : String) : String;
begin
    if ExistingStatus = '' then
        Result := AddStatus
    else
        Result := ExistingStatus + '|' + AddStatus;
end;

function V63TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    { V63 AMBITIOUS CAST MATRIX / HISTORY TRACE
      -------------------------------------------------------------------------
      Goal: skip several single-version iterations by running a cast-only matrix
      on all recurrent child candidates: ObjectId 37/27/8/18/22/5 plus 44/4.

      Executable rules:
      - CAST ONLY. No .Location, .BoundingRectangle, .StartLocation, .X1, .Corner,
        .Radius, .GetState_Vertex or any other geometry property is read here.
      - Active interfaces are restricted to identifiers already used successfully
        by previous compile attempts: ISch_Line, ISch_Rectangle, ISch_Arc.
      - Wider families are recorded in the CSV plan, but left inactive until one
        of these cast-only results tells us which family deserves a one-off test.

      Confirmed historical failures to avoid repeating:
      - Obj.Location.X/Y                         -> undeclared Location.
      - Obj.BoundingRectangle                    -> undeclared BoundingRectangle.
      - Obj.X1/Y1/X2/Y2                          -> not safely exposed generically.
      - ISch_Line.Location                       -> undeclared Location.
      - ISch_Line.GetState_Vertex(0/1)           -> undeclared GetState_Vertex.
      - Variant StartLocation/EndLocation        -> undeclared StartLocation.
    }
    ...
    try
        ...
        if LineObj <> nil then
        begin
            Status := V63AppendStatus(Status, 'ISch_Line_CAST_OK');
            Result := True;
        end
        else
            Status := V63AppendStatus(Status, 'ISch_Line_CAST_NIL');
    except
        Status := V63AppendStatus(Status, 'ISch_Line_CAST_FAILED');
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := V63AppendStatus(Status, 'ISch_Rectangle_CAST_OK');
            Result := True;
        end
        else
            Status := V63AppendStatus(Status, 'ISch_Rectangle_CAST_NIL');
    except
        Status := V63AppendStatus(Status, 'ISch_Rectangle_CAST_FAILED');
    end;
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := V63AppendStatus(Status, 'ISch_Arc_CAST_OK');
            Result := True;
        end
        else
            Status := V63AppendStatus(Status, 'ISch_Arc_CAST_NIL');
    except
        Status := V63AppendStatus(Status, 'ISch_Arc_CAST_FAILED');
    end;

    if Status = '' then
        Status := 'V63_CAST_MATRIX_NO_RESULT';
    Status := Status + '|GEOMETRY_NOT_READ_V63_CAST_ONLY';
end;

procedure V63AmbitiousChildCastMatrixProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V63IsTarget(Oid) then
        begin
            TargetRole := V63DecisionForObjectId(Oid);
            ExpectedKind := V63ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V63InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V63CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ...
            if CurrentComp <> nil then
                V63ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if V63IsTarget(Oid) then
            begin
                V63TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'v63CastOnlyMatrixOnCurrentIteratorObject';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'ambitiousCastMatrix_v63:' + NativeStatus
                    else
...
procedure ExportSchV63AmbitiousChildCastMatrixProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v63 ambitious child cast matrix probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v63 ambitious child cast matrix probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v63RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;ambitiousCastMatrixProbe_v63;childIteratorObjectCount_v63;childIteratorObjectIds_v63;childIteratorDecodedRoles_v63;childIteratorTargetOrCandidateHits_v63;routeResultPlan_v63;recommendedNextAction');
        Summary.Add('Altium schematic v63 ambitious child cast matrix probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v63 ambitious compile-safe cast matrix: run cast-only checks on child ObjectIds 37/27/8/18/22/5 plus 44/4, while preserving textRender and avoiding all risky geometry accessors.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference/Location, cast-only ISch_Line/ISch_Rectangle/ISch_Arc matrix; no direct generic/Variant geometry property is executed.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V63AmbitiousChildCastMatrixProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V63_AMBITIOUS_CHILD_CAST_MATRIX_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 / 37 / 27 / 8 / 18 / 22 / 5 rows are included in the active cast-only matrix for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v63 actively runs cast-only ISch_Line/ISch_Rectangle/ISch_Arc checks on all decoded child candidates.');
        Summary.Add('Next step after v63: inspect which ObjectIds cast OK to which interface, then enable exactly one confirmed geometry accessor in v64.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63_ambitious_child_cast_matrix_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63_ambitious_child_cast_matrix_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v63 ambitious child cast matrix probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v63 ambitious child cast matrix probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63_ambitious_child_cast_matrix_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63_ambitious_child_cast_matrix_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV63AmbitiousChildCastMatrixProbeReport;
    end
