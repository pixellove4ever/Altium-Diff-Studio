...
begin
    ...
    try
        ...
        // 2a. Board Outline (Contour de carte)
        JsonList.Add('  "boardOutline": [');
        if PcbBoard.BoardOutline <> nil then
        begin
            for i := 1 to PcbBoard.BoardOutline.VerticesCount do
            begin
                if i > 1 then JsonList.Add('    ,');
                JsonList.Add('    { "x": ' + JsonFloat(CoordToMMs(PcbBoard.BoardOutline.Vertex[i].X)) + 
                             ', "y": ' + JsonFloat(CoordToMMs(PcbBoard.BoardOutline.Vertex[i].Y)) + ' }');
            end;
        end;
        JsonList.Add('  ],');
        ...
        // 2c. Tracks (Filtre élargi à la sérigraphie, au Keepout et au Mécanique)
        JsonList.Add('  "tracks": [');
        Count := 0;
        Iterator := PcbBoard.BoardIterator_Create;
        ...
        while Track <> nil do
        begin
            if Track.Layer in [eTopLayer, eBottomLayer, eMid1Layer..eMid30Layer, 
                               eTopSilkLayer, eBottomSilkLayer, eKeepOutLayer, 
                               eMechanicalLayer1..eMechanicalLayer32] then
            begin
                if Count > 0 then JsonList.Add('    ,');
                JsonList.Add('    {');
        ...
        // 2d. Arcs (Sérigraphie et formes mécaniques arrondies)
        JsonList.Add('  "arcs": [');
        Count := 0;
        Iterator := PcbBoard.BoardIterator_Create;
        ...
        while Arc <> nil do
        begin
            if Arc.Layer in [eTopLayer, eBottomLayer, eMid1Layer..eMid30Layer, 
                             eTopSilkLayer, eBottomSilkLayer, eKeepOutLayer, 
                             eMechanicalLayer1..eMechanicalLayer32] then
            begin
                if Count > 0 then JsonList.Add('    ,');
                ...
                JsonList.Add('      "width": ' + JsonFloat(CoordToMMs(Arc.Width)) + '');
                JsonList.Add('    }');
                Inc(Count);
            end;
        ...
        begin
            if Count > 0 then JsonList.Add('    ,');
            JsonList.Add('    {');
            JsonList.Add('      "id": "' + 'POLY_' + IntToStr(Count) + '",');
            JsonList.Add('      "layer": "' + Layer2String(Poly.Layer) + '",');
            if Poly.Net <> nil then
                JsonList.Add('      "net": "' + EscapeStr(Poly.Net.Name) + '",')
            else
                JsonList.Add('      "net": "",');
            
            JsonList.Add('      "vertices": [');
            for i := 1 to Poly.VerticesCount do
            begin
                if i > 1 then JsonList.Add('        ,');
                JsonList.Add('        { "x": ' + JsonFloat(CoordToMMs(Poly.Vertex[i].X)) + 
                             ', "y": ' + JsonFloat(CoordToMMs(Poly.Vertex[i].Y)) + ' }');
            end;
            JsonList.Add('      ]');
            JsonList.Add('    }');
            Inc(Count);
        ...
        // 2h. Texts (Pour la sérigraphie et annotations libres)
        JsonList.Add('  "texts": [');
        Count := 0;
        Iterator := PcbBoard.BoardIterator_Create;
        ...
        begin
            if TextObj.Layer in [eTopSilkLayer, eBottomSilkLayer, eMechanicalLayer1..eMechanicalLayer32] then
            begin
                if Count > 0 then JsonList.Add('    ,');
                JsonList.Add('    {');
                ...
                JsonList.Add('      "x": ' + JsonFloat(CoordToMMs(TextObj.XLocation)) + ',');
                JsonList.Add('      "y": ' + JsonFloat(CoordToMMs(TextObj.YLocation)) + ',');
                JsonList.Add('      "height": ' + JsonFloat(CoordToMMs(TextObj.TextHeight)) + ',');
