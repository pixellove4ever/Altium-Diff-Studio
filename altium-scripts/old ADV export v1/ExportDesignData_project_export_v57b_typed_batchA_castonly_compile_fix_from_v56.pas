{==============================================================================}
{ Altium Diff Studio - Exporter Script v57b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v57b';
    SCHEMA_VERSION = 'ads-json-v57bb';
    SCHEMATIC_SCHEMA_VERSION = 'v57b';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndTypedBatchASymbolProbeV57b",');
    JsonList.Add('      "graphicsNotes": "V57b keeps validated textRender and accelerates native symbol discovery with a compile-safe typed batch-A matrix for ObjectId 44/4/25: component context, LibRef grouping, fallback scoring, and typed-interface candidate batches. Symbol bodies still use pin-bounds fallback in the JSON until one typed primitive interface is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v57b",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v57b INTERFACE MATRIX COMPILE-SAFE            }
{======================================================================================
...
function V57bDecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V57bExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V57bIsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V57bInterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V57bCompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V57bTryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'TYPED_BATCH_A_CASTONLY_NO_MATCH';

    { v57b compile fix:
      The v57 typed geometry branch failed because LineObj.Location / RectObj.Location /
      ArcObj.Location are not exposed by this Altium scripting API for those interfaces.
      We therefore keep a narrower and faster test: interface-cast only.
      This still tells us whether ObjectId 44 can be treated as ISch_Line, ISch_Rectangle,
      or ISch_Arc. If a cast validates, v58 can test the correct geometry property names
      for that specific interface only, instead of trying every ObjectId/property pair. }

    try
        LineObj := Obj;
        if LineObj <> nil then
        begin
            Status := 'ISch_Line_CAST_OK_GEOMETRY_NOT_READ_V57B';
            Result := True;
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := 'ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V57B';
            Result := True;
            Exit;
        end;
    except
        if Status = 'TYPED_BATCH_A_CASTONLY_NO_MATCH' then
            Status := 'ISch_Rectangle_CAST_FAILED'
        else
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := 'ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V57B';
            Result := True;
            Exit;
        end;
    except
        if Status = 'TYPED_BATCH_A_CASTONLY_NO_MATCH' then
            Status := 'ISch_Arc_CAST_FAILED'
        else
            Status := Status + '|ISch_Arc_CAST_FAILED';
    end;
end;

procedure V57bTypedBatchASymbolProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V57bIsTarget(Oid) then
        begin
            TargetRole := V57bDecisionForObjectId(Oid);
            ExpectedKind := V57bExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V57bInterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V57bCompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V57bTryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and ((NativeStatus = 'ISch_Line_CAST_OK_GEOMETRY_NOT_READ_V57B') or (NativeStatus = 'ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V57B') or (NativeStatus = 'ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V57B')) then
                    begin
...
procedure ExportSchV57bTypedBatchASymbolProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v57b typed batch-A cast-only symbol primitive probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v57b typed batch-A cast-only symbol primitive probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v57bRoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;typedBatchA_v57b;batchResultPlan_v57b;recommendedNextAction');
        Summary.Add('Altium schematic v57b typed batch-A cast-only symbol primitive probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=cleaned v57b accelerated typed batch-A probe after generic Location/BoundingRectangle/X1/Y1/X2/Y2 failures; ObjectId 44 is tested through typed ISch_Line/ISch_Rectangle/ISch_Arc interface casts only; no Location/Corner/Radius properties are read in v57b.');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 generic properties remain disabled; v57b actively tests typed batch A interface casts only: ISch_Line, ISch_Rectangle and ISch_Arc.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V57bTypedBatchASymbolProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V57b_TYPED_BATCHA_SYMBOL_PRIMITIVE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 generic geometry remains disabled; v57b actively tries typed batch A: ISch_Line / ISch_Rectangle / ISch_Arc.');
        Summary.Add('Next step after v57b: if a cast validates, v58 should probe only the matching interface property names; otherwise move to batch B or child-iterator route.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57b_typed_batchA_castonly_symbol_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57b_typed_batchA_castonly_symbol_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v57b typed batch-A cast-only symbol primitive probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v57b typed batch-A cast-only symbol primitive probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57b_typed_batchA_castonly_symbol_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v57b_typed_batchA_castonly_symbol_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV57bTypedBatchASymbolProbeReport;
    end
