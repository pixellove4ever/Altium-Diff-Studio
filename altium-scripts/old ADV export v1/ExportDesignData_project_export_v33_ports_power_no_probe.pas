{==============================================================================}
{ Altium Diff Studio - Exporter Script v33                                         }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v33';
    SCHEMA_VERSION = 'ads-json-v33';
    SCHEMATIC_SCHEMA_VERSION = 'v33';
    PCB_OUTPUT_SUFFIX = '_pcb_v33.json';
    constKindPcb = 'PCB';
...
begin
    ...
    JsonList.Add('    "notes": "V33 tags each keepout edge and exports arc-aware viewer-ready render bounds. width <= 1.1mm is treated as boardOutlineCandidate; wider edges are kept as localKeepoutOrCutoutCandidate. Arc bounds now use the swept arc endpoints and cardinal extrema only when they lie inside the arc sweep."');
    JsonList.Add('  },');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV33",');
    JsonList.Add('      "graphicsNotes": "V33 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v33",');
    ...
    begin
        ...
        JsonList.Add('          "text": "' + EscapeStr(PortObj.Text) + '",');
        JsonList.Add('          "x": ' + IntToStr(PortObj.Location.X) + ',');
        JsonList.Add('          "y": ' + IntToStr(PortObj.Location.Y) + ',');
        JsonList.Add('          "orientation": ' + IntToStr(PortObj.Orientation));
        JsonList.Add('        }');
        Inc(PortCount);
    ...
    begin
        ...
        JsonList.Add('          "orientation": ' + IntToStr(PowerObj.Orientation));
        JsonList.Add('        }');
        Inc(PowerCount);
    ...
    JsonList.Add('        "notes": "V33 keeps SCH extraction compile-safe in the target Altium version: components, pins, wires and net labels are exported; junctions, buses, directives and native symbol graphics remain typed empty containers; ports and power ports are exported when the target Altium API exposes ePort/ePowerObject until their exact object enums/properties are validated."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v33: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount) + ' ports=' + IntToStr(PortCount) + ' powerPorts=' + IntToStr(PowerCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V33 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV33",');
        JsonList.Add('  "compatibilityMode": "v33-safe-sch-containers",');
