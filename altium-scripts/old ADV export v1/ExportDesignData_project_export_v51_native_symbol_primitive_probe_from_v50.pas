{==============================================================================}
{ Altium Diff Studio - Exporter Script v51                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v51';
    SCHEMA_VERSION = 'ads-json-v51';
    SCHEMATIC_SCHEMA_VERSION = 'v51';
...
{ V51: typed, compile-visible helpers for component display text geometry.
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndNativePrimitiveProbeV51",');
    JsonList.Add('      "graphicsNotes": "V51 keeps validated textRender and adds a targeted native symbol primitive probe for ObjectId 44 / 4 / 25 around components. Symbol bodies still use pin-bounds fallback in the JSON until native primitive properties are validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v51",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus targeted native symbol primitive probe",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v51 SAFE                    }
{==============================================================================}
{ Purpose:
...
function V51DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V51ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V51IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
procedure V51NativePrimitiveProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V51IsTarget(Oid) then
        begin
            TargetRole := V51DecisionForObjectId(Oid);
            ExpectedKind := V51ExpectedTextKindForObjectId(Oid);
...
procedure ExportSchV51NativePrimitiveProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v51 native primitive probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v51 native primitive probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v51RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('Altium schematic v51 native symbol primitive probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=probe native symbol primitive candidates ObjectId 44/4/25 around components while preserving validated textRender 45/46');
        Summary.Add('SafeProperties=ObjectId and iterator order only on unknown primitive candidates; text geometry read only through typed ISch_Component.Designator/Comment Location accessors');
        Summary.Add('CsvColumns=sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v51RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V51NativePrimitiveProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V51_NATIVE_PRIMITIVE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('No generic geometry property is read from ObjectId 44/4 yet; this version proves association and ordering safely.');
        Summary.Add('Next step v52: attempt one native primitive geometry property at a time on ObjectId 44, starting from the clusters found here.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v51_native_primitive_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v51_native_primitive_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v51 native primitive probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v51 native primitive probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v51_native_primitive_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v51_native_primitive_probe_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV51NativePrimitiveProbeReport;
    end
