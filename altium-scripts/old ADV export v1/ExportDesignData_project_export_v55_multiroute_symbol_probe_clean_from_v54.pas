{==============================================================================}
{ Altium Diff Studio - Exporter Script v55                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v55';
    SCHEMA_VERSION = 'ads-json-v55';
    SCHEMATIC_SCHEMA_VERSION = 'v55';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndMultiRouteSymbolPrimitiveProbeV55",');
    JsonList.Add('      "graphicsNotes": "V55 keeps validated textRender and adds a faster compile-safe multi-route symbol primitive probe: ObjectId cluster mapping, component text anchors, LibRef grouping, fallback-quality scoring, and explicit disabled native-access candidates. Symbol bodies still use pin-bounds fallback in the JSON until native primitives are validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v55",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe multi-route native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v55 MULTI-ROUTE COMPILE-SAFE             }
{==============================================================================}
{ Purpose:
...
function V55DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V55ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V55IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V55TryMultiRouteSymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X1 := 0;
    Y1 := 0;
    ...
    { Direct generic accesses such as Obj.X1 / Obj.Y1 / Obj.X2 / Obj.Y2
      ...
      catch anything. V55 therefore records a clean compile-safe baseline for
      line-like geometry candidates and keeps the ObjectId 44/4/25 cluster map. }
end;

procedure V55MultiRouteSymbolPrimitiveProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V55IsTarget(Oid) then
        begin
            TargetRole := V55DecisionForObjectId(Oid);
            ExpectedKind := V55ExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V55TryMultiRouteSymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
...
procedure ExportSchV55MultiRouteSymbolPrimitiveProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v55 multi-route symbol primitive probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v55 multi-route symbol primitive probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v55RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;recommendedNextAction');
        Summary.Add('Altium schematic v55 multi-route symbol primitive probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=cleaned v55 accelerated compile-safe multi-route probe after generic Location/BoundingRectangle/X1/Y1/X2/Y2 failures; multiple safe routes are evaluated in one run without direct unsupported properties.');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 multi-route symbol primitives are marked skipped because generic direct property access is not supported by compiler');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V55MultiRouteSymbolPrimitiveProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V55_MULTIROUTE_SYMBOL_PRIMITIVE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 direct native geometry access remains disabled in V55; this version accelerates by collecting all safe routes and next-action hints in one CSV.');
        Summary.Add('Next step v56: choose one real typed interface/child iterator candidate from the V55 map and enable only that compile-risk branch.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v55_multiroute_symbol_primitive_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v55_multiroute_symbol_primitive_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v55 multi-route symbol primitive probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v55 multi-route symbol primitive probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v55_multiroute_symbol_primitive_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v55_multiroute_symbol_primitive_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV55MultiRouteSymbolPrimitiveProbeReport;
    end
