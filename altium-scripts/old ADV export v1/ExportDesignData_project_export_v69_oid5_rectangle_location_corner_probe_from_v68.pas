{==============================================================================}
{ Altium Diff Studio - Exporter Script v69 OID5 RECTANGLE LOCATION/CORNER PROBE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v69';
    SCHEMA_VERSION = 'ads-json-v69';
    SCHEMATIC_SCHEMA_VERSION = 'v69';
...
begin
    ...
    Iterator := PcbBoard.BoardIterator_Create;
    Iterator.AddFilter_ObjectSet(MkSet(eRectObject));
    Iterator.AddFilter_LayerSet(MkSet(eKeepOutLayer));
    Iterator.AddFilter_Method(eProcessAll);
    Rect := Iterator.FirstPCBObject;
    while Rect <> nil do
    begin
        Inc(RectCandidateCount);
        if CoordToMMs(Rect.LineWidth) <= 1.1 then
        begin
            TrackClass := 'boardOutlineCandidate';
            Inc(OutlineCandidateCount);
        end
        ...
        CXmm := CoordToMMs(Rect.XCenter);
        CYmm := CoordToMMs(Rect.YCenter);
        Rmm := CoordToMMs(Rect.Radius);

        { V31: use true swept rect bounds, not full-circle bounds. }
        IncludeRectSweepBounds(HasKeepoutBounds, KeepoutMinX, KeepoutMinY, KeepoutMaxX, KeepoutMaxY, CXmm, CYmm, Rmm, Rect.StartAngle, Rect.EndAngle);
        if TrackClass = 'boardOutlineCandidate' then
            IncludeRectSweepBounds(HasCandidateBounds, CandidateMinX, CandidateMinY, CandidateMaxX, CandidateMaxY, CXmm, CYmm, Rmm, Rect.StartAngle, Rect.EndAngle);

        if OutlineEdgeCount > 0 then JsonList.Add('    ,');
        JsonList.Add('    {');
        JsonList.Add('      "id": "OUTLINE_EDGE_' + IntToStr(OutlineEdgeCount) + '",');
        JsonList.Add('      "type": "rect",');
        JsonList.Add('      "source": "keepoutLayerRect",');
        JsonList.Add('      "edgeClass": "' + TrackClass + '",');
        JsonList.Add('      "layer": "' + EscapeStr(Layer2String(Rect.Layer)) + '",');
        JsonList.Add('      "center": { "x": ' + JsonFloat(CXmm) + ', "y": ' + JsonFloat(CYmm) + ' },');
        JsonList.Add('      "radius": ' + JsonFloat(Rmm) + ',');
        JsonList.Add('      "startAngle": ' + JsonFloat(Rect.StartAngle) + ',');
        JsonList.Add('      "endAngle": ' + JsonFloat(Rect.EndAngle) + ',');
        JsonList.Add('      "width": ' + JsonFloat(CoordToMMs(Rect.LineWidth)));
        JsonList.Add('    }');
        Inc(OutlineEdgeCount);
        Rect := Iterator.NextPCBObject;
    end;
    PcbBoard.BoardIterator_Destroy(Iterator);
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineEdgesSource": "keepOutLayerTracksAndRects_InteractiveHTMLBOMStyle",');
    JsonList.Add('  "boardOutlineEdgesCount": ' + IntToStr(OutlineEdgeCount) + ',');
    JsonList.Add('  "boardOutlineClassification": {');
    JsonList.Add('    "method": "widthHeuristicV31_rectAwareBounds",');
    ...
    JsonList.Add('    "notes": "V33e tags each keepout edge and exports rect-aware viewer-ready render bounds. width <= 1.1mm is treated as boardOutlineCandidate; wider edges are kept as localKeepoutOrCutoutCandidate. Rect bounds now use the swept rect endpoints and cardinal extrema only when they lie inside the rect sweep."');
    JsonList.Add('  },');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV69",');
    JsonList.Add('      "graphicsNotes": "V69 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v69",');
    JsonList.Add('        "focus": "safe rollback after v63 hang: text rendering plus child ObjectId decoder only, no cast matrix",');
...
{==============================================================================}
{ 3g. V69 MINI-PROBE ISOLEE OBJECTID 5 - RECTANGLE LOCATION/CORNER READ       }
{==============================================================================}
{ Objectif v69:
  - partir de la V68 validee;
  - ne pas refaire de matrice large de casts;
  - isoler seulement ObjectId 5, qui etait recurrent dans V67;
  - tenter UNE seule lecture geometrique typee: ISch_Rectangle.Location + ISch_Rectangle.Corner;
  - budget petit et log avant/apres pour eviter un nouveau blocage Altium.

  Historique conserve:
  - V63 ambitieuse : matrice large de casts COM => blocage Altium, a ne pas refaire.
  - V64 : ObjectId 27 => ISch_Parameter, pas corps graphique.
  - V65 : ObjectId 37 => ISch_Pin, pas corps graphique.
  - V66 : ObjectId 8 => ISch_NetLabel, pas corps graphique.
  - V67 : ObjectId 18 et 5 acceptent plusieurs casts, donc le cast seul n est pas discriminant.
  - V68 : ObjectId 18 + ISch_Arc.Location/Radius compile et tourne, mais aucun hit exploitable
          dans le passage limite; on bascule donc vers ObjectId 5.
  - ObjectId 44 : ISch_Line cast-only valide, mais GetState_Vertex/Location/BoundingRectangle
    ont echoue ou ne compilent pas sur les routes testees.

  Risque v69:
  - Si RectObj.Corner ne compile pas dans cette API Altium, v69b retirera uniquement Corner
    et testera RectObj.Location seul.
  - Aucun autre accesseur, aucune matrice large, aucune lecture Variant.
}

function V69IsRectGeometryTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 5);
end;

function V69TryOid5RectLocationCorner(ChildObj : Variant; const SheetId : String; const CompDesignator : String; ChildIndex : Integer; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer) : String;
var
...
begin
    ...
    AppendExportLog('V69 oid5 rectangle geometry BEFORE sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    try
        RectObj := ChildObj;
        if RectObj = nil then
        begin
            Result := 'OID5_ISch_Rectangle_CAST_RETURNED_NIL_V69';
            AppendExportLog('V69 oid5 rectangle geometry AFTER result=' + Result);
            Exit;
        end;

        { Active v69 geometry read: one accessor family only. }
        Loc := RectObj.Location;
        Cor := RectObj.Corner;
        X1 := Loc.X;
        Y1 := Loc.Y;
        X2 := Cor.X;
        Y2 := Cor.Y;
        Result := 'OID5_ISch_Rectangle_LOCATION_CORNER_OK_V69';
    except
        Result := 'OID5_ISch_Rectangle_LOCATION_CORNER_RUNTIME_EXCEPTION_V69';
    end;

    AppendExportLog('V69 oid5 rectangle geometry AFTER sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result + ' x1=' + IntToStr(X1) + ' y1=' + IntToStr(Y1) + ' x2=' + IntToStr(X2) + ' y2=' + IntToStr(Y2));
end;

procedure V69AddOid5RectRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; const Status : String; X1 : Integer; Y1 : Integer; X2 : Integer; Y2 : Integer);
var
    Key : String;
begin
    Key := Status;
    ...
    Csv.Add(
        ...
        IntToStr(ChildIndex) + ';5;' +
        ProbeEscapeCell(V67DecodeChildObjectId(5)) + ';' +
        'ISch_Rectangle;' +
        ProbeEscapeCell(Status) + ';' +
        IntToStr(X1) + ';' +
        IntToStr(Y1) + ';' +
        IntToStr(X2) + ';' +
        IntToStr(Y2) + ';' +
        'v69SingleAccessorProbe'
    );
end;

procedure V69Oid5RectProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer; var HitBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
...
begin
    ...
    ComponentIndex := -1;
    Obj := Iterator.FirstSchObject;
    while (Obj <> nil) and (ComponentBudget > 0) and (HitBudget > 0) do
    begin
        Oid := ProbeTryObjectId(Obj);
        if Oid = 26 then
        begin
            ...
            begin
                try CompDesignator := Component.Designator.Text; except CompDesignator := 'READ_ERROR'; end;
                try CompComment := Component.Comment.Text; except CompComment := 'READ_ERROR'; end;
                try CompLibRef := Component.LibReference; except CompLibRef := 'READ_ERROR'; end;

                ChildIter := Component.SchIterator_Create;
                if ChildIter = nil then
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed_V69')
                else
                begin
                    try
                        ChildIndex := 0;
                        Guard := 0;
                        ChildObj := ChildIter.FirstSchObject;
                        while (ChildObj <> nil) and (Guard < 64) and (HitBudget > 0) do
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V69IsRectGeometryTarget(ChildOid) then
                            begin
                                Dec(HitBudget);
                                Status := V69TryOid5RectLocationCorner(ChildObj, SheetId, CompDesignator, ChildIndex, X1, Y1, X2, Y2);
                                V69AddOid5RectRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, Status, X1, Y1, X2, Y2);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 64 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated64_V69');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV69Oid5RectProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
    ...
    CsvPath         : String;
    SummaryPath     : String;
    ComponentBudget : Integer;
    HitBudget       : Integer;
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v69 oid5 rectangle probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v69 oid5 rectangle probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;flatObjectIndex;componentIndex;componentDesignatorText;componentCommentText;componentLibRef;childIndex;childObjectId;childDecodedRole;interfaceTest;status;x1;y1;x2;y2;probeVersion');
        Summary.Add('Altium schematic v69 isolated ObjectId 5 ISch_Rectangle Location/Corner geometry probe');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v69 isolates ObjectId 5 and tries one geometry accessor family only: ISch_Rectangle.Location + ISch_Rectangle.Corner.');
        Summary.Add('Limits=first 30 component roots across project; max 64 child objects per component; max 20 ObjectId5 hits.');
        Summary.Add('KnownGood=27->ISch_Parameter; 37->ISch_Pin; 8->ISch_NetLabel; 44->ISch_Line cast-only; v67 ObjectId5 recurrent but casts are not discriminant.');
        Summary.Add('KnownBad=V63 broad cast matrix caused hang; generic/Variant Location/BoundingRectangle/StartLocation/GetState_Vertex routes failed or did not compile; v68 ObjectId18 Arc Location/Radius produced no exploitable hit.');
        Summary.Add('ActiveTest=ObjectId5 only; ISch_Rectangle.Location and ISch_Rectangle.Corner only.');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V69Oid5RectProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget, HitBudget);
                    Inc(SheetCount);
                end;
            end;
            if (ComponentBudget <= 0) or (HitBudget <= 0) then Break;
        end;

        Summary.Add('SheetCountVisited=' + IntToStr(SheetCount));
        Summary.Add('RemainingComponentBudget=' + IntToStr(ComponentBudget));
        Summary.Add('RemainingHitBudget=' + IntToStr(HitBudget));
        Summary.Add('[V69_OID5_RECT_LOCATION_CORNER_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If LOCATION_CORNER_OK appears with non-zero coordinates, v70 can export ObjectId5 rectangles into symbolGraphics for a tiny subset first.');
        Summary.Add('If this does not compile, v69b should remove RectObj.Corner and try only RectObj.Location, not a matrix.');
        Summary.Add('If runtime exceptions only, ObjectId5 is probably not accessible through ISch_Rectangle geometry in this Altium API.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69_oid5_rect_location_corner_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69_oid5_rect_location_corner_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v69 oid5 rectangle probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v69 oid5 rectangle probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69_oid5_rect_location_corner_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69_oid5_rect_location_corner_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV69Oid5RectProbeReport;
    end
