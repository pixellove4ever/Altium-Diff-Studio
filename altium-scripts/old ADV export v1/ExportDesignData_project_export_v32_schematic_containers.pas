{==============================================================================}
{ Altium Diff Studio - Exporter Script v32                                         }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v32';
    SCHEMA_VERSION = 'ads-json-v32';
    SCHEMATIC_SCHEMA_VERSION = 'v32';
    PCB_OUTPUT_SUFFIX = '_pcb_v32.json';
    constKindPcb = 'PCB';
...
begin
    ...
    JsonList.Add('    "notes": "V32 tags each keepout edge and exports arc-aware viewer-ready render bounds. width <= 1.1mm is treated as boardOutlineCandidate; wider edges are kept as localKeepoutOrCutoutCandidate. Arc bounds now use the swept arc endpoints and cardinal extrema only when they lie inside the arc sweep."');
    JsonList.Add('  },');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV32",');
    JsonList.Add('      "graphicsNotes": "V32 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v32",');
    ...
    JsonList.Add('        "notes": "V32 keeps SCH extraction compile-safe in the target Altium version: components, pins, wires and net labels are exported; ports, power ports, junctions, buses, directives and native symbol graphics remain typed empty containers until their exact object enums/properties are validated."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v32: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V32 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV32",');
        JsonList.Add('  "compatibilityMode": "v32-safe-sch-containers",');
