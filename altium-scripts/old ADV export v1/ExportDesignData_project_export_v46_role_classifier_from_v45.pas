{==============================================================================}
{ Altium Diff Studio - Exporter Script v46                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v46';
    SCHEMA_VERSION = 'ads-json-v46';
    SCHEMATIC_SCHEMA_VERSION = 'v46';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV42",');
    JsonList.Add('      "graphicsNotes": "V42 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v42",');
    JsonList.Add('        "focus": "schematic hierarchy anchors and viewer-ready metadata",');
...
{==============================================================================}
{ 3f. TARGETED COMPONENT ROLE CLASSIFIER - v46 SAFE                            }
{==============================================================================}
{ Purpose:
  V45 confirmed that ObjectId 25, 45 and 46 are strongly component-correlated,
  and that ObjectId 44 / 4 are likely symbol graphics or component anchors.
  V46 still avoids risky generic properties (Text, Name, Location, Bounds) on
  unknown objects. Instead it creates a viewer/developer-ready classification
  file that assigns stable semantic hypotheses to each targeted object using
  only ObjectId and flat iterator order around component roots.

  This is the last safe step before trying typed extraction of those families.
  The output tells us exactly which ObjectId families should be targeted in v47
  for real text/geometry extraction. }

function V46DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
    if ObjectId = 25 then Result := 'componentPrimaryDisplayObject_candidateDesignatorOrPrimaryText'
    ...
    else if ObjectId = 44 then Result := 'componentGraphicOrModelObject_mediumConfidence'
    else if ObjectId = 4 then Result := 'componentPrimitiveOrAnchor_mediumConfidence'
    else if ObjectId = 40 then Result := 'sheetEntryLocationObject_validated'
    else if ObjectId = 41 then Result := 'sheetSymbolLocationObject_validated'
    else if ObjectId = 61 then Result := 'harnessOrSpecialConnector_candidate'
    else if ObjectId = 26 then Result := 'componentRoot'
    else if ObjectId = 37 then Result := 'componentPin'
    else if ObjectId = 27 then Result := 'componentOrGlobalParameter';
end;

function V46ConfidenceForObjectId(ObjectId : Integer) : String;
begin
    Result := 'low';
    if (ObjectId = 45) or (ObjectId = 46) then Result := 'high'
    else if (ObjectId = 25) then Result := 'high-count-match-but-role-needs-text-probe'
    else if (ObjectId = 44) or (ObjectId = 4) then Result := 'medium'
    else if (ObjectId = 40) or (ObjectId = 41) then Result := 'high-location-only'
    else if (ObjectId = 61) then Result := 'medium-special-object'
    else if (ObjectId = 26) or (ObjectId = 37) or (ObjectId = 27) then Result := 'known-native';
end;

function V46NextFieldForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 25 then Result := 'tryTextLikePropertiesAndParentComponentAssociation'
    else if ObjectId = 45 then Result := 'tryDesignatorTextProperties'
    else if ObjectId = 46 then Result := 'tryCommentTextProperties'
    else if ObjectId = 44 then Result := 'tryPrimitiveGeometryOrModelKind'
    else if ObjectId = 4 then Result := 'tryPrimitiveGeometryAnchorOrOwner'
    else if ObjectId = 40 then Result := 'trySheetEntryNameOwnerSideAndBounds'
    else if ObjectId = 41 then Result := 'trySheetSymbolNameFileNameBounds'
    else if ObjectId = 61 then Result := 'tryHarnessConnectorInterfaces'
    else if ObjectId = 26 then Result := 'alreadyExportedComponentRoot'
    else if ObjectId = 37 then Result := 'alreadyExportedPin'
    else if ObjectId = 27 then Result := 'alreadyExportedParameter';
end;

function V46IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
    if (ObjectId = 25) or (ObjectId = 45) or (ObjectId = 46) or
       (ObjectId = 44) or (ObjectId = 4) or (ObjectId = 40) or
       (ObjectId = 41) or (ObjectId = 61) or (ObjectId = 26) or
       (ObjectId = 37) or (ObjectId = 27) then
        Result := True;
end;

procedure V46RoleClassifierDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    SheetId         : String;
...
begin
    ...
    begin
        ...
        if V46IsTarget(Oid) then
        begin
            KnownName := ProbeKnownObjectIdName(Oid);
            Decision := V46DecisionForObjectId(Oid);
            Confidence := V46ConfidenceForObjectId(Oid);
            NextField := V46NextFieldForObjectId(Oid);
            ...
            Key := IntToStr(Oid) + ' ' + Decision + ' confidence=' + Confidence;
            ProbeIncCounter(Counters, Key);
            ...
            Csv.Add(
                ...
                IntToStr(Oid) + ';' +
                ProbeEscapeCell(KnownName) + ';' +
                ProbeEscapeCell(Decision) + ';' +
                ProbeEscapeCell(Confidence) + ';' +
                ProbeEscapeCell(NextField)
            );
        end;
...
procedure ExportSchV46RoleClassifierReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v46 role classifier skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v46 role classifier skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v46RoleDecision;confidence;nextFieldToProbe');
        Summary.Add('Altium schematic v46 component role classifier embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=classify component-correlated unknown ObjectId families before typed text/geometry extraction');
        Summary.Add('SafeProperties=ObjectId only, iterator order only, no risky generic Text/Name/Location/Bounds access');
        Summary.Add('CsvColumns=sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v46RoleDecision;confidence;nextFieldToProbe');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V46RoleClassifierDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V46_CLASSIFIED_OBJECT_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('v47 should start from this file and try very narrow typed probes on high-confidence families only.');
        Summary.Add('Primary target 45: designator display object, try text-like interfaces/properties.');
        Summary.Add('Primary target 46: comment display object, try text-like interfaces/properties.');
        Summary.Add('Secondary target 44/4/25: symbol graphics, primitive geometry, anchors or primary display text.');
        Summary.Add('Hierarchy target 40/41: sheet entry/sheet symbol name, filename, owner and bounds.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v46_role_classifier.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v46_role_classifier_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v46 role classifier CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v46 role classifier summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_object_probe_raw.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_object_probe_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_api_mapper_targeted.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_api_mapper_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_object_order.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_object_order_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_component_context.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_component_context_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v46_role_classifier.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v46_role_classifier_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchObjectProbeReport;
        ExportSchApiMapperReport;
        ExportSchObjectOrderReport;
        ExportSchComponentContextReport;
        ExportSchV46RoleClassifierReport;
    end
