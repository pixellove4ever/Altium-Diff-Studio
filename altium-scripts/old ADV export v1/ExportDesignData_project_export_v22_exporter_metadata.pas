{==============================================================================}
{ Altium Diff Studio - Exporter Script v21                                         }
{                                                                              }
...
begin
    ...
    { Compatibility-safe outline: many Altium versions expose BoardOutline through
      different APIs. To avoid breaking the OutJob script, derive a rectangular
      bounding outline from exported PCB objects. This gives the HTML viewer a
      stable frame while preserving all real primitives separately. }
    ...
    JsonList.Add('    { "x": ' + JsonFloat(MinX) + ', "y": ' + JsonFloat(MinY) + ' }');
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineSource": "boundingBoxFallback",');
end;
...
begin
    ...
    JsonList.Add('    "scriptVersion": "v22",');
    JsonList.Add('    "schemaVersion": "ads-json-v21"');
    JsonList.Add('  },');
end;
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphics",');
    JsonList.Add('      "graphicsNotes": "Symbol bodies are bounding-box fallbacks derived from component pins; native Altium symbol primitives are reserved in graphicPrimitives when API-compatible extraction is added.",');
    ...
    JsonList.Add('        "nativeSymbolPrimitives": "notExportedInCompatibilityBuild",');
    JsonList.Add('        "ports": "reserved",');
    JsonList.Add('        "powerPorts": "reserved",');
    JsonList.Add('        "notes": "This schema is ready for native SCH primitives; current build stays compatible with the Altium API version already validated."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v21: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('  "type": "schematic",');
        AddExporterMetadata(JsonList);
        JsonList.Add('  "schemaVersion": "v21",');
        JsonList.Add('  "exportGoal": "logicalPlusSchematicGraphicsFallback",');
