{==============================================================================}
{ Altium Diff Studio - Exporter Script v52b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v52b';
    SCHEMA_VERSION = 'ads-json-v52b';
    SCHEMATIC_SCHEMA_VERSION = 'v52b';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndObject44LocationProbeV52b",');
    JsonList.Add('      "graphicsNotes": "V52b keeps validated textRender and adds a one-property native geometry probe on ObjectId 44 using late-bound Obj.Location.X/Y. Symbol bodies still use pin-bounds fallback in the JSON until ObjectId 44 Location geometry is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v52b",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus ObjectId 44 Location probe",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v52b SAFE                    }
{==============================================================================}
{ Purpose:
...
function V52bDecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V52bExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V52bIsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V52bTryObjectLocationXY(Obj : Variant; var X : Integer; var Y : Integer; var Status : String) : Boolean;
begin
    { V52 failed to compile on this Altium because generic Obj.Location is not
      accepted by the script compiler, even inside try/except. Keep the probe
      shape but do not touch Location here. V53 will test a typed interface
      instead of generic late-bound Location. }
    Result := False;
    X := 0;
    Y := 0;
    Status := 'SKIPPED_GENERIC_LOCATION_UNDECLARED_IDENTIFIER';
end;

procedure V52bObject44LocationProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V52bIsTarget(Oid) then
        begin
            TargetRole := V52bDecisionForObjectId(Oid);
            ExpectedKind := V52bExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V52bTryObjectLocationXY(Obj, NativeX, NativeY, NativeStatus);
                NativeSource := 'Obj.Location.XY';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and (NativeStatus = 'Obj.Location.XY_OK') then
                    begin
                        DisplayX := NativeX;
                        DisplayY := NativeY;
                        GeometrySource := 'Obj.Location.XY';
                    end
                    else
...
procedure ExportSchV52bObject44LocationProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v52b ObjectId 44 Location probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v52b ObjectId 44 Location probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v52bRoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeLocationX;nativeLocationY;nativeLocationStatus;nativeLocationSource');
        Summary.Add('Altium schematic v52b ObjectId 44 Location probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=compile-safe ObjectId 44 candidate probe after V52 generic Obj.Location compile failure; no generic Location access is attempted');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 additionally probes late-bound Obj.Location.X/Y inside try/except; text geometry read only through typed ISch_Component.Designator/Comment Location accessors');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V52bObject44LocationProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V52b_OBJECT44_LOCATION_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 Location access is intentionally skipped in V52b because generic Obj.Location caused an undeclared identifier compile error in V52.');
        Summary.Add('Next step v53: test ObjectId 44 through a typed interface or alternate property, not generic Obj.Location.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52b_object44_location_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52b_object44_location_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v52b ObjectId 44 Location probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v52b ObjectId 44 Location probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52b_object44_location_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v52b_object44_location_probe_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV52bObject44LocationProbeReport;
    end
