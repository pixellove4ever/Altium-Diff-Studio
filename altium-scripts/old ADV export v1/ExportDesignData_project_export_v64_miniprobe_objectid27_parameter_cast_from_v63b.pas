{==============================================================================}
{ Altium Diff Studio - Exporter Script v64 MINI-PROBE OID27                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v64';
    SCHEMA_VERSION = 'ads-json-v64';
    SCHEMATIC_SCHEMA_VERSION = 'v64';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV64",');
    JsonList.Add('      "graphicsNotes": "V64 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v64",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v64 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v64  : reprend la route V62 sûre après le blocage constaté de la V63 ambitieuse.
          Aucun cast-matrix large n est exécuté; uniquement inventaire enfants ObjectId-only.

 Decision v64 SAFE after v63 hang:
...
function V64DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V64ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V64IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V64DecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V64AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V64DecodeChildObjectId(ObjectId);
...
function V64InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V64CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V64ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V64AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V64IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V64TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V64_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V64 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V64_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V64_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V64_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V64_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V64_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V64ChildObjectIdDecoderProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V64IsTarget(Oid) then
        begin
            TargetRole := V64DecisionForObjectId(Oid);
            ExpectedKind := V64ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V64InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V64CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ...
            if CurrentComp <> nil then
                V64ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V64TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v64:' + NativeStatus
                    else
...
procedure ExportSchV64ChildObjectIdDecoderProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v64 child ObjectId decoder probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v64 child ObjectId decoder probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v64RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;childObjectIdDecoderProbe_v64;childIteratorObjectCount_v64;childIteratorObjectIds_v64;childIteratorDecodedRoles_v64;childIteratorTargetOrCandidateHits_v64;routeResultPlan_v64;recommendedNextAction');
        Summary.Add('Altium schematic v64 SAFE child ObjectId decoder probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v64 SAFE rollback after v63 hang: keep the proven child ObjectId decoder route, add no broad cast matrix, and avoid all risky geometry accessors or COM-heavy loops.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V64ChildObjectIdDecoderProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V64_SAFE_CHILD_OBJECTID_DECODER_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 rows are included and childIteratorDecodedRoles_v64 decodes recurrent child ObjectIds 37, 27, 8, 18, 22 and 5 for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v64 actively inventories and decodes Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v64: analyse CSV safely; if needed, v64 should test one ObjectId and one interface at a time with progress logging.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_child_objectid_decoder_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_child_objectid_decoder_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v64 child ObjectId decoder probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v64 child ObjectId decoder probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
{==============================================================================}
{ 3g. V64 MINI-PROBE CIBLEE OBJECTID 27 - ISch_Parameter CAST-ONLY              }
{==============================================================================}
{ Objectif v64:
  - accelerer sans refaire la matrice large v63 qui a bloque Altium;
  - tester seulement ObjectId 27, car il revient souvent dans les enfants composant;
  ...
  Test actif v64:
  - ChildOid = 27 depuis Component.SchIterator_Create
  - cast-only vers ISch_Parameter
  - aucune lecture ParamObj.Name/Text/Location dans cette version.
}

procedure V64MiniProbe27DumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
                ...
                begin
                    try
                        ...
                        begin
                            ...
                            if ChildOid = 27 then
                            begin
                                AppendExportLog('V64 mini-probe BEFORE ObjectId27 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));
                                Status := 'OID27_FOUND_CAST_NOT_ATTEMPTED';
                                try
                                    ParamObj := ChildObj;
                                    if ParamObj <> nil then
                                        Status := 'OID27_ISch_Parameter_CAST_OK_NO_PROPERTY_READ_V64'
                                    else
                                        Status := 'OID27_ISch_Parameter_CAST_RETURNED_NIL_V64';
                                except
                                    Status := 'OID27_ISch_Parameter_CAST_EXCEPTION_V64';
                                end;
                                AppendExportLog('V64 mini-probe AFTER ObjectId27 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' status=' + Status);
                                ...
                                Csv.Add(
                                    ...
                                    IntToStr(ChildIndex) + ';' +
                                    IntToStr(ChildOid) + ';' +
                                    ProbeEscapeCell(V64DecodeChildObjectId(ChildOid)) + ';' +
                                    ProbeEscapeCell(Status) + ';' +
                                    'ISch_Parameter;castOnly;noGeometryNoTextNoLocation;v64MiniProbe'
                                );
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
...
procedure ExportSchV64MiniProbe27Report;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v64 mini-probe ObjectId27 skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v64 mini-probe ObjectId27 skipped: no focused project'); Exit; end;
    ...
    try
        ...
        Summary.Add('Altium schematic v64 mini-probe ObjectId 27 ISch_Parameter cast-only');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v64 targeted acceleration after v63 hang: test only child ObjectId 27 with ISch_Parameter cast-only, limited component budget, before/after debug logging.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference. ObjectId 27 is cast-only to ISch_Parameter; no geometry/text/location property is read.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V64MiniProbe27DumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V64_MINIPROBE_OBJECTID27_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If ObjectId 27 casts OK to ISch_Parameter, v65 can read one safe textual property only if already validated by older BOM parameter code.');
        Summary.Add('If ObjectId 27 is not useful, target ObjectId 37 or 8 with the same mini-probe pattern; do not re-enable broad cast matrices.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_miniprobe_oid27_parameter_cast.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_miniprobe_oid27_parameter_cast_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v64 mini-probe ObjectId27 CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v64 mini-probe ObjectId27 summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_child_objectid_decoder_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_child_objectid_decoder_probe_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_miniprobe_oid27_parameter_cast.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v64_miniprobe_oid27_parameter_cast_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV64ChildObjectIdDecoderProbeReport;
        ExportSchV64MiniProbe27Report;
    end
