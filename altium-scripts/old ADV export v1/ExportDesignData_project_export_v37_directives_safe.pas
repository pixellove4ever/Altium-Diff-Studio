{==============================================================================}
{ Altium Diff Studio - Exporter Script v37                                          }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v37';
    SCHEMA_VERSION = 'ads-json-v37';
    SCHEMATIC_SCHEMA_VERSION = 'v37';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV37",');
    JsonList.Add('      "graphicsNotes": "V37 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v37",');
    ...
    JsonList.Add('        "notes": "V37 keeps the native eJunction pass, but adds an explicit fallback contract because this Altium version returned zero native junction objects on the AGMA project."');
    JsonList.Add('      },');
    ...
    JsonList.Add('        "notes": "V37 keeps SCH extraction compile-safe in the target Altium version: components, pins, wires and net labels are exported; native symbol graphics remain a typed empty container; schematic directives are exported via eParameterSet; buses and bus entries are exported when the target Altium API exposes eBus/eBusEntry; ports, power ports and NoERC markers are exported when the target Altium API exposes ePort/ePowerObject/eNoERC. Native eJunction is still attempted, and V37 adds an explicit viewer-side topology fallback for projects where Altium returns zero native junction objects."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v37: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount) + ' ports=' + IntToStr(PortCount) + ' powerPorts=' + IntToStr(PowerCount) + ' junctions=' + IntToStr(JunctionCount) + ' noERC=' + IntToStr(NoERCCount) + ' buses=' + IntToStr(BusCount) + ' busEntries=' + IntToStr(BusEntryCount) + ' directives=' + IntToStr(DirectiveCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V37 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV37",');
        JsonList.Add('  "compatibilityMode": "v37-safe-sch-containers",');
