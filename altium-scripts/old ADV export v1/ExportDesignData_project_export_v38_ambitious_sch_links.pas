{==============================================================================}
{ Altium Diff Studio - Exporter Script v38                                          }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v38';
    SCHEMA_VERSION = 'ads-json-v38';
    SCHEMATIC_SCHEMA_VERSION = 'v38';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV38",');
    JsonList.Add('      "graphicsNotes": "V38 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v38",');
    JsonList.Add('        "focus": "schematic typed containers and viewer-ready metadata",');
    ...
    OffSheetCount := 0;
    Iterator := SchDoc.SchIterator_Create;
    Iterator.AddFilter_ObjectSet(MkSet(eOffSheetConnector));
    OffSheetObj := Iterator.FirstSchObject;
    while OffSheetObj <> nil do
    begin
        if OffSheetCount > 0 then JsonList.Add('        ,');
        JsonList.Add('        {');
        JsonList.Add('          "id": "SHEET_' + IntToStr(SheetIndex) + '_OFFSHEET_' + IntToStr(OffSheetCount) + '",');
        JsonList.Add('          "source": "eOffSheetConnector",');
        JsonList.Add('          "x": ' + IntToStr(OffSheetObj.Location.X) + ',');
        JsonList.Add('          "y": ' + IntToStr(OffSheetObj.Location.Y));
        JsonList.Add('        }');
        Inc(OffSheetCount);
        OffSheetObj := Iterator.NextSchObject;
    end;
    SchDoc.SchIterator_Destroy(Iterator);
    JsonList.Add('      ],');
    ...
    JsonList.Add('        "notes": "V38 keeps the native eJunction pass, but adds an explicit fallback contract because this Altium version returned zero native junction objects on the AGMA project."');
    JsonList.Add('      },');
    ...
    JsonList.Add('        "offSheetConnectors": "exportedViaEOffSheetConnector",');
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V38 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV38",');
        JsonList.Add('  "compatibilityMode": "v38-safe-sch-containers",');
