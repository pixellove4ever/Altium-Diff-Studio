{==============================================================================}
{ Altium Diff Studio - Exporter Script v54                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v54';
    SCHEMA_VERSION = 'ads-json-v54';
    SCHEMATIC_SCHEMA_VERSION = 'v54';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndObject44LineXYProbeV54",');
    JsonList.Add('      "graphicsNotes": "V54 keeps validated textRender and adds a compile-safe ObjectId 44 line endpoint probe using X1/Y1/X2/Y2 status mapping. Symbol bodies still use pin-bounds fallback in the JSON until native symbol primitives are validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v54",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus ObjectId 44 line X1/Y1/X2/Y2 probe",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v54 OBJECT44 LINE X/Y                    }
{==============================================================================}
{ Purpose:
...
function V54DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V54ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V54IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V54TryObject44LineXY(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X1 := 0;
    Y1 := 0;
    X2 := 0;
    Y2 := 0;
    Status := 'X1Y1X2Y2_NOT_ATTEMPTED_COMPILE_SAFE';
    { Direct generic accesses such as Obj.X1 / Obj.Y1 / Obj.X2 / Obj.Y2
      ...
      catch anything. V54 therefore records a clean compile-safe baseline for
      line-like geometry candidates and keeps the ObjectId 44/4/25 cluster map. }
end;

procedure V54Object44LineXYProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V54IsTarget(Oid) then
        begin
            TargetRole := V54DecisionForObjectId(Oid);
            ExpectedKind := V54ExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V54TryObject44LineXY(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'compileSafeNoDirectObjX1Y1X2Y2';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
...
procedure ExportSchV54Object44LineXYProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v54 ObjectId 44 line X1/Y1/X2/Y2 probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v54 ObjectId 44 line X1/Y1/X2/Y2 probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v54RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeLineX1;nativeLineY1;nativeLineX2;nativeLineY2;nativeLineStatus;nativeLineSource');
        Summary.Add('Altium schematic v54 ObjectId 44 line X1/Y1/X2/Y2 probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=cleaned v54 compile-safe ObjectId 44 line endpoint probe after generic Location/BoundingRectangle failures; no direct Obj.X1/Y1/X2/Y2 access is attempted');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 line endpoints are marked skipped because generic direct property access is not supported by compiler');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V54Object44LineXYProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V54_OBJECT44_LINE_XY_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 generic line endpoint access is not attempted in V54 to keep the script compile-safe.');
        Summary.Add('Next step v55: try a typed primitive/interface cast for ObjectId 44 or use child-iterator access from ISch_Component, one typed method at a time.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v54_object44_linexy_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v54_object44_linexy_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v54 ObjectId 44 line X1/Y1/X2/Y2 probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v54 ObjectId 44 line X1/Y1/X2/Y2 probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v54_object44_linexy_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v54_object44_linexy_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV54Object44LineXYProbeReport;
    end
