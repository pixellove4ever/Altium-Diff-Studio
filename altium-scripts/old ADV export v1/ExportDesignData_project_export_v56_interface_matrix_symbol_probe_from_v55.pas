{==============================================================================}
{ Altium Diff Studio - Exporter Script v56                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v56';
    SCHEMA_VERSION = 'ads-json-v56';
    SCHEMATIC_SCHEMA_VERSION = 'v56';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndInterfaceMatrixSymbolProbeV56",');
    JsonList.Add('      "graphicsNotes": "V56 keeps validated textRender and accelerates native symbol discovery with a compile-safe interface matrix for ObjectId 44/4/25: component context, LibRef grouping, fallback scoring, and typed-interface candidate batches. Symbol bodies still use pin-bounds fallback in the JSON until one typed primitive interface is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v56",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe typed-interface matrix for native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v56 INTERFACE MATRIX COMPILE-SAFE             }
{==============================================================================}
{ Purpose:
...
function V56DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V56ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V56IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V56InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V56CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V56TryInterfaceMatrixSymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X1 := 0;
    Y1 := 0;
    X2 := 0;
    Y2 := 0;
    Status := 'NATIVE_DIRECT_GEOMETRY_NOT_ATTEMPTED_COMPILE_SAFE';
    { Direct generic accesses such as Obj.X1 / Obj.Y1 / Obj.X2 / Obj.Y2
      are intentionally NOT attempted here. Previous probes showed that
      generic Variant-style property names like Location and BoundingRectangle
      are rejected by the Altium PascalScript compiler before try/except can
      catch anything. V56 therefore records a clean compile-safe baseline for
      line-like geometry candidates and keeps the ObjectId 44/4/25 cluster map. }
end;

procedure V56InterfaceMatrixSymbolProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V56IsTarget(Oid) then
        begin
            TargetRole := V56DecisionForObjectId(Oid);
            ExpectedKind := V56ExpectedTextKindForObjectId(Oid);
            ...
            ProbeRouteB := 'nativeTypedAccess_DISABLED_compileSafe';
            ProbeRouteC := 'libRefGrouping_ACTIVE';
            ProbeRouteD := 'pinBoundsFallback_ACTIVE';
            RecommendedNext := 'keepAsAnchor';
            if Oid = 44 then
            begin
                RecommendedNext := 'tryTypedInterfaceOrChildIteratorInNextTargetedBuild';
                ProbeRouteB := 'candidateProperties_disabled:Location,BoundingRectangle,X1Y1X2Y2,StartEnd';
            end;
            if Oid = 4 then
            ...
            InterfaceMatrix := V56InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V56CompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V56TryInterfaceMatrixSymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'compileSafeNoDirectNativeGeometry';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and (NativeStatus = 'Obj.X1Y1X2Y2_OK') then
                    begin
                        DisplayX := NativeX1;
                        DisplayY := NativeY1;
                        GeometrySource := 'Obj.X1Y1X2Y2';
                    end
                    else
...
procedure ExportSchV56InterfaceMatrixSymbolProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v56 interface-matrix symbol primitive probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v56 interface-matrix symbol primitive probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v56RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;interfaceMatrix_v56;compileRiskPlan_v56;recommendedNextAction');
        Summary.Add('Altium schematic v56 interface-matrix symbol primitive probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=cleaned v56 accelerated compile-safe interface matrix after generic Location/BoundingRectangle/X1/Y1/X2/Y2 failures; multiple typed-interface and child-iterator routes are planned and scored in one run without direct unsupported properties.');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 generic properties remain disabled, but v56 records typed-interface candidate batches to enable faster targeted compile-risk testing.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V56InterfaceMatrixSymbolProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V56_INTERFACE_MATRIX_SYMBOL_PRIMITIVE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 direct generic geometry remains disabled; v56 accelerates by recording typed-interface batches: ISch_Line/Rectangle/Arc first, Ellipse/Polygon/Bezier second, then component child/library routes.');
        Summary.Add('Next step v57: enable one complete typed-interface batch from the v56 matrix, preferably ISch_Line/ISch_Rectangle/ISch_Arc, and keep the other batches disabled.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v56_interface_matrix_symbol_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v56_interface_matrix_symbol_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v56 interface-matrix symbol primitive probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v56 interface-matrix symbol primitive probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v56_interface_matrix_symbol_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v56_interface_matrix_symbol_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV56InterfaceMatrixSymbolProbeReport;
    end
