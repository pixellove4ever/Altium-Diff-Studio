{==============================================================================}
{ Altium Diff Studio - Exporter Script v36                                          }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v36';
    SCHEMA_VERSION = 'ads-json-v36';
    SCHEMATIC_SCHEMA_VERSION = 'v36';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV36",');
    JsonList.Add('      "graphicsNotes": "V36 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v36",');
    ...
    JsonList.Add('        "notes": "V36 keeps the native eJunction pass, but adds an explicit fallback contract because this Altium version returned zero native junction objects on the AGMA project."');
    JsonList.Add('      },');
    ...
    JsonList.Add('        "notes": "V36 keeps SCH extraction compile-safe in the target Altium version: components, pins, wires and net labels are exported; directives and native symbol graphics remain typed empty containers; buses and bus entries are exported when the target Altium API exposes eBus/eBusEntry; ports, power ports and NoERC markers are exported when the target Altium API exposes ePort/ePowerObject/eNoERC. Native eJunction is still attempted, and V36 adds an explicit viewer-side topology fallback for projects where Altium returns zero native junction objects."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v36: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount) + ' ports=' + IntToStr(PortCount) + ' powerPorts=' + IntToStr(PowerCount) + ' junctions=' + IntToStr(JunctionCount) + ' noERC=' + IntToStr(NoERCCount) + ' buses=' + IntToStr(BusCount) + ' busEntries=' + IntToStr(BusEntryCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V36 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV36",');
        JsonList.Add('  "compatibilityMode": "v36-safe-sch-containers",');
