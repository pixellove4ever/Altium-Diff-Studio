{==============================================================================}
{ Altium Diff Studio - Exporter Script v34                                          }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v34';
    SCHEMA_VERSION = 'ads-json-v34';
    SCHEMATIC_SCHEMA_VERSION = 'v34';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV34",');
    JsonList.Add('      "graphicsNotes": "V34 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v34",');
    ...
    JsonList.Add('        "junctions": "exportedViaEJunction",
        "noERC": "exportedViaENoERC",');
    JsonList.Add('        "buses": "reserved",');
    JsonList.Add('        "notes": "V34 keeps SCH extraction compile-safe in the target Altium version: components, pins, wires and net labels are exported; buses, directives and native symbol graphics remain typed empty containers; ports, power ports, junctions and NoERC markers are exported when the target Altium API exposes ePort/ePowerObject/eJunction/eNoERC until their exact object properties are fully validated."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v34: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount) + ' ports=' + IntToStr(PortCount) + ' powerPorts=' + IntToStr(PowerCount) + ' junctions=' + IntToStr(JunctionCount) + ' noERC=' + IntToStr(NoERCCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V34 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV34",');
        JsonList.Add('  "compatibilityMode": "v34-safe-sch-containers",');
