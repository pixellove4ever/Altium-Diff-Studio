{==============================================================================}
{ Altium Diff Studio - Exporter Script v69b OID5 RECTANGLE LOCATION/CORNER PROBE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v69b';
    SCHEMA_VERSION = 'ads-json-v69b';
    SCHEMATIC_SCHEMA_VERSION = 'v69b';
...
begin
    ...
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineEdgesSource": "keepOutLayerTracksOnly_eRectObjectDisabled_V69b",');
    JsonList.Add('  "boardOutlineEdgesCount": ' + IntToStr(OutlineEdgeCount) + ',');
    JsonList.Add('  "boardOutlineClassification": {');
    JsonList.Add('    "method": "widthHeuristic_tracksOnly_V69b",');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV69b",');
    JsonList.Add('      "graphicsNotes": "V69b SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v69b",');
...
{==============================================================================}
{ 3g. V69b MINI-PROBE ISOLEE OBJECTID 5 - RECTANGLE LOCATION/CORNER READ       }
{==============================================================================}
{ Objectif v69b:
  ...
  Risque v69b:
  - Si RectObj.Corner ne compile pas dans cette API Altium, v69bb retirera uniquement Corner
    et testera RectObj.Location seul.
  - Aucun autre accesseur, aucune matrice large, aucune lecture Variant.
}

function V69bIsRectGeometryTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 5);
end;

function V69bTryOid5RectLocationCorner(ChildObj : Variant; const SheetId : String; const CompDesignator : String; ChildIndex : Integer; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer) : String;
var
...
begin
    ...
    AppendExportLog('V69b oid5 rectangle geometry BEFORE sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    try
        RectObj := ChildObj;
        if RectObj = nil then
        begin
            Result := 'OID5_ISch_Rectangle_CAST_RETURNED_NIL_V69b';
            AppendExportLog('V69b oid5 rectangle geometry AFTER result=' + Result);
            Exit;
        end;

        { Active v69b geometry read: one accessor family only. }
        ...
        X2 := Cor.X;
        Y2 := Cor.Y;
        Result := 'OID5_ISch_Rectangle_LOCATION_CORNER_OK_V69b';
    except
        Result := 'OID5_ISch_Rectangle_LOCATION_CORNER_RUNTIME_EXCEPTION_V69b';
    end;

    AppendExportLog('V69b oid5 rectangle geometry AFTER sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result + ' x1=' + IntToStr(X1) + ' y1=' + IntToStr(Y1) + ' x2=' + IntToStr(X2) + ' y2=' + IntToStr(Y2));
end;

procedure V69bAddOid5RectRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; const Status : String; X1 : Integer; Y1 : Integer; X2 : Integer; Y2 : Integer);
var
    Key : String;
begin
    Key := Status;
    ...
    Csv.Add(
        ...
        IntToStr(X2) + ';' +
        IntToStr(Y2) + ';' +
        'v69bSingleAccessorProbe'
    );
end;

procedure V69bOid5RectProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer; var HitBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
        ...
        begin
            ...
            begin
                ...
                if ChildIter = nil then
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed_V69b')
                else
                begin
                    try
                        ChildIndex := 0;
                        ...
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V69bIsRectGeometryTarget(ChildOid) then
                            begin
                                Dec(HitBudget);
                                Status := V69bTryOid5RectLocationCorner(ChildObj, SheetId, CompDesignator, ChildIndex, X1, Y1, X2, Y2);
                                V69bAddOid5RectRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, Status, X1, Y1, X2, Y2);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 64 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated64_V69b');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV69bOid5RectProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v69b oid5 rectangle probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v69b oid5 rectangle probe skipped: no focused project'); Exit; end;
    ...
    try
        ...
        Summary.Add('Altium schematic v69b isolated ObjectId 5 ISch_Rectangle Location/Corner geometry probe');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v69b isolates ObjectId 5 and tries one geometry accessor family only: ISch_Rectangle.Location + ISch_Rectangle.Corner.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V69bOid5RectProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget, HitBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V69b_OID5_RECT_LOCATION_CORNER_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('If this does not compile, v69bb should remove RectObj.Corner and try only RectObj.Location, not a matrix.');
        ...
        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69b_oid5_rect_location_corner_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69b_oid5_rect_location_corner_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v69b oid5 rectangle probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v69b oid5 rectangle probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69b_oid5_rect_location_corner_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v69b_oid5_rect_location_corner_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV69bOid5RectProbeReport;
    end
