{==============================================================================}
{ Altium Diff Studio - Exporter Script v26                                         }
{                                                                              }
...
function TryReadBoardOutlinePointNative(OutlineObj : Variant; Index : Integer; var Xmm : Double; var Ymm : Double) : Boolean;
var
    P : Variant;
begin
    Result := False;

    { Different Altium builds expose board-outline vertices through different
      dispatch names.  Keep every access late-bound through Variant and protect
      each candidate with try/except so an unsupported property does not abort
      the export. }

    try
        P := OutlineObj.GetState_Vertex(Index);
        Xmm := CoordToMMs(P.X);
        Ymm := CoordToMMs(P.Y);
        Result := True;
        Exit;
...
function TryAddNativeBoardOutlineVertices(PcbBoard : IPCB_Board; JsonList : TStringList) : Boolean;
var
    OutlineObj : Variant;
    VertexCount : Integer;
    I : Integer;
...
begin
    ...
    if PcbBoard.BoardOutline = nil then Exit;

    OutlineObj := PcbBoard.BoardOutline;
    VertexCount := 0;

    { Try the most common count accessors first.
      IMPORTANT: in Altium Script, unknown Variant properties like
      OutlineObj.VerticesCount may still be checked at compile time and cause
      "Undeclared identifier".  Therefore v26b only calls accessor-like methods
      with parentheses. Unsupported methods are caught at runtime. }
    try
        VertexCount := OutlineObj.GetState_VerticesCount();
        SourceName := 'nativeBoardOutlineGetStateVerticesCount';
    except
        VertexCount := 0;
    end;
    ...
    begin
        try
            VertexCount := OutlineObj.GetState_VertexCount();
            SourceName := 'nativeBoardOutlineGetStateVertexCount';
        except
            VertexCount := 0;
        end;
    end;
    ...
    begin
        try
            VertexCount := OutlineObj.GetState_PointCount();
            SourceName := 'nativeBoardOutlineGetStatePointCount';
        except
            VertexCount := 0;
        end;
    end;
    ...
    begin
        try
            VertexCount := OutlineObj.GetVertexCount();
            SourceName := 'nativeBoardOutlineGetVertexCount';
        except
            VertexCount := 0;
        end;
    end;
    ...
    begin
        try
            VertexCount := OutlineObj.GetPointCount();
            SourceName := 'nativeBoardOutlineGetPointCount';
        except
            VertexCount := 0;
        end;
    end;
    ...
    try
        ...
        { Most Altium collection-style APIs are 1-based.  If an index does not
          work, TryReadBoardOutlinePointNative simply returns False and we keep
          going. }
        for I := 1 to VertexCount do
        begin
            if TryReadBoardOutlinePointNative(OutlineObj, I, Xmm, Ymm) then
            begin
                ...
                if Pts.Count > 0 then
                    Pts.Add('    ,');
                Pts.Add('    { "x": ' + JsonFloat(Xmm) + ', "y": ' + JsonFloat(Ymm) + ' }');
            end;
        end;

        { Some builds expose vertices as 0-based despite reporting a count.  If
          the 1-based pass found nothing, retry 0..count-1. }
        if Pts.Count = 0 then
        begin
        ...
        if Pts.Count = 0 then Exit;

        { Close the polyline if Altium did not already return a closed contour. }
        if GotFirst then
        begin
            if TryReadBoardOutlinePointNative(OutlineObj, VertexCount, Xmm, Ymm) then
            begin
                if (Abs(Xmm - FirstX) > 0.000001) or (Abs(Ymm - FirstY) > 0.000001) then
                begin
                    Pts.Add('    ,');
                    Pts.Add('    { "x": ' + JsonFloat(FirstX) + ', "y": ' + JsonFloat(FirstY) + ' }');
                end;
            end;
        end;
        ...
        for I := 0 to Pts.Count - 1 do
            JsonList.Add(Pts[I]);
        JsonList.Add('  ],');
        JsonList.Add('  "boardOutlineSource": "' + SourceName + '",');
        JsonList.Add('  "boardOutlineNativeVertexCount": ' + IntToStr(VertexCount) + ',');
        Result := True;
    finally
        Pts.Free;
    end;
...
begin
    { V26b outline strategy:
      1) Try to export native BoardOutline vertices/points. This is the first
         attempt at a real board contour instead of a rectangle.
      2) If the target Altium build exposes only BoundingRectangle, keep the v25
         native rectangle path.
      3) Keep the all-object bounding fallback only if BoardOutline access fails. }

    if TryAddNativeBoardOutlineVertices(PcbBoard, JsonList) then
        Exit;
...
begin
    ...
    JsonList.Add('    "scriptVersion": "v26b",');
    JsonList.Add('    "schemaVersion": "ads-json-v26b"');
    JsonList.Add('  },');
end;
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV26bb",');
    ...
    JsonList.Add('        "notes": "V26b keeps the universal schema stable. Native SCH primitive extraction should be enabled object family by object family after API validation in the target Altium version."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v26: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "boardOutline": "nativeBoardOutlineVerticesThenBoundingRectangleThenAllObjectFallback",');
        JsonList.Add('    "polygons": "reserved",');
        JsonList.Add('    "splitPlanes": "reserved",');
        JsonList.Add('    "regions": "reserved",');
        JsonList.Add('    "notes": "V26b first tries native BoardOutline vertices/points through late-bound API variants. If unavailable, it falls back to the validated native Board.BoardOutline.BoundingRectangle path from v25, then to all-object bounds."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "type": "schematic",');
        AddExporterMetadata(JsonList);
        JsonList.Add('  "schemaVersion": "v26b",');
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV26bb",');
