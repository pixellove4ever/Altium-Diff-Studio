{==============================================================================}
{ Altium Diff Studio - Exporter Script v68 OID18 ARC LOCATION/RADIUS PROBE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v68';
    SCHEMA_VERSION = 'ads-json-v68';
    SCHEMATIC_SCHEMA_VERSION = 'v68';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV68",');
    JsonList.Add('      "graphicsNotes": "V68 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v68",');
...
{==============================================================================}
{ 3g. V68 MINI-PROBE ISOLEE OBJECTID 18 - ARC LOCATION/RADIUS READ          }
{==============================================================================}
{ Objectif v68:
  - partir de la V67 validee;
  - arreter les casts multiples non discriminants;
  - isoler seulement ObjectId 18, qui apparait de façon recurrente dans V67;
  - tenter UNE seule lecture geometrique typée: ISch_Arc.Location + ISch_Arc.Radius;
  - budget tres petit et log avant/apres pour eviter un nouveau blocage Altium.
  ...
  Risque v68:
  - Si ArcObj.Location ou ArcObj.Radius ne compile pas dans cette API Altium, ce sera une preuve
    utile et on fera une v68b sans ces accesseurs.
  - Aucun autre accesseur, aucune matrice large, aucune lecture Variant.
}

function V68IsArcGeometryTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 18);
end;

function V68TryOid18ArcLocationRadius(ChildObj : Variant; const SheetId : String; const CompDesignator : String; ChildIndex : Integer; var X : Integer; var Y : Integer; var R : Integer) : String;
var
    ArcObj    : ISch_Arc;
    ALocation : TLocation;
begin
    ...
    AppendExportLog('V68 oid18 arc geometry BEFORE sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    try
        ArcObj := ChildObj;
        if ArcObj = nil then
        begin
            Result := 'OID18_ISch_Arc_CAST_RETURNED_NIL_V68';
            AppendExportLog('V68 oid18 arc geometry AFTER result=' + Result);
            Exit;
        end;

        { Active v68 geometry read: one accessor family only. }
        ALocation := ArcObj.Location;
        X := ALocation.X;
        Y := ALocation.Y;
        R := ArcObj.Radius;
        Result := 'OID18_ISch_Arc_LOCATION_RADIUS_OK_V68';
    except
        Result := 'OID18_ISch_Arc_LOCATION_RADIUS_RUNTIME_EXCEPTION_V68';
    end;

    AppendExportLog('V68 oid18 arc geometry AFTER sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result + ' x=' + IntToStr(X) + ' y=' + IntToStr(Y) + ' r=' + IntToStr(R));
end;

procedure V68AddOid18ArcRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; const Status : String; X : Integer; Y : Integer; R : Integer);
var
    Key : String;
begin
    Key := Status;
    ...
    Csv.Add(
        ...
        IntToStr(ChildIndex) + ';18;' +
        ProbeEscapeCell(V67DecodeChildObjectId(18)) + ';' +
        'ISch_Arc;' +
        ProbeEscapeCell(Status) + ';' +
        IntToStr(X) + ';' +
        IntToStr(Y) + ';' +
        IntToStr(R) + ';' +
        'v68SingleAccessorProbe'
    );
end;

procedure V68Oid18ArcProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer; var HitBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
        ...
        begin
            ...
            begin
                ...
                if ChildIter = nil then
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed_V68')
                else
                begin
                    try
                        ChildIndex := 0;
                        ...
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V68IsArcGeometryTarget(ChildOid) then
                            begin
                                Dec(HitBudget);
                                Status := V68TryOid18ArcLocationRadius(ChildObj, SheetId, CompDesignator, ChildIndex, X, Y, R);
                                V68AddOid18ArcRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, Status, X, Y, R);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 64 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated64_V68');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV68Oid18ArcProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v68 oid18 arc probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v68 oid18 arc probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;flatObjectIndex;componentIndex;componentDesignatorText;componentCommentText;componentLibRef;childIndex;childObjectId;childDecodedRole;interfaceTest;status;x;y;radius;probeVersion');
        Summary.Add('Altium schematic v68 isolated ObjectId 18 ISch_Arc Location/Radius geometry probe');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v68 isolates ObjectId 18 and tries one geometry accessor family only: ISch_Arc.Location + ISch_Arc.Radius.');
        Summary.Add('Limits=first 20 component roots across project; max 64 child objects per component; max 20 ObjectId18 hits.');
        Summary.Add('KnownGood=27->ISch_Parameter; 37->ISch_Pin; 8->ISch_NetLabel; 44->ISch_Line cast-only; v67 ObjectId18 recurrent but casts are not discriminant.');
        ...
        Summary.Add('ActiveTest=ObjectId18 only; ISch_Arc.Location and ISch_Arc.Radius only.');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V68Oid18ArcProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget, HitBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V68_OID18_ARC_LOCATION_RADIUS_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If LOCATION_RADIUS_OK appears with non-zero coordinates/radius, v69 can export ObjectId18 arcs into symbolGraphics for a tiny subset first.');
        Summary.Add('If this does not compile, v68b should remove ArcObj.Location/Radius and try another single typed accessor, not a matrix.');
        Summary.Add('If runtime exceptions only, ObjectId18 is probably not accessible through ISch_Arc geometry in this Altium API.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v68_oid18_arc_location_radius_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v68_oid18_arc_location_radius_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v68 oid18 arc probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v68 oid18 arc probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v68_oid18_arc_location_radius_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v68_oid18_arc_location_radius_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV68Oid18ArcProbeReport;
    end
