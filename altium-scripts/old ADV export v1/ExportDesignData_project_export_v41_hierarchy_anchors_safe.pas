...
const
    SCRIPT_VERSION = 'v41';
    SCHEMA_VERSION = 'ads-json-v41';
    SCHEMATIC_SCHEMA_VERSION = 'v41';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV41",');
    JsonList.Add('      "graphicsNotes": "V41 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v41",');
    ...
    JsonList.Add('        "notes": "V41 keeps the native eJunction pass, but adds an explicit fallback contract because this Altium version returned zero native junction objects on the AGMA project."');
    JsonList.Add('      },');
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V41 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusHierarchyAnchorsV41",');
        JsonList.Add('  "compatibilityMode": "v41-safe-sch-hierarchy-anchors",');
        ...
        JsonList.Add('    "edges": [],');
        JsonList.Add('    "notes": "V41 adds a global hierarchy contract without reading new risky Altium properties. Existing per-sheet arrays plus sheets[].hierarchyAnchors contain the raw data needed by the viewer to infer inter-sheet connectivity. Native sheet symbol and sheet entry names remain unavailable in this target API and are therefore not used in the script."');
        JsonList.Add('  }');
        JsonList.Add('}');
