{==============================================================================}
{ Altium Diff Studio - Exporter Script v63b SAFE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v63b';
    SCHEMA_VERSION = 'ads-json-v63b';
    SCHEMATIC_SCHEMA_VERSION = 'v63b';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV63b",');
    JsonList.Add('      "graphicsNotes": "V63b SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v63b",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v63b ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v63b  : reprend la route V62 sûre après le blocage constaté de la V63 ambitieuse.
          Aucun cast-matrix large n est exécuté; uniquement inventaire enfants ObjectId-only.

 Decision v63b SAFE after v63 hang:
...
function V63bDecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V63bExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V63bIsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V63bDecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V63bAppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V63bDecodeChildObjectId(ObjectId);
...
function V63bInterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V63bCompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V63bChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V63bAppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V63bIsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V63bTryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V63b_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V63b AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V63b_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V63b_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V63b_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V63b_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V63b_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V63bChildObjectIdDecoderProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V63bIsTarget(Oid) then
        begin
            TargetRole := V63bDecisionForObjectId(Oid);
            ExpectedKind := V63bExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V63bInterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V63bCompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ...
            if CurrentComp <> nil then
                V63bChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V63bTryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v63b:' + NativeStatus
                    else
...
procedure ExportSchV63bChildObjectIdDecoderProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v63b child ObjectId decoder probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v63b child ObjectId decoder probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v63bRoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;childObjectIdDecoderProbe_v63b;childIteratorObjectCount_v63b;childIteratorObjectIds_v63b;childIteratorDecodedRoles_v63b;childIteratorTargetOrCandidateHits_v63b;routeResultPlan_v63b;recommendedNextAction');
        Summary.Add('Altium schematic v63b SAFE child ObjectId decoder probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v63b SAFE rollback after v63 hang: keep the proven child ObjectId decoder route, add no broad cast matrix, and avoid all risky geometry accessors or COM-heavy loops.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V63bChildObjectIdDecoderProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V63b_SAFE_CHILD_OBJECTID_DECODER_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 rows are included and childIteratorDecodedRoles_v63b decodes recurrent child ObjectIds 37, 27, 8, 18, 22 and 5 for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v63b actively inventories and decodes Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v63b: analyse CSV safely; if needed, v64 should test one ObjectId and one interface at a time with progress logging.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63b_child_objectid_decoder_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63b_child_objectid_decoder_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v63b child ObjectId decoder probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v63b child ObjectId decoder probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63b_child_objectid_decoder_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v63b_child_objectid_decoder_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV63bChildObjectIdDecoderProbeReport;
    end
