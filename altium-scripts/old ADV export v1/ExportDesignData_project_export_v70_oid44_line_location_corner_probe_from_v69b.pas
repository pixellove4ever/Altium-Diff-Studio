...
{==============================================================================}
{ Altium Diff Studio - Exporter Script v70 OID44 LINE LOCATION/CORNER PROBE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v70';
    SCHEMA_VERSION = 'ads-json-v70';
    SCHEMATIC_SCHEMA_VERSION = 'v70';
...
begin
    ...
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineEdgesSource": "keepOutLayerTracksOnly_eRectObjectDisabled_V70",');
    JsonList.Add('  "boardOutlineEdgesCount": ' + IntToStr(OutlineEdgeCount) + ',');
    JsonList.Add('  "boardOutlineClassification": {');
    JsonList.Add('    "method": "widthHeuristic_tracksOnly_V70",');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV70",');
    JsonList.Add('      "graphicsNotes": "V70 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v70",');
...
{==============================================================================}
{ 3g. V70 MINI-PROBE ISOLEE OBJECTID 44 - LINE LOCATION/CORNER READ           }
{==============================================================================}
{ Objectif v70:
  ...
  - tenter UNE seule famille d'accesseurs: ISch_Line.Location + ISch_Line.Corner;
  ...
  - Si ISch_Line.Location ou ISch_Line.Corner ne compile pas dans cette API Altium,
    on corrigera en v70b en retirant l'accesseur fautif. Ne pas ajouter de matrice.
...
function V70IsLineGeometryTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 44);
end;

function V70TryOid44LineLocationCorner(ChildObj : Variant; const SheetId : String; const CompDesignator : String; ChildIndex : Integer; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer) : String;
var
    LineObj   : ISch_Line;
    Loc       : TLocation;
    Cor       : TLocation;
begin
    ...
    AppendExportLog('V70 oid44 line geometry BEFORE sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    try
        LineObj := ChildObj;
        if LineObj = nil then
        begin
            Result := 'OID44_ISch_Line_CAST_RETURNED_NIL_V70';
            AppendExportLog('V70 oid44 line geometry AFTER result=' + Result);
            Exit;
        end;

        { Active v70 geometry read: one accessor family only. }
        Loc := LineObj.Location;
        Cor := LineObj.Corner;
        X1 := Loc.X;
        Y1 := Loc.Y;
        X2 := Cor.X;
        Y2 := Cor.Y;
        Result := 'OID44_ISch_Line_LOCATION_CORNER_OK_V70';
    except
        Result := 'OID44_ISch_Line_LOCATION_CORNER_RUNTIME_EXCEPTION_V70';
    end;

    AppendExportLog('V70 oid44 line geometry AFTER sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result + ' x1=' + IntToStr(X1) + ' y1=' + IntToStr(Y1) + ' x2=' + IntToStr(X2) + ' y2=' + IntToStr(Y2));
end;

procedure V70AddOid44LineRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; const Status : String; X1 : Integer; Y1 : Integer; X2 : Integer; Y2 : Integer);
var
    Key : String;
begin
    Key := Status;
    ...
    Csv.Add(
        ...
        IntToStr(X2) + ';' +
        IntToStr(Y2) + ';' +
        'v70SingleAccessorProbe'
    );
end;

procedure V70Oid44LineProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer; var HitBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
        ...
        begin
            ...
            begin
                ...
                if ChildIter = nil then
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed_V70')
                else
                begin
                    try
                        ChildIndex := 0;
                        ...
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V70IsLineGeometryTarget(ChildOid) then
                            begin
                                Dec(HitBudget);
                                Status := V70TryOid44LineLocationCorner(ChildObj, SheetId, CompDesignator, ChildIndex, X1, Y1, X2, Y2);
                                V70AddOid44LineRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, Status, X1, Y1, X2, Y2);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 64 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated64_V70');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV70Oid44LineProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v70 oid44 line probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v70 oid44 line probe skipped: no focused project'); Exit; end;
    ...
    try
        ...
        Summary.Add('Altium schematic v70 isolated ObjectId 44 ISch_Line Location/Corner geometry probe');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v70 isolates ObjectId 44 and tries one geometry accessor family only: ISch_Line.Location + ISch_Line.Corner.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V70Oid44LineProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget, HitBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V70_OID44_LINE_LOCATION_CORNER_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If LOCATION_CORNER_OK appears with non-zero coordinates, v71 can export ObjectId44 lines into symbolGraphics for a tiny subset first.');
        Summary.Add('If this does not compile, v70b should remove the failing accessor and try one remaining typed accessor only, not a matrix.');
        ...
        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70_oid44_line_location_corner_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70_oid44_line_location_corner_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v70 oid44 line probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v70 oid44 line probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70_oid44_line_location_corner_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70_oid44_line_location_corner_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV70Oid44LineProbeReport;
    end
