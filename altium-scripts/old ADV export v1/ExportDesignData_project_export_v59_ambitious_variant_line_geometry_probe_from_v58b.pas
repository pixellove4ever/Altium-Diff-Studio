{==============================================================================}
{ Altium Diff Studio - Exporter Script v59                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v59';
    SCHEMA_VERSION = 'ads-json-v59';
    SCHEMATIC_SCHEMA_VERSION = 'v59';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndTypedBatchASymbolProbeV59",');
    JsonList.Add('      "graphicsNotes": "V59 keeps validated textRender and accelerates native symbol discovery with a compile-safe typed batch-A matrix for ObjectId 44/4/25: component context, LibRef grouping, fallback scoring, and typed-interface candidate batches. Symbol bodies still use pin-bounds fallback in the JSON until one typed primitive interface is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v59",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v59 INTERFACE MATRIX COMPILE-SAFE            }
{======================================================================================
 ...
 - v59  : test ciblé sur ISch_Line.GetState_Vertex(0/1), choisi car Wire/Bus utilisent déjà
...
function V59DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V59ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V59IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V59InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V59CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V59TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V59_NO_TYPED_GEOMETRY_MATCH_YET';

    { V59 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
      ...
      V59 acceleration:
      - Keep the proven ISch_Line cast.
      - Switch geometry discovery to Variant late-bound probing, so unknown properties should
        fail at runtime inside try/except instead of breaking compile immediately.
      - Probe many likely Altium naming families in one run:
          1) StartLocation / EndLocation
          2) Point1 / Point2
          3) Vertex1 / Vertex2
          4) FirstPoint / SecondPoint
          5) StartPoint / EndPoint
          6) X1/Y1/X2/Y2
          7) Location + Corner fallback
      - If any route works, nativeX1..nativeY2 are populated in the CSV and the status names
        the winning accessor. If none works, v59 still compiles and preserves cast-only proof. }
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V59_VARIANT_GEOMETRY_PROBES_STARTED';

            try
                X1 := LineVar.StartLocation.X;
                Y1 := LineVar.StartLocation.Y;
                X2 := LineVar.EndLocation.X;
                Y2 := LineVar.EndLocation.Y;
                Status := 'ISch_Line_VARIANT_StartLocation_EndLocation_OK_V59';
                Exit;
            except
                Status := Status + '|StartLocation_EndLocation_FAIL';
            end;
            ...
            try
                ...
                X2 := LineVar.Point2.X;
                Y2 := LineVar.Point2.Y;
                Status := 'ISch_Line_VARIANT_Point1_Point2_OK_V59';
                Exit;
            except
                Status := Status + '|Point1_Point2_FAIL';
            end;

            try
                X1 := LineVar.Vertex1.X;
                Y1 := LineVar.Vertex1.Y;
                X2 := LineVar.Vertex2.X;
                Y2 := LineVar.Vertex2.Y;
                Status := 'ISch_Line_VARIANT_Vertex1_Vertex2_OK_V59';
                Exit;
            except
                Status := Status + '|Vertex1_Vertex2_FAIL';
            end;

            try
                X1 := LineVar.FirstPoint.X;
                Y1 := LineVar.FirstPoint.Y;
                X2 := LineVar.SecondPoint.X;
                Y2 := LineVar.SecondPoint.Y;
                Status := 'ISch_Line_VARIANT_FirstPoint_SecondPoint_OK_V59';
                Exit;
            except
                Status := Status + '|FirstPoint_SecondPoint_FAIL';
            end;

            try
                X1 := LineVar.StartPoint.X;
                Y1 := LineVar.StartPoint.Y;
                X2 := LineVar.EndPoint.X;
                Y2 := LineVar.EndPoint.Y;
                Status := 'ISch_Line_VARIANT_StartPoint_EndPoint_OK_V59';
                Exit;
            except
                Status := Status + '|StartPoint_EndPoint_FAIL';
            end;
            ...
            try
                ...
                X2 := LineVar.X2;
                Y2 := LineVar.Y2;
                Status := 'ISch_Line_VARIANT_X1Y1X2Y2_OK_V59';
                Exit;
            except
                Status := Status + '|X1Y1X2Y2_FAIL';
            end;

            try
                X1 := LineVar.Location.X;
                Y1 := LineVar.Location.Y;
                X2 := LineVar.Corner.X;
                Y2 := LineVar.Corner.Y;
                Status := 'ISch_Line_VARIANT_Location_Corner_OK_V59';
                Exit;
            except
                Status := Status + '|Location_Corner_FAIL';
            end;

            Status := Status + '|ALL_V59_VARIANT_LINE_GEOMETRY_ROUTES_FAILED_GEOMETRY_NOT_READ';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V59';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V59';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V59';
            Result := True;
            Exit;
        end;
    except
...
procedure V59LineVertexProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V59IsTarget(Oid) then
        begin
            TargetRole := V59DecisionForObjectId(Oid);
            ExpectedKind := V59ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V59InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V59CompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V59TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'typedBatchA_v59:' + NativeStatus
                    else
...
procedure ExportSchV59LineVertexProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v59 ambitious Variant ISch_Line geometry probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v59 ambitious Variant ISch_Line geometry probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v59RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;lineGeometryProbe_v59;batchResultPlan_v59;recommendedNextAction');
        Summary.Add('Altium schematic v59 ambitious Variant ISch_Line geometry probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v59 ambitious compile-safe Variant geometry matrix for ObjectId 44 after ISch_Line cast was validated; probes StartLocation/EndLocation, Point1/Point2, Vertex1/Vertex2, FirstPoint/SecondPoint, StartPoint/EndPoint, X1/Y1/X2/Y2 and Location/Corner via Variant try/except while preserving all failure history.');
        Summary.Add('SafeProperties=ObjectId, iterator order, typed ISch_Line cast, and Variant late-bound property probes guarded by try/except; direct typed/generic geometry remains disabled.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V59LineVertexProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V59_LINE_GETSTATE_VERTEX_GEOMETRY_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 generic geometry remains disabled; v59 attempts many Variant late-bound ISch_Line geometry route names in one run.');
        Summary.Add('Next step after v59: if a Variant route succeeds, promote it to JSON symbolGraphics; if all routes fail, switch to component child/library-symbol route.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59_ambitious_variant_line_geometry_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59_ambitious_variant_line_geometry_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v59 ambitious Variant ISch_Line geometry probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v59 ambitious Variant ISch_Line geometry probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59_ambitious_variant_line_geometry_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59_ambitious_variant_line_geometry_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV59LineVertexProbeReport;
    end
