{==============================================================================}
{ Altium Diff Studio - Exporter Script v49b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v49b';
    SCHEMA_VERSION = 'ads-json-v49b';
    SCHEMATIC_SCHEMA_VERSION = 'v49b';
...
{ V49b: typed, compile-visible helpers for component display text geometry.
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusDisplayGeometryV49b",');
    JsonList.Add('      "graphicsNotes": "V49 keeps the validated v48 displayObjects and adds targeted text geometry derived from typed component Designator/Comment display objects when available. ObjectId 45 is designator display, ObjectId 46 is comment display, ObjectId 25 is primary display candidate.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v49b",');
    JsonList.Add('        "focus": "component display objects text geometry for designator/comment/primary",');
...
{==============================================================================}
{ 3f. TARGETED DISPLAY TEXT GEOMETRY PROBE - v49 SAFE                    }
{==============================================================================}
{ Purpose:
...
function V49DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V49ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 25 then Result := 'primaryDisplayText_or_designator'
    else if ObjectId = 45 then Result := 'designatorText_expected'
    else if ObjectId = 46 then Result := 'commentText_expected';
end;

function V49IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
    if (ObjectId = 25) or (ObjectId = 45) or (ObjectId = 46) then
        Result := True;
end;

procedure V49GeometryDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V49IsTarget(Oid) then
        begin
            TargetRole := V49DecisionForObjectId(Oid);
            ExpectedKind := V49ExpectedTextKindForObjectId(Oid);
...
procedure ExportSchV49GeometryReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v49 geometry skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v49 geometry skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v49RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('Altium schematic v49 display text geometry probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=probe geometry for high-confidence display objects 25/45/46 using typed component Designator/Comment display accessors');
        Summary.Add('SafeProperties=ObjectId on target objects; geometry read only through typed ISch_Component.Designator/Comment Location accessors');
        Summary.Add('CsvColumns=sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v49RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V49GeometryDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V49_DISPLAY_GEOMETRY_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('If ObjectId 45 rows consistently carry the current component designator, it is the designator display object.');
        Summary.Add('If ObjectId 46 rows consistently carry the current component comment, it is the comment display object.');
        Summary.Add('ObjectId 25 remains the primary display object candidate and can be compared against 45/46 by cluster.');
        Summary.Add('Direct Obj.Text probing is intentionally not used because this DelphiScript engine rejected generic Variant.Text at compile time.');
        Summary.Add('Next step v50: if geometry compiles and values are stable, render designator/comment text positions in the viewer; then probe symbol body primitives 44/4.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v49b_display_geometry.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v49b_display_geometry_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v49 display geometry CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v49 display geometry summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v49b_display_geometry.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v49b_display_geometry_summary.txt'));
