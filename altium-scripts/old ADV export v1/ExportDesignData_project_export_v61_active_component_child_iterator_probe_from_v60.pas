{==============================================================================}
{ Altium Diff Studio - Exporter Script v61                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v61';
    SCHEMA_VERSION = 'ads-json-v61';
    SCHEMATIC_SCHEMA_VERSION = 'v61';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndActiveComponentChildIteratorProbeV61",');
    JsonList.Add('      "graphicsNotes": "V61 keeps validated textRender and switches native symbol discovery away from direct ObjectId 44 geometry access toward a compile-safe active component child iterator route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v61",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe active component child iterator route probe for native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v61 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v61  : active réellement Component.SchIterator_Create sur le composant courant, sans géométrie,
...
function V61DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V61ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V61IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V61InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V61CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V61ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
function V61TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V61_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V61 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V61_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V61_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V61_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V61_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V61_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V61ActiveComponentChildIteratorProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V61IsTarget(Oid) then
        begin
            TargetRole := V61DecisionForObjectId(Oid);
            ExpectedKind := V61ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V61InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V61CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ChildIterHits := 0;
            ChildIterStatus := 'notAttempted';
            if CurrentComp <> nil then
                V61ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V61TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v61:' + NativeStatus
                    else
...
procedure ExportSchV61ActiveComponentChildIteratorProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v61 active component child iterator route probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v61 active component child iterator route probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v61RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;activeComponentChildIteratorProbe_v61;childIteratorObjectCount_v61;childIteratorObjectIds_v61;childIteratorTargetHits_v61;routeResultPlan_v61;recommendedNextAction');
        Summary.Add('Altium schematic v61 active component child iterator route probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v61 compile-safe route activation after v60: stop probing unknown direct/Variant geometry properties and map ObjectId 44 through component cluster, LibRef grouping, and active component child iterator route without touching risky geometry accessors.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V61ActiveComponentChildIteratorProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V61_ACTIVE_COMPONENT_CHILD_ITERATOR_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v61 actively inventories Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v61: if child iterator inventories reveal 44/4 primitives inside components, use that route for symbolGraphics; otherwise improve viewer fallback and proceed to BOM/SCH/PCB comparator.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v61_active_component_child_iterator_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v61_active_component_child_iterator_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v61 active component child iterator route probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v61 active component child iterator route probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v61_active_component_child_iterator_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v61_active_component_child_iterator_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV61ActiveComponentChildIteratorProbeReport;
    end
