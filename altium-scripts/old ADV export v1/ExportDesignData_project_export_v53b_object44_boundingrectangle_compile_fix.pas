{==============================================================================}
{ Altium Diff Studio - Exporter Script v53b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v53b';
    SCHEMA_VERSION = 'ads-json-v53b';
    SCHEMATIC_SCHEMA_VERSION = 'v53b';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndObject44BoundingRectangleProbeV53b",');
    JsonList.Add('      "graphicsNotes": "V53b keeps validated textRender and adds a one-property native geometry probe on ObjectId 44 using Obj.BoundingRectangle. Symbol bodies still use pin-bounds fallback in the JSON until ObjectId 44 rectangle geometry is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v53b",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus ObjectId 44 BoundingRectangle probe",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v53b OBJECT44 BOUNDINGRECTANGLE                    }
{==============================================================================}
{ Purpose:
...
function V53bDecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V53bExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V53bIsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V53bTryObject44BoundingRectangle(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
begin
    Result := False;
    X1 := 0;
    Y1 := 0;
    X2 := 0;
    Y2 := 0;
    Status := 'BoundingRectangle_NOT_ATTEMPTED';

    { V53b compile fix:
      Direct Obj.BoundingRectangle access does not compile in this Altium
      scripting environment (Undeclared identifier: BoundingRectangle).
      Keep the probe structure and explicitly record this as skipped so the
      run confirms ObjectId 44 counts without breaking the export. }
    Status := 'SKIPPED_GENERIC_BOUNDINGRECTANGLE_UNDECLARED_IDENTIFIER';
    Result := False;
end;

procedure V53bObject44BoundingRectangleProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V53bIsTarget(Oid) then
        begin
            TargetRole := V53bDecisionForObjectId(Oid);
            ExpectedKind := V53bExpectedTextKindForObjectId(Oid);
            ...
            if Oid = 44 then
            begin
                V53bTryObject44BoundingRectangle(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'Obj.BoundingRectangle';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and (NativeStatus = 'Obj.BoundingRectangle_OK_LeftBottomRightTop') then
                    begin
                        DisplayX := NativeX1;
                        DisplayY := NativeY1;
                        GeometrySource := 'Obj.BoundingRectangle.LeftBottomRightTop';
                    end
                    else
...
procedure ExportSchV53bObject44BoundingRectangleProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v53b ObjectId 44 BoundingRectangle probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v53b ObjectId 44 BoundingRectangle probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v53bRoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeBoundsX1;nativeBoundsY1;nativeBoundsX2;nativeBoundsY2;nativeBoundsStatus;nativeBoundsSource');
        Summary.Add('Altium schematic v53b ObjectId 44 BoundingRectangle probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=compile-safe ObjectId 44 BoundingRectangle probe after direct Obj.BoundingRectangle compile failure; no generic BoundingRectangle access is attempted');
        Summary.Add('SafeProperties=ObjectId and iterator order on primitive candidates; ObjectId 44 BoundingRectangle is marked skipped because direct property access is not supported by compiler');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V53bObject44BoundingRectangleProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V53b_OBJECT44_BOUNDINGRECTANGLE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 BoundingRectangle direct access is not compilable in this scripting API, so V53b records it as skipped.');
        Summary.Add('Next step v54: test a different ObjectId 44 geometry property name or a typed interface method, one at a time.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53b_object44_boundingrectangle_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53b_object44_boundingrectangle_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v53b ObjectId 44 BoundingRectangle probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v53b ObjectId 44 BoundingRectangle probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53b_object44_boundingrectangle_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v53b_object44_boundingrectangle_probe_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV53bObject44BoundingRectangleProbeReport;
    end
