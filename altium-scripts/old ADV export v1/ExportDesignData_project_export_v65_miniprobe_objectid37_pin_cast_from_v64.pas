{==============================================================================}
{ Altium Diff Studio - Exporter Script v65 MINI-PROBE OID37                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v65';
    SCHEMA_VERSION = 'ads-json-v65';
    SCHEMATIC_SCHEMA_VERSION = 'v65';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV65",');
    JsonList.Add('      "graphicsNotes": "V65 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v65",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v65 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v65  : reprend la route V62 sûre après le blocage constaté de la V63 ambitieuse.
          Aucun cast-matrix large n est exécuté; uniquement inventaire enfants ObjectId-only.

 Decision v65 SAFE after v63 hang:
...
function V65DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V65ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V65IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V65DecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V65AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V65DecodeChildObjectId(ObjectId);
...
function V65InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V65CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V65ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V65AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V65IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V65TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V65_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V65 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V65_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V65_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V65_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V65_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V65_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V65ChildObjectIdDecoderProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V65IsTarget(Oid) then
        begin
            TargetRole := V65DecisionForObjectId(Oid);
            ExpectedKind := V65ExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V65InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V65CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ...
            if CurrentComp <> nil then
                V65ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V65TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v65:' + NativeStatus
                    else
...
procedure ExportSchV65ChildObjectIdDecoderProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v65 child ObjectId decoder probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v65 child ObjectId decoder probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v65RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;childObjectIdDecoderProbe_v65;childIteratorObjectCount_v65;childIteratorObjectIds_v65;childIteratorDecodedRoles_v65;childIteratorTargetOrCandidateHits_v65;routeResultPlan_v65;recommendedNextAction');
        Summary.Add('Altium schematic v65 SAFE child ObjectId decoder probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v65 SAFE rollback after v63 hang: keep the proven child ObjectId decoder route, add no broad cast matrix, and avoid all risky geometry accessors or COM-heavy loops.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V65ChildObjectIdDecoderProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V65_SAFE_CHILD_OBJECTID_DECODER_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 rows are included and childIteratorDecodedRoles_v65 decodes recurrent child ObjectIds 37, 27, 8, 18, 22 and 5 for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v65 actively inventories and decodes Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v65: analyse CSV safely; if needed, v65 should test one ObjectId and one interface at a time with progress logging.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_child_objectid_decoder_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_child_objectid_decoder_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v65 child ObjectId decoder probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v65 child ObjectId decoder probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
{==============================================================================}
{ 3g. V65 MINI-PROBE CIBLEE OBJECTID 37 - ISch_Pin CAST-ONLY              }
{==============================================================================}
{ Objectif v65:
  - accelerer sans refaire la matrice large v63 qui a bloque Altium;
  - tester seulement ObjectId 37, car V64 a montr� que 27 = param�tres; 37 est le prochain candidat enfant r�current;
  ...
  Test actif v65:
  - ChildOid = 37 depuis Component.SchIterator_Create
  - cast-only vers ISch_Pin
  - aucune lecture PinObj.Location/Name/Designator dans cette version.
}

procedure V65MiniProbe37DumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
                ...
                begin
                    try
                        ...
                        begin
                            ...
                            if ChildOid = 37 then
                            begin
                                AppendExportLog('V65 mini-probe BEFORE ObjectId37 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));
                                Status := 'OID37_FOUND_CAST_NOT_ATTEMPTED';
                                try
                                    PinObj := ChildObj;
                                    if PinObj <> nil then
                                        Status := 'OID37_ISch_Pin_CAST_OK_NO_PROPERTY_READ_V65'
                                    else
                                        Status := 'OID37_ISch_Pin_CAST_RETURNED_NIL_V65';
                                except
                                    Status := 'OID37_ISch_Pin_CAST_EXCEPTION_V65';
                                end;
                                AppendExportLog('V65 mini-probe AFTER ObjectId37 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' status=' + Status);
                                ...
                                Csv.Add(
                                    ...
                                    IntToStr(ChildIndex) + ';' +
                                    IntToStr(ChildOid) + ';' +
                                    ProbeEscapeCell(V65DecodeChildObjectId(ChildOid)) + ';' +
                                    ProbeEscapeCell(Status) + ';' +
                                    'ISch_Pin;castOnly;noGeometryNoTextNoLocation;v65MiniProbe'
                                );
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
...
procedure ExportSchV65MiniProbe37Report;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v65 mini-probe ObjectId37 skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v65 mini-probe ObjectId37 skipped: no focused project'); Exit; end;
    ...
    try
        ...
        Summary.Add('Altium schematic v65 mini-probe ObjectId 37 ISch_Pin cast-only');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v65 targeted acceleration after v63 hang: test only child ObjectId 37 with ISch_Pin cast-only, limited component budget, before/after debug logging.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference. ObjectId 37 is cast-only to ISch_Pin; no geometry/text/location property is read.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V65MiniProbe37DumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V65_MINIPROBE_OBJECTID37_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If ObjectId 37 casts OK to ISch_Pin, v66 can either confirm it is pin metadata or move to ObjectId 8/18/22/5; no geometry accessor yet.');
        Summary.Add('If ObjectId 37 is not useful, target ObjectId 8 or 18 with the same mini-probe pattern; do not re-enable broad cast matrices.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_miniprobe_oid37_pin_cast.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_miniprobe_oid37_pin_cast_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v65 mini-probe ObjectId37 CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v65 mini-probe ObjectId37 summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_child_objectid_decoder_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_child_objectid_decoder_probe_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_miniprobe_oid37_pin_cast.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v65_miniprobe_oid37_pin_cast_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV65ChildObjectIdDecoderProbeReport;
        ExportSchV65MiniProbe37Report;
    end
