...
{==============================================================================}
{ Altium Diff Studio - Exporter Script v70b OID44 LINE LOCATION/CORNER PROBE                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v70b';
    SCHEMA_VERSION = 'ads-json-v70b';
    SCHEMATIC_SCHEMA_VERSION = 'v70b';
...
begin
    ...
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineEdgesSource": "keepOutLayerTracksOnly_eRectObjectDisabled_V70b",');
    JsonList.Add('  "boardOutlineEdgesCount": ' + IntToStr(OutlineEdgeCount) + ',');
    JsonList.Add('  "boardOutlineClassification": {');
    JsonList.Add('    "method": "widthHeuristic_tracksOnly_V70b",');
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV70b",');
    JsonList.Add('      "graphicsNotes": "V70b SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v70b",');
...
{==============================================================================}
{ 3g. V70b MINI-PROBE ISOLEE OBJECTID 44 - LINE LOCATION/CORNER READ           }
{==============================================================================}
{ Objectif v70b:
  - partir de la V69b validee;
  - faire le dernier essai COM geometrique raisonnable avant d'arreter la piste;
  - isoler seulement ObjectId 44, deja valide en cast-only ISch_Line;
  - tenter UNE seule famille d'accesseurs: ISch_Line cast-only because Location/Corner do not compile;
  - budget tres petit et log avant/apres chaque tentative.
  ...
  - V68 : ObjectId 18 + ISch_Arc.Location/Radius => aucun hit exploitable.
  - V69b : ObjectId 5 + ISch_Rectangle.Location/Corner => aucun hit exploitable.
  - Routes deja mauvaises/non compilees: Variant Location, BoundingRectangle,
    StartLocation, GetState_Vertex, matrices larges de casts.

  Risque v70b:
  - Si ISch_Line.Location ou ISch_Line.Corner ne compile pas dans cette API Altium,
    on corrigera en v70bb en retirant l'accesseur fautif. Ne pas ajouter de matrice.
  - Si aucune ligne exploitable n'est trouvee, on arrete la recherche COM sur les
    corps de symboles et on garde pinBoundsFallback ou on bascule vers parsing SchDoc.
}

function V70bIsLineGeometryTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 44);
end;

function V70bTryOid44LineLocationCorner(ChildObj : Variant; const SheetId : String; const CompDesignator : String; ChildIndex : Integer; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer) : String;
var
    LineObj   : ISch_Line;
begin
    X1 := 0;
    ...
    AppendExportLog('V70b oid44 line geometry BEFORE sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    try
        LineObj := ChildObj;
        if LineObj = nil then
        begin
            Result := 'OID44_ISch_Line_CAST_RETURNED_NIL_V70b';
            AppendExportLog('V70b oid44 line geometry AFTER result=' + Result);
            Exit;
        end;

        { V70b compile fix: ISch_Line.Location is not declared in this Altium API.
          Keep only the proven safe cast. No geometry property is read here. }
        Result := 'OID44_ISch_Line_CAST_OK_NO_PROPERTY_READ_V70b';
    except
        Result := 'OID44_ISch_Line_CAST_RUNTIME_EXCEPTION_V70b';
    end;

    AppendExportLog('V70b oid44 line geometry AFTER sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result + ' x1=' + IntToStr(X1) + ' y1=' + IntToStr(Y1) + ' x2=' + IntToStr(X2) + ' y2=' + IntToStr(Y2));
end;

procedure V70bAddOid44LineRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; const Status : String; X1 : Integer; Y1 : Integer; X2 : Integer; Y2 : Integer);
var
    Key : String;
begin
    Key := Status;
    ...
    Csv.Add(
        ...
        IntToStr(ChildIndex) + ';44;' +
        ProbeEscapeCell(V67DecodeChildObjectId(44)) + ';' +
        'ISch_Line;' +
        ...
        IntToStr(X2) + ';' +
        IntToStr(Y2) + ';' +
        'v70bSingleAccessorProbe'
    );
end;

procedure V70bOid44LineProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer; var HitBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
        ...
        begin
            ...
            begin
                ...
                if ChildIter = nil then
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed_V70b')
                else
                begin
                    try
                        ChildIndex := 0;
                        ...
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V70bIsLineGeometryTarget(ChildOid) then
                            begin
                                Dec(HitBudget);
                                Status := V70bTryOid44LineLocationCorner(ChildObj, SheetId, CompDesignator, ChildIndex, X1, Y1, X2, Y2);
                                V70bAddOid44LineRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, Status, X1, Y1, X2, Y2);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 64 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated64_V70b');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV70bOid44LineProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v70b oid44 line probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v70b oid44 line probe skipped: no focused project'); Exit; end;
    ...
    try
        ...
        Summary.Add('Altium schematic v70b isolated ObjectId 44 ISch_Line cast-only geometry probe');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v70b isolates ObjectId 44 and tries one geometry accessor family only: ISch_Line cast-only because Location/Corner do not compile.');
        Summary.Add('Limits=first 30 component roots across project; max 64 child objects per component; max 20 ObjectId44 hits.');
        Summary.Add('KnownGood=27->ISch_Parameter; 37->ISch_Pin; 8->ISch_NetLabel; 44->ISch_Line cast-only.');
        Summary.Add('KnownBad=V63 broad cast matrix caused hang; Variant Location/BoundingRectangle/StartLocation/GetState_Vertex routes failed or did not compile; v68/v69b geometry probes produced no exploitable hit.');
        Summary.Add('ActiveTest=ObjectId44 only; ISch_Line.Location and ISch_Line.Corner only.');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V70bOid44LineProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget, HitBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V70b_OID44_LINE_CAST_ONLY_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If CAST_OK_NO_PROPERTY_READ appears with non-zero coordinates, v71 can export ObjectId44 lines into symbolGraphics for a tiny subset first.');
        Summary.Add('If this does not compile, v70bb should remove the failing accessor and try one remaining typed accessor only, not a matrix.');
        Summary.Add('If no exploitable geometry appears, stop COM probing for SCH symbol bodies and keep pinBoundsFallback or switch to direct SchDoc parsing.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70b_oid44_line_cast_only_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70b_oid44_line_cast_only_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v70b oid44 line probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v70b oid44 line probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70b_oid44_line_cast_only_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v70b_oid44_line_cast_only_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV70bOid44LineProbeReport;
    end
