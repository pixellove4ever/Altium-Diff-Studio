{==============================================================================}
{ Altium Diff Studio - Exporter Script v59b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v59b';
    SCHEMA_VERSION = 'ads-json-v59b';
    SCHEMATIC_SCHEMA_VERSION = 'v59b';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndTypedBatchASymbolProbeV59b",');
    JsonList.Add('      "graphicsNotes": "V59b keeps validated textRender and accelerates native symbol discovery with a compile-safe typed batch-A matrix for ObjectId 44/4/25: component context, LibRef grouping, fallback scoring, and typed-interface candidate batches. Symbol bodies still use pin-bounds fallback in the JSON until one typed primitive interface is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v59b",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v59b INTERFACE MATRIX COMPILE-SAFE            }
{======================================================================================
 ...
 - v59b  : test ciblé sur ISch_Line.GetState_Vertex(0/1), choisi car Wire/Bus utilisent déjà
...
function V59bDecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V59bExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V59bIsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V59bInterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V59bCompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V59bTryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V59b_NO_TYPED_GEOMETRY_MATCH_YET';

    { V59b AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
      ...
      - If any route works, nativeX1..nativeY2 are populated in the CSV and the status names
        the winning accessor. If none works, v59b still compiles and preserves cast-only proof. }
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V59b_VARIANT_GEOMETRY_PROBES_STARTED';
            ...
            Status := Status + '|ALL_V59b_VARIANT_LINE_GEOMETRY_ROUTES_FAILED_GEOMETRY_NOT_READ';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V59b';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V59b';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V59b';
            Result := True;
            Exit;
        end;
    except
...
procedure V59bLineVertexProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V59bIsTarget(Oid) then
        begin
            TargetRole := V59bDecisionForObjectId(Oid);
            ExpectedKind := V59bExpectedTextKindForObjectId(Oid);
            ...
            InterfaceMatrix := V59bInterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V59bCompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V59bTryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'typedBatchA_v59b:' + NativeStatus
                    else
...
procedure ExportSchV59bLineVertexProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
    ...
    try
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V59bLineVertexProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V59b_LINE_GETSTATE_VERTEX_GEOMETRY_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 generic geometry remains disabled; v59b attempts many Variant late-bound ISch_Line geometry route names in one run.');
        Summary.Add('Next step after v59b: if a Variant route succeeds, promote it to JSON symbolGraphics; if all routes fail, switch to component child/library-symbol route.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59b_ambitious_variant_line_geometry_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59b_ambitious_variant_line_geometry_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v59b ambitious Variant ISch_Line geometry probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v59b ambitious Variant ISch_Line geometry probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59b_ambitious_variant_line_geometry_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v59b_ambitious_variant_line_geometry_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV59bLineVertexProbeReport;
    end
