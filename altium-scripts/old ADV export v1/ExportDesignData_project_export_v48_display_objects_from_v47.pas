{==============================================================================}
{ Altium Diff Studio - Exporter Script v48                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v48';
    SCHEMA_VERSION = 'ads-json-v48';
    SCHEMATIC_SCHEMA_VERSION = 'v48';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusDisplayObjectsV48",');
    JsonList.Add('      "graphicsNotes": "V48 adds component displayObjects derived from the validated v47 association: ObjectId 45 is designator display, ObjectId 46 is comment display, ObjectId 25 is primary display candidate. Symbol bodies remain pin-bounds fallbacks until native geometry is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v48",');
    JsonList.Add('        "focus": "component display objects metadata for designator/comment/primary text",');
    ...
    Begin
        ...
        JsonList.Add('            "designator": { "objectId": 45, "role": "designatorDisplayObject", "text": "' + EscapeStr(Component.Designator.Text) + '", "source": "v47ComponentAssociation" },');
        JsonList.Add('            "comment": { "objectId": 46, "role": "commentDisplayObject", "text": "' + EscapeStr(Component.Comment.Text) + '", "source": "v47ComponentAssociation" },');
        JsonList.Add('            "primary": { "objectId": 25, "role": "primaryDisplayObjectCandidate", "text": "' + EscapeStr(Component.Designator.Text) + '", "source": "v47ComponentAssociationCandidate" }');
        JsonList.Add('          },');
...
{==============================================================================}
{ 3f. TARGETED COMPONENT TEXT ASSOCIATION PROBE - v47 SAFE                    }
{==============================================================================}
{ Purpose:
...
function V47DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V47ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V47IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
procedure V47TextAssociationDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V47IsTarget(Oid) then
        begin
            TargetRole := V47DecisionForObjectId(Oid);
            ExpectedKind := V47ExpectedTextKindForObjectId(Oid);
            ...
            Csv.Add(
                ...
                ProbeEscapeCell('ISch_Component.Designator.Text / ISch_Component.Comment.Text')
            );
        end;
...
procedure ExportSchV47TextAssociationReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v48 text association skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v48 text association skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v48RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;textSource');
        Summary.Add('Altium schematic v48 component text/displayObjects association probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=associate high-confidence display objects 25/45/46 with native component designator/comment text');
        Summary.Add('SafeProperties=ObjectId on target objects; text read only from validated ISch_Component.Designator.Text and ISch_Component.Comment.Text');
        Summary.Add('CsvColumns=sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v48RoleDecision;expectedTextKind;componentDesignatorText;componentCommentText;componentLibRef;textSource');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V47TextAssociationDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V47_TEXT_ASSOCIATION_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('Next step v49: probe geometry for 25/45/46 display objects and component symbol primitives.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v48_text_association.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v48_text_association_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v48 text association CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v48 text association summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v48_text_association.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v48_text_association_summary.txt'));
...
begin
    ...
    begin
        ...
        ExportSchV47TextAssociationReport;
    end
