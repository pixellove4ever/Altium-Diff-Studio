{==============================================================================}
{ Altium Diff Studio - Exporter Script v67 GROUPED MINI-PROBE OID18/22/5                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v67';
    SCHEMA_VERSION = 'ads-json-v67';
    SCHEMATIC_SCHEMA_VERSION = 'v67';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV67",');
    JsonList.Add('      "graphicsNotes": "V67 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v67",');
...
{==============================================================================}
{ 3g. V67 MINI-PROBE GROUPEE OBJECTID 18 / 22 / 5 - CAST-ONLY SAFE          }
{==============================================================================}
{ Objectif v67:
  - repartir de V66 validee;
  - retirer du chemin d execution les anciens rapports larges/redondants;
  - tester les trois derniers candidats serieux en une seule passe: ObjectId 18, 22 et 5;
  - rester SAFE: budget limite, logs avant/apres chaque sous-test, aucune lecture de geometrie;
  - ne jamais relancer la matrice large de la V63 qui avait bloque Altium.

  Historique conserve, utile pour ne pas refaire les memes essais:
  ...
  - V66 : ObjectId 8 caste OK en ISch_NetLabel sur 162 objets => labels/nets, pas corps graphique.
  - ObjectId 44 : ISch_Line cast-only valide, mais tous les acces geometrie connus ont echoue ou ne compilent pas.

  Test actif v67:
  - ChildOid in [18, 22, 5] depuis Component.SchIterator_Create.
  - Cast-only sur interfaces compilees et deja presentes dans le script: ISch_Line, ISch_Rectangle, ISch_Arc.
  - Aucune lecture .Location/.X/.Y/.Text/.Name/.Corner/.Radius.
  - Budget volontairement plus petit que V66 pour limiter le risque COM: 60 composants, 96 enfants max/composant.
}
function V67IsGroupedMiniProbeTarget(ObjectId : Integer) : Boolean;
begin
    Result := (ObjectId = 18) or (ObjectId = 22) or (ObjectId = 5);
end;

function V67GroupedCandidateRole(ObjectId : Integer) : String;
begin
    Result := 'notGroupedTarget';
    if ObjectId = 18 then
        Result := 'candidate18_possibleSymbolPrimitiveOrSymbolPart'
    else if ObjectId = 22 then
        Result := 'candidate22_possibleSymbolPrimitiveOrSymbolPart'
    else if ObjectId = 5 then
        Result := 'candidate5_possiblePrimitiveOrSmallGraphicOrMetadata';
end;

function V67TrySinglePrimitiveCast(ChildObj : Variant; ChildOid : Integer; const InterfaceName : String; const SheetId : String; const CompDesignator : String; ChildIndex : Integer) : String;
var
    LineObj : ISch_Line;
...
begin
    ...
    AppendExportLog('V67 grouped mini-probe BEFORE OID=' + IntToStr(ChildOid) + ' interface=' + InterfaceName + ' sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));

    if InterfaceName = 'ISch_Line' then
    begin
        try
            LineObj := ChildObj;
            if LineObj <> nil then
                ...
                Result := 'CAST_RETURNED_NIL';
        except
            Result := 'CAST_EXCEPTION';
        end;
    end
    else if InterfaceName = 'ISch_Rectangle' then
    begin
        try
            RectObj := ChildObj;
            if RectObj <> nil then
    ...
    else if InterfaceName = 'ISch_Arc' then
    begin
        try
            ArcObj := ChildObj;
            if ArcObj <> nil then
        ...
        Result := 'UNKNOWN_INTERFACE_SKIPPED';

    AppendExportLog('V67 grouped mini-probe AFTER OID=' + IntToStr(ChildOid) + ' interface=' + InterfaceName + ' sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' result=' + Result);
end;

procedure V67AddGroupedMiniProbeRow(Csv : TStringList; Counters : TStringList; const SheetId : String; const SheetName : String; const SchPath : String; ObjIndex : Integer; ComponentIndex : Integer; const CompDesignator : String; const CompComment : String; const CompLibRef : String; ChildIndex : Integer; ChildOid : Integer; const InterfaceName : String; const CastResult : String);
var
    Key : String;
begin
    Key := 'OID' + IntToStr(ChildOid) + '_' + InterfaceName + '_' + CastResult + '_NO_PROPERTY_READ_V67';
    ProbeIncCounter(Counters, Key);
    Csv.Add(
        ProbeEscapeCell(SheetId) + ';' +
        ProbeEscapeCell(SheetName) + ';' +
        ...
        IntToStr(ChildIndex) + ';' +
        IntToStr(ChildOid) + ';' +
        ProbeEscapeCell(V67DecodeChildObjectId(ChildOid)) + ';' +
        ProbeEscapeCell(V67GroupedCandidateRole(ChildOid)) + ';' +
        ProbeEscapeCell(InterfaceName) + ';' +
        ProbeEscapeCell(CastResult) + ';' +
        'castOnly;noGeometryNoTextNoLocation;v67GroupedMiniProbe'
    );
end;

procedure V67GroupedMiniProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
            ...
            begin
                ...
                begin
                    try
                        ...
                        Guard := 0;
                        ChildObj := ChildIter.FirstSchObject;
                        while (ChildObj <> nil) and (Guard < 96) do
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if V67IsGroupedMiniProbeTarget(ChildOid) then
                            begin
                                CastResult := V67TrySinglePrimitiveCast(ChildObj, ChildOid, 'ISch_Line', SheetId, CompDesignator, ChildIndex);
                                V67AddGroupedMiniProbeRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, ChildOid, 'ISch_Line', CastResult);

                                CastResult := V67TrySinglePrimitiveCast(ChildObj, ChildOid, 'ISch_Rectangle', SheetId, CompDesignator, ChildIndex);
                                V67AddGroupedMiniProbeRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, ChildOid, 'ISch_Rectangle', CastResult);

                                CastResult := V67TrySinglePrimitiveCast(ChildObj, ChildOid, 'ISch_Arc', SheetId, CompDesignator, ChildIndex);
                                V67AddGroupedMiniProbeRow(Csv, Counters, SheetId, SheetName, SchPath, ObjIndex, ComponentIndex, CompDesignator, CompComment, CompLibRef, ChildIndex, ChildOid, 'ISch_Arc', CastResult);
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                            ChildObj := ChildIter.NextSchObject;
                        ...
                        if Guard >= 96 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated96');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV67GroupedMiniProbeReport;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v67 grouped mini-probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v67 grouped mini-probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;flatObjectIndex;componentIndex;componentDesignatorText;componentCommentText;componentLibRef;childIndex;childObjectId;childDecodedRole;candidateRole;interfaceTest;castStatus;accessMode;disabledReads;probeVersion');
        Summary.Add('Altium schematic v67 grouped mini-probe ObjectId 18/22/5 cast-only');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v67 targeted acceleration after v63 hang: test the three remaining serious child ObjectId candidates 18, 22 and 5 in one bounded pass.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference. Candidate objects are cast-only to ISch_Line/ISch_Rectangle/ISch_Arc; no geometry/text/location property is read.');
        Summary.Add('Limits=first 60 component roots across project; max 96 child objects per component. This is intentionally smaller than v66 to avoid another Altium hang.');
        Summary.Add('KnownGood=27->ISch_Parameter; 37->ISch_Pin; 8->ISch_NetLabel; 44->ISch_Line cast-only but no geometry access.');
        Summary.Add('KnownBad=V63 broad cast matrix caused hang; generic/Variant Location/BoundingRectangle/StartLocation/GetState_Vertex routes failed or did not compile.');
        Summary.Add('ActiveTest=ObjectId 18/22/5 only; interfaces ISch_Line, ISch_Rectangle, ISch_Arc only; no broad matrix.');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V67GroupedMiniProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget);
                    Inc(SheetCount);
                end;
            end;
        ...
        Summary.Add('[V67_GROUPED_MINIPROBE_OBJECTID18_22_5_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If one ObjectId/interface pair casts cleanly and recurrently, next version can isolate that single pair and only then test one confirmed coordinate accessor, still with logs.');
        Summary.Add('If all three candidates only return nil/exception or look like metadata, stop COM probing for SCH symbol bodies and keep pinBoundsFallback or switch to direct SchDoc parsing.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v67_grouped_miniprobe_oid18_22_5_cast.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v67_grouped_miniprobe_oid18_22_5_cast_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v67 grouped mini-probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v67 grouped mini-probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v67_grouped_miniprobe_oid18_22_5_cast.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v67_grouped_miniprobe_oid18_22_5_cast_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV67GroupedMiniProbeReport;
    end
