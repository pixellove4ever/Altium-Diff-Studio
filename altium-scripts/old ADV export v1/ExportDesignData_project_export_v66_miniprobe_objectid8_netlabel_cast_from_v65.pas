{==============================================================================}
{ Altium Diff Studio - Exporter Script v66 MINI-PROBE OID8                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v66';
    SCHEMA_VERSION = 'ads-json-v66';
    SCHEMATIC_SCHEMA_VERSION = 'v66';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndSafeChildDecoderProbeV66",');
    JsonList.Add('      "graphicsNotes": "V66 SAFE keeps validated textRender and rolls back from the hanging v63 cast matrix to the proven compile-safe child ObjectId decoder route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v66",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v66 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v66  : reprend la route V62 sûre après le blocage constaté de la V63 ambitieuse.
          Aucun cast-matrix large n est exécuté; uniquement inventaire enfants ObjectId-only.

 Decision v66 SAFE after v63 hang:
...
function V66DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V66ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V66IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V66DecodeChildObjectId(ObjectId : Integer) : String;
begin
...
function V66AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V66DecodeChildObjectId(ObjectId);
...
function V66InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V66CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V66ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
...
begin
    ...
    try
        ...
        begin
            ...
            ChildDecodedRoles := V66AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V66IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
...
function V66TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V66_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V66 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
      What is already validated:
      - SCH logical export, hierarchy anchors, textRender, designator/comment locations.
      - ObjectId 44 appears 378 times and is the best native symbol-body candidate.
      - v57b/v58b proved ObjectId 44 casts successfully to ISch_Line on all candidates.

      What failed before and must stay documented:
      - Generic Obj.Location.X/Y                  -> compile-time undeclared Location.
      - Generic Obj.BoundingRectangle             -> compile-time undeclared BoundingRectangle.
      - Generic Obj.X1/Y1/X2/Y2                   -> not exposed on generic Obj.
      - Typed ISch_Line.Location                  -> compile-time undeclared Location.
      - Typed ISch_Line.GetState_Vertex(0/1)      -> compile-time undeclared GetState_Vertex.
      - Rect/Arc typed geometry reads were intentionally not enabled after the line failures.

      V66 route change:
      - Keep the proven ISch_Line cast as evidence that ObjectId 44 belongs to the native symbol body.
      - Stop all unknown direct/Variant geometry accessors: they fail at compile time in Altium script.
      - Use the CSV to map ObjectId 44 by component cluster and LibRef.
      - Next executable experiment should target a confirmed child/library iterator API from ISch_Component
        or library symbol documents, not LineObj.Location/StartLocation-style properties. }
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V66_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';

            { V66 ROUTE PIVOT
              -----------------------------------------------------------------
              V59 proved an important negative result: PascalScript still resolves
              unknown Variant dot-properties at compile time in this environment.
              Therefore LineVar.StartLocation.X failed with:
                  undeclared identifier: StartLocation

              Consequence: all dot-based Variant geometry probes are disabled here
              to keep the script runnable. The candidate property families are kept
              in the history above, but they must be tested one at a time only if
              we have a confirmed Altium API accessor name.

              Current safe state remains:
              - ObjectId 44 casts to ISch_Line.
              - Geometry is not read yet.
              - No StartLocation/Point1/Vertex1/FirstPoint/StartPoint/X1/Location
                property access is attempted in executable code. }

            Status := Status + '|V66_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V66_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;

    { Rectangle and arc still remain in the matrix as cast-only fallbacks. }
    try
        RectObj := Obj;
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V66_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V66_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V66ChildObjectIdDecoderProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
    ...
    ChildIterCount  : Integer;
    ChildIterIds    : String;
    ChildIterDecodedRoles : String;
    ChildIterHits   : Integer;
    ChildIterStatus : String;
    Key             : String;
begin
    SheetId := 'SHEET_' + IntToStr(SheetIndex);
    SheetName := ChangeFileExt(ExtractFileName(SchPath), '');
    ...
    begin
        ...
        if V66IsTarget(Oid) then
        begin
            TargetRole := V66DecisionForObjectId(Oid);
            ExpectedKind := V66ExpectedTextKindForObjectId(Oid);
            ...
            ProbeRouteB := 'activeComponentChildIterator_ACTIVE';
            ProbeRouteC := 'libRefGrouping_ACTIVE';
            ProbeRouteD := 'pinBoundsFallback_ACTIVE';
            RecommendedNext := 'keepAsAnchor';
            if Oid = 44 then
            begin
                RecommendedNext := 'inspectChildLibraryRouteResultsThenDecidePromotionOrFallback';
                ProbeRouteB := 'activeComponentChildIterator_ACTIVE:ObjectId44ContextOnly';
            end;
            if Oid = 4 then
            ...
            InterfaceMatrix := V66InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V66CompileRiskPlanForObjectId(Oid);
            ChildIterCount := 0;
            ChildIterIds := '';
            ChildIterDecodedRoles := '';
            ChildIterHits := 0;
            ChildIterStatus := 'notAttempted';
            if CurrentComp <> nil then
                V66ChildIteratorInventory(CurrentComp, ChildIterCount, ChildIterIds, ChildIterDecodedRoles, ChildIterHits, ChildIterStatus);

            if Oid = 44 then
            begin
                V66TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'activeComponentChildIteratorObjectIdOnly';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'activeComponentChildIterator_v66:' + NativeStatus
                    else
            ...
            Csv.Add(
                ...
                ProbeEscapeCell(InterfaceMatrix + ':' + ChildIterStatus) + ';' +
                IntToStr(ChildIterCount) + ';' +
                ProbeEscapeCell(ChildIterIds) + ';' +
                ProbeEscapeCell(ChildIterDecodedRoles) + ';' +
                IntToStr(ChildIterHits) + ';' +
...
procedure ExportSchV66ChildObjectIdDecoderProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v66 child ObjectId decoder probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v66 child ObjectId decoder probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v66RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;childObjectIdDecoderProbe_v66;childIteratorObjectCount_v66;childIteratorObjectIds_v66;childIteratorDecodedRoles_v66;childIteratorTargetOrCandidateHits_v66;routeResultPlan_v66;recommendedNextAction');
        Summary.Add('Altium schematic v66 SAFE child ObjectId decoder probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v66 SAFE rollback after v63 hang: keep the proven child ObjectId decoder route, add no broad cast matrix, and avoid all risky geometry accessors or COM-heavy loops.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference/Location; no broad cast matrix, no direct generic/Variant geometry property is executed.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V66ChildObjectIdDecoderProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V66_SAFE_CHILD_OBJECTID_DECODER_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[INTERPRETATION]');
        Summary.Add('ObjectId 44 / 4 rows are included and childIteratorDecodedRoles_v66 decodes recurrent child ObjectIds 37, 27, 8, 18, 22 and 5 for symbol-primitive correlation.');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v66 actively inventories and decodes Component.SchIterator_Create child ObjectIds per component cluster.');
        Summary.Add('Next step after v66: analyse CSV safely; if needed, v66 should test one ObjectId and one interface at a time with progress logging.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_child_objectid_decoder_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_child_objectid_decoder_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v66 child ObjectId decoder probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v66 child ObjectId decoder probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
{==============================================================================}
{ 3g. V66 MINI-PROBE CIBLEE OBJECTID 8 - ISch_NetLabel CAST-ONLY         }
{==============================================================================}
{ Objectif v66:
  - repartir de V65 validee, mais retirer l'appel au rapport decodeur large devenu redondant;
  - tester seulement ObjectId 8 avec la meme methode SAFE que V64/V65;
  - limiter le nombre de composants sondes;
  - logger avant/apres chaque cast pour identifier exactement le dernier test en cas de blocage;
  - ne lire aucune geometrie et ne pas acceder a des proprietes inconnues.

  Historique conserve:
  - V63 ambitieuse : matrice large de casts COM => blocage Altium, a ne pas refaire.
  - V63b : rollback safe OK, child iterator confirme.
  - V64 : ObjectId 27 caste OK en ISch_Parameter sur 1023 objets => parametres, pas corps graphique.
  - V65 : ObjectId 37 caste OK en ISch_Pin sur 375 objets => pins, pas corps graphique.

  Test actif v66:
  - ChildOid = 8 depuis Component.SchIterator_Create
  - cast-only vers ISch_NetLabel comme hypothese compile-safe;
  - aucune lecture NetLabelObj.Location/Text/Name dans cette version.
}
procedure V66MiniProbe8DumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer; var ComponentBudget : Integer);
var
    Iterator       : ISch_Iterator;
    Obj            : Variant;
    Component      : ISch_Component;
...
begin
    ...
    ComponentIndex := -1;
    Obj := Iterator.FirstSchObject;
    while (Obj <> nil) and (ComponentBudget > 0) do
    begin
        Oid := ProbeTryObjectId(Obj);
        if Oid = 26 then
        begin
            ...
            begin
                ...
                begin
                    ProbeIncCounter(Counters, 'componentChildIteratorCreateFailed');
                end
                else
                begin
                    try
                        ...
                        Guard := 0;
                        ChildObj := ChildIter.FirstSchObject;
                        while (ChildObj <> nil) and (Guard < 128) do
                        begin
                            ChildOid := ProbeTryObjectId(ChildObj);
                            if ChildOid = 8 then
                            begin
                                AppendExportLog('V66 mini-probe BEFORE ObjectId8 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex));
                                Status := 'OID8_FOUND_CAST_NOT_ATTEMPTED';
                                try
                                    NetLabelObj := ChildObj;
                                    if NetLabelObj <> nil then
                                        Status := 'OID8_ISch_NetLabel_CAST_OK_NO_PROPERTY_READ_V66'
                                    else
                                        Status := 'OID8_ISch_NetLabel_CAST_RETURNED_NIL_V66';
                                except
                                    Status := 'OID8_ISch_NetLabel_CAST_EXCEPTION_V66';
                                end;
                                AppendExportLog('V66 mini-probe AFTER ObjectId8 cast sheet=' + SheetId + ' comp=' + CompDesignator + ' childIndex=' + IntToStr(ChildIndex) + ' status=' + Status);
                                ...
                                Csv.Add(
                                    ...
                                    IntToStr(ChildIndex) + ';' +
                                    IntToStr(ChildOid) + ';' +
                                    ProbeEscapeCell(V66DecodeChildObjectId(ChildOid)) + ';' +
                                    ProbeEscapeCell(Status) + ';' +
                                    'ISch_NetLabel;castOnly;noGeometryNoTextNoLocation;v66MiniProbe'
                                );
                            end;
                            Inc(ChildIndex);
                            Inc(Guard);
                        ...
                        if Guard >= 128 then
                            ProbeIncCounter(Counters, 'componentChildIteratorTruncated128');
                    finally
                        Component.SchIterator_Destroy(ChildIter);
                    end;
                end;
...
procedure ExportSchV66MiniProbe8Report;
var
    Workspace       : IWorkSpace;
    Project         : IProject;
    Document        : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v66 mini-probe ObjectId8 skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v66 mini-probe ObjectId8 skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;flatObjectIndex;componentIndex;componentDesignatorText;componentCommentText;componentLibRef;childIndex;childObjectId;childDecodedRole;castStatus;interfaceTest;accessMode;disabledReads;probeVersion');
        Summary.Add('Altium schematic v66 mini-probe ObjectId 8 ISch_NetLabel cast-only');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v66 targeted acceleration after v63 hang: test only child ObjectId 8 with ISch_NetLabel cast-only, limited component budget, before/after debug logging.');
        Summary.Add('SafeProperties=ObjectId, iterator order, component Designator/Comment/LibReference. ObjectId 8 is cast-only to ISch_NetLabel; no geometry/text/location property is read.');
        Summary.Add('Limits=first 120 component roots across project; max 128 child objects per component.');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V66MiniProbe8DumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount, ComponentBudget);
                    Inc(SheetCount);
                end;
            end;
            if ComponentBudget <= 0 then Break;
        end;

        Summary.Add('SheetCountVisited=' + IntToStr(SheetCount));
        Summary.Add('RemainingComponentBudget=' + IntToStr(ComponentBudget));
        Summary.Add('[V66_MINIPROBE_OBJECTID8_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        Summary.Add('[NEXT]');
        Summary.Add('If ObjectId 8 casts OK to ISch_NetLabel, v67 can decide whether it is useful metadata or move to ObjectId 18/22/5; no geometry accessor yet.');
        Summary.Add('If ObjectId 8 is not useful, target ObjectId 18 with the same mini-probe pattern; do not re-enable broad cast matrices.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_miniprobe_oid8_netlabel_cast.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_miniprobe_oid8_netlabel_cast_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v66 mini-probe ObjectId8 CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v66 mini-probe ObjectId8 summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_child_objectid_decoder_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_child_objectid_decoder_probe_summary.txt'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_miniprobe_oid8_netlabel_cast.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v66_miniprobe_oid8_netlabel_cast_summary.txt'));
