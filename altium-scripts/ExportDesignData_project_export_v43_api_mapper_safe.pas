{==============================================================================}
{ Altium Diff Studio - Exporter Script v42                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v43';
    SCHEMA_VERSION = 'ads-json-v43';
    SCHEMATIC_SCHEMA_VERSION = 'v43';
...
begin
    ...
    JsonList.Add('      "graphicsNotes": "V43 keeps the stable logical export and reserves typed SCH containers for native graphic/symbol extraction. Symbol bodies remain pin-bounds fallbacks until each target Altium object API is validated.",');
    ...
    JsonList.Add('        "notes": "V43 keeps the native eJunction pass, but adds an explicit fallback contract because this Altium version returned zero native junction objects on the AGMA project."');
    JsonList.Add('      },');
...
begin
    ...
    try
        ...
        JsonList.Add('    "notes": "V43 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes arc-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusHierarchyAnchorsAndApiMapperV43",');
        JsonList.Add('  "compatibilityMode": "v43-safe-sch-hierarchy-anchors-plus-api-mapper",');
        ...
        JsonList.Add('    "edges": [],');
        JsonList.Add('    "notes": "V43 keeps a global hierarchy contract without reading new risky Altium properties. Existing per-sheet arrays plus sheets[].hierarchyAnchors contain the raw data needed by the viewer to infer inter-sheet connectivity. Native sheet symbol and sheet entry names remain unavailable in this target API and are therefore not used in the script."');
        JsonList.Add('  }');
        JsonList.Add('}');
...
{==============================================================================}
{ 3b. SCHEMATIC OBJECT PROBE - v43 DIAGNOSTIC/API MAPPER ADD-ON                           }
{==============================================================================}
...
begin
    // V43: generic Obj.Location is not exposed in this Altium API.
    Result := '';
end;
...
begin
    // V43: keep probe compile-safe. Generic Obj.Text is not exposed in this Altium API.
    Result := '';
end;

function ProbeTryName(Obj : Variant) : String;
begin
    // V43: keep probe compile-safe. Generic Obj.Name is not exposed reliably.
    Result := '';
end;

function ProbeTryDesignator(Obj : Variant) : String;
begin
    // V43: avoid generic Obj.Designator.Text in the all-object probe.
    Result := '';
end;

function ProbeTryComment(Obj : Variant) : String;
begin
    // V43: avoid generic Obj.Comment.Text in the all-object probe.
    Result := '';
end;

function ProbeTryLibRef(Obj : Variant) : String;
begin
    // V43: avoid generic Obj.LibReference in the all-object probe.
    Result := '';
end;

function ProbeTryFileName(Obj : Variant) : String;
begin
    // V43: avoid generic Obj.FileName in the all-object probe.
    Result := '';
end;
...
procedure ProbeDumpSchDocument(Raw : TStringList; Targeted : TStringList; Summary : TStringList; GlobalCounters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator      : ISch_Iterator;
    Obj           : Variant;
...
begin
    ...
    try
        ...
        ProbeDumpKnownFilteredObjectsForSheet(Targeted, Summary, SchDoc, SheetId, SheetName);
    finally
        SheetCounters.Free;
    end;
end;
...
procedure ProbeDumpKnownFilteredObjectsForSheet(Targeted : TStringList; Summary : TStringList; SchDoc : ISch_Document; const SheetId : String; const SheetName : String);
var
    ItRaw            : TStringList;
    Targeted       : TStringList;
    Summary        : TStringList;
...
begin
    ...
    try
        ...
        Targeted.Add('sheetId;sheetName;objectType;objectIndex;x;y;textA;textB;propertyStatus');
        ...
        Summary.Add('Purpose=discover raw ObjectId values and targeted safe property availability returned by SchIterator_Create');
        Summary.Add('');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    ProbeDumpSchDocument(Raw, Targeted, Summary, GlobalCounters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end
                else
