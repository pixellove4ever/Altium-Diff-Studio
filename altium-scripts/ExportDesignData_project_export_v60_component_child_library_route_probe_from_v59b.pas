{==============================================================================}
{ Altium Diff Studio - Exporter Script v60                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v60';
    SCHEMA_VERSION = 'ads-json-v60';
    SCHEMATIC_SCHEMA_VERSION = 'v60';
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderAndComponentChildLibraryRouteProbeV60",');
    JsonList.Add('      "graphicsNotes": "V60 keeps validated textRender and switches native symbol discovery away from direct ObjectId 44 geometry access toward a compile-safe component child/library-symbol route. Symbol bodies still use pin-bounds fallback in the JSON until a safe child/library primitive source is validated.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v60",');
    JsonList.Add('        "focus": "viewer-ready component text rendering plus compile-safe component child/library-symbol route probe for native symbol discovery",');
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v60 COMPONENT CHILD/LIBRARY ROUTE            }
{======================================================================================
 ...
 - v60  : pivot stratégique: ne plus sonder la géométrie de ObjectId 44 directement; cartographier
          le chemin composant -> cluster -> LibRef -> futur child/library-symbol iterator.
 ...
 - on prépare la route composant/enfant/librairie pour trouver les primitives natives sans casser la compilation.
======================================================================================}
...
function V60DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V60ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V60IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V60InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
...
function V60CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V60TryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'V60_COMPONENT_CHILD_LIBRARY_ROUTE_NOT_READ_YET';

    { V60 AMBITIOUS STRATEGY / HISTORY TRACE
      -------------------------------------------------------------------------
    ...
    try
        ...
        begin
            Result := True;
            LineVar := Obj;
            Status := 'ISch_Line_CAST_OK_V60_DIRECT_GEOMETRY_DISABLED_ROUTE_PIVOT_TO_COMPONENT_CHILD_LIBRARY';
            ...
            Status := Status + '|V60_CHILD_LIBRARY_ROUTE_PLANNED_NOT_EXECUTED_COMPILE_SAFE';
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V60_CHILD_LIBRARY_ROUTE_STILL_AVAILABLE';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V60_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V60_LOWER_PRIORITY';
            Result := True;
            Exit;
        end;
    except
...
procedure V60ComponentChildLibraryRouteProbeDumpSchDocument(Csv : TStringList; Counters : TStringList; SchDoc : ISch_Document; const SchPath : String; SheetIndex : Integer);
var
    Iterator        : ISch_Iterator;
    Obj             : Variant;
    Component       : ISch_Component;
...
begin
    ...
    begin
        ...
        if V60IsTarget(Oid) then
        begin
            TargetRole := V60DecisionForObjectId(Oid);
            ExpectedKind := V60ExpectedTextKindForObjectId(Oid);
            ...
            ProbeRouteB := 'componentChildLibraryRoute_ACTIVE';
            ...
            begin
                ...
                ProbeRouteB := 'componentChildLibraryRoute_ACTIVE:ObjectId44ContextOnly';
            end;
            if Oid = 4 then
            ...
            InterfaceMatrix := V60InterfaceMatrixForObjectId(Oid);
            CompileRiskPlan := V60CompileRiskPlanForObjectId(Oid);

            if Oid = 44 then
            begin
                V60TryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
                NativeSource := 'componentChildOrLibrarySymbolRoute';
            end;
            if CurrentComp <> nil then
            begin
                if Oid = 46 then
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if Oid = 44 then
                        GeometrySource := 'componentChildLibraryRoute_v60:' + NativeStatus
                    else
...
procedure ExportSchV60ComponentChildLibraryRouteProbeReport;
var
    Workspace   : IWorkSpace;
    Project     : IProject;
    Document    : IDocument;
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v60 component child/library-symbol route probe skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v60 component child/library-symbol route probe skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v60RoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;componentChildLibraryRouteProbe_v60;routeResultPlan_v60;recommendedNextAction');
        Summary.Add('Altium schematic v60 component child/library-symbol route probe embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v60 compile-safe route change after v59b: stop probing unknown direct/Variant geometry properties and map ObjectId 44 through component cluster, LibRef grouping, and planned component-child/library-symbol access without touching risky accessors.');
        ...
        begin
            ...
            begin
                ...
                if SchDoc <> nil then
                begin
                    V60ComponentChildLibraryRouteProbeDumpSchDocument(Csv, Counters, SchDoc, Document.DM_FullPath, SheetCount);
                    Inc(SheetCount);
                end;
            end;
        end;
        ...
        Summary.Add('SheetCount=' + IntToStr(SheetCount));
        Summary.Add('[V60_COMPONENT_CHILD_LIBRARY_ROUTE_PROBE_COUNTS]');
        Counters.Sort;
        for i := 0 to Counters.Count - 1 do
            Summary.Add('  ' + Counters.Strings[i]);
        Summary.Add('');
        ...
        Summary.Add('ObjectId 44 direct and Variant geometry remain disabled; v60 documents the pivot to component child/library-symbol route.');
        Summary.Add('Next step after v60: if child/library route exposes a safe primitive iterator, promote it to JSON symbolGraphics; otherwise improve viewer fallback from pins/bounds and proceed to BOM/SCH/PCB comparator.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v60_component_child_library_route_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v60_component_child_library_route_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v60 component child/library-symbol route probe CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v60 component child/library-symbol route probe summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v60_component_child_library_route_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v60_component_child_library_route_probe_summary.txt'));
...
begin
    ...
    begin
        ExportAllProjectSchToJson;
        ExportSchV60ComponentChildLibraryRouteProbeReport;
    end
