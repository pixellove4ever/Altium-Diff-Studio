...
const
    SCRIPT_VERSION = 'v42';
    SCHEMA_VERSION = 'ads-json-v42';
    SCHEMATIC_SCHEMA_VERSION = 'v42';
...
begin
    Result := '';
    try
        Result := IntToStr(Obj.Location.X);
    except
        Result := '';
    end;
end;
...
begin
    Result := '';
    try
        Result := IntToStr(Obj.Location.Y);
    except
        Result := '';
    end;
end;
...
begin
    // V42b: keep probe compile-safe. Generic Obj.Text is not exposed in this Altium API.
    Result := '';
end;

function ProbeTryName(Obj : Variant) : String;
begin
    // V42b: keep probe compile-safe. Generic Obj.Name is not exposed reliably.
    Result := '';
end;

function ProbeTryDesignator(Obj : Variant) : String;
begin
    // V42b: avoid generic Obj.Designator.Text in the all-object probe.
    Result := '';
end;

function ProbeTryComment(Obj : Variant) : String;
begin
    // V42b: avoid generic Obj.Comment.Text in the all-object probe.
    Result := '';
end;

function ProbeTryLibRef(Obj : Variant) : String;
begin
    // V42b: avoid generic Obj.LibReference in the all-object probe.
    Result := '';
end;

function ProbeTryFileName(Obj : Variant) : String;
begin
    // V42b: avoid generic Obj.FileName in the all-object probe.
    Result := '';
end;
