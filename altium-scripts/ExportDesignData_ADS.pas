{==============================================================================}
{ Altium Diff Studio - Canonical Exporter                                                       }
{                                                                              }
...
begin
    ...
    Value := '';
    SerecthKey := Key + '=';
    P := Pos(SerecthKey, Parameters);
    if P = 0 then Exit;

    P := P + Length(SerecthKey);
    E := P;
...
function AngleInRectSweep(TestAngle : Double; StartAngle : Double; EndAngle : Double) : Boolean;
var
    T : Double;
    S : Double;
    E : Double;
begin
    ...
    { Altium keepout rects exported here are rendered as the counter-clockwise sweep
      from StartAngle to EndAngle, with wrap handled explicitly. If Start and End
      are identical, treat it as a full circle to avoid losing real circular rects. }
...
procedure IncludeRectAnglePoint(var HasBounds : Boolean; var MinX : Double; var MinY : Double; var MaxX : Double; var MaxY : Double; CX : Double; CY : Double; R : Double; A : Double);
var
    Rad : Double;
begin
...
procedure IncludeRectSweepBounds(var HasBounds : Boolean; var MinX : Double; var MinY : Double; var MaxX : Double; var MaxY : Double; CX : Double; CY : Double; R : Double; StartAngle : Double; EndAngle : Double);
begin
    { Always include rect endpoints. Then include the cardinal extrema only if they
      lie on the actual swept rect. This prevents a small rect from expanding the
      viewer bounds to the complete circle. }
    IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, StartAngle);
    IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, EndAngle);

    if AngleInRectSweep(0, StartAngle, EndAngle) then
        IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, 0);
    if AngleInRectSweep(90, StartAngle, EndAngle) then
        IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, 90);
    if AngleInRectSweep(180, StartAngle, EndAngle) then
        IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, 180);
    if AngleInRectSweep(270, StartAngle, EndAngle) then
        IncludeRectAnglePoint(HasBounds, MinX, MinY, MaxX, MaxY, CX, CY, R, 270);
end;
...
begin
    ...
    { V31 outline strategy:
      ...
      3) Export KeepOutLayer tracks/rects in the same spirit as InteractiveHTMLBOM.
      ...
      5) V31 fixes rect bounds: rects contribute only their real swept start/end/cardinal
    ...
    { V28: InteractiveHTMLBOM-compatible outline extraction.
      InteractiveHTMLBOM reads eTrackObject + eRectObject from eKeepOutLayer to
    ...
    { V70b compile fix:
      eRectObject is not declared in this Altium PascalScript environment.
      The old PCB KeepOut rectangle/arc outline route is disabled to keep this SCH probe compiling.
      Known-good PCB outline path remains: BoardOutline.BoundingRectangle + KeepOut eTrackObject edges. }
    JsonList.Add('  ],');
    JsonList.Add('  "boardOutlineEdgesSource": "keepOutLayerTracksOnly_eRectObjectDisabled_V71",');
    JsonList.Add('  "boardOutlineEdgesCount": ' + IntToStr(OutlineEdgeCount) + ',');
    JsonList.Add('  "boardOutlineClassification": {');
    JsonList.Add('    "method": "widthHeuristic_tracksOnly_V71",');
    ...
    JsonList.Add('    "rectCandidateCount": ' + IntToStr(RectCandidateCount) + ',');
    JsonList.Add('    "notes": "V33e tags each keepout edge and exports rect-aware viewer-ready render bounds. width <= 1.1mm is treated as boardOutlineCandidate; wider edges are kept as localKeepoutOrCutoutCandidate. PCB rect/arc keepout extraction disabled because eRectObject is undeclared in this API; tracks remain active."');
    JsonList.Add('  },');
...
begin
    ...
    JsonList.Add('    "scriptName": "ExportDesignData_ADS.pas",');
...
var
    ...
    OffSheetObj : Variant;
    SheetSymbolObj : ISch_SheetSymbol;
    SheetEntryObj : ISch_SheetEntry;
    FileName    : String;
    CompCount   : Integer;
    WireCount   : Integer;
    NetCount    : Integer;
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusTextRenderStableFallbackV71",');
    JsonList.Add('      "graphicsNotes": "V71 CLEAN/STABLE keeps validated textRender, pins, net labels, hierarchy anchors and pin-bounds symbol fallback. COM geometry probes are stopped: V68/V69b/V70b did not expose usable native symbol-body coordinates.",');
    JsonList.Add('      "schemaUpgrade": {');
    JsonList.Add('        "version": "v71",');
    JsonList.Add('        "focus": "stable fallback after COM geometry probes: no active symbol-body geometry probe",');
    ...
    Begin
        ...
        JsonList.Add('          "uniqueId": "' + EscapeStr(Component.UniqueId) + '",');
        JsonList.Add('          "sourceLibraryName": "' + EscapeStr(Component.SourceLibraryName) + '",');
        ...
        begin
            if (Parameter.Name <> '') and
               (Parameter.Name <> 'Designator') and
               (Parameter.Name <> 'Comment') and
               (not JsonKeyAlreadyWritten(ParamKeys, Parameter.Name)) then
            begin
        ...
        begin
            ...
            JsonList.Add('              "uniqueId": "' + EscapeStr(Pin.UniqueId) + '",');
            JsonList.Add('              "description": "' + EscapeStr(Pin.Description) + '",');
            ...
            JsonList.Add('              "ownerPartId": ' + IntToStr(Pin.OwnerPartId) + ',');
            JsonList.Add('              "ownerPartDisplayMode": ' + IntToStr(Pin.OwnerPartDisplayMode) + ',');
            JsonList.Add('              "length": ' + IntToStr(Pin.PinLength) + ',');
            JsonList.Add('              "electricalType": ' + IntToStr(Pin.Electrical) + ',');
            JsonList.Add('              "hidden": ' + JsonBool(Pin.IsHidden) + ',');
            JsonList.Add('              "hiddenNetName": "' + EscapeStr(Pin.HiddenNetName) + '",');
            JsonList.Add('              "showName": ' + JsonBool(Pin.ShowName) + ',');
            JsonList.Add('              "showDesignator": ' + JsonBool(Pin.ShowDesignator));
            JsonList.Add('            }');
    ...
    begin
        ...
        JsonList.Add('          "orientation": ' + IntToStr(NetLabel.Orientation) + ',');
        JsonList.Add('          "justification": ' + IntToStr(NetLabel.Justification) + ',');
        JsonList.Add('          "mirrored": ' + JsonBool(NetLabel.IsMirrored));
        JsonList.Add('        }');
        Inc(NetCount);
    ...
    begin
        ...
        JsonList.Add('          "type": "text",');
        JsonList.Add('          "text": "' + EscapeStr(LabelObj.Text) + '",');
        JsonList.Add('          "displayText": "' + EscapeStr(LabelObj.DisplayString) + '",');
        JsonList.Add('          "x": ' + IntToStr(LabelObj.Location.X) + ',');
        JsonList.Add('          "y": ' + IntToStr(LabelObj.Location.Y) + ',');
        JsonList.Add('          "orientation": ' + IntToStr(LabelObj.Orientation) + ',');
        JsonList.Add('          "justification": ' + IntToStr(LabelObj.Justification) + ',');
        JsonList.Add('          "mirrored": ' + JsonBool(LabelObj.IsMirrored) + ',');
        JsonList.Add('          "fontId": ' + IntToStr(LabelObj.FontId) + ',');
        JsonList.Add('          "color": ' + IntToStr(LabelObj.Color));
        JsonList.Add('        }');
        Inc(AnnotationCount);
    ...
    TextFrameObj := Iterator.FirstSchObject;
    while TextFrameObj <> nil do
    begin
        if AnnotationCount > 0 then JsonList.Add('        ,');
        JsonList.Add('        {');
        JsonList.Add('          "id": "SHEET_' + IntToStr(SheetIndex) + '_ANNOTATION_' + IntToStr(AnnotationCount) + '",');
        JsonList.Add('          "type": "textFrame",');
        JsonList.Add('          "text": "' + EscapeStr(TextFrameObj.Text) + '",');
        JsonList.Add('          "x": ' + IntToStr(TextFrameObj.Location.X) + ',');
        JsonList.Add('          "y": ' + IntToStr(TextFrameObj.Location.Y) + ',');
        JsonList.Add('          "bounds": { "x1": ' + IntToStr(TextFrameObj.Location.X) + ', "y1": ' + IntToStr(TextFrameObj.Location.Y) + ', "x2": ' + IntToStr(TextFrameObj.Corner.X) + ', "y2": ' + IntToStr(TextFrameObj.Corner.Y) + ' },');
        JsonList.Add('          "fontId": ' + IntToStr(TextFrameObj.FontId) + ',');
        JsonList.Add('          "color": ' + IntToStr(TextFrameObj.TextColor) + ',');
        JsonList.Add('          "showBorder": ' + JsonBool(TextFrameObj.ShowBorder) + ',');
        JsonList.Add('          "wordWrap": ' + JsonBool(TextFrameObj.WordWrap) + ',');
        JsonList.Add('          "clipToRect": ' + JsonBool(TextFrameObj.ClipToRect) + ',');
        JsonList.Add('          "alignment": ' + IntToStr(TextFrameObj.Alignment));
        JsonList.Add('        }');
        Inc(AnnotationCount);
        TextFrameObj := Iterator.NextSchObject;
    end;
    SchDoc.SchIterator_Destroy(Iterator);
    JsonList.Add('      ],');
    ...
    begin
        ...
        JsonList.Add('          "uniqueId": "' + EscapeStr(SheetSymbolObj.UniqueId) + '",');
        if SheetSymbolObj.SheetName <> nil then
            JsonList.Add('          "name": "' + EscapeStr(SheetSymbolObj.SheetName.Text) + '",')
        else
            JsonList.Add('          "name": "",');
        if SheetSymbolObj.SheetFileName <> nil then
    ...
    begin
        ...
        JsonList.Add('          "name": "' + EscapeStr(SheetEntryObj.Name) + '",');
        JsonList.Add('          "metadataStatus": "nativeSheetEntryNameV2",');
        if SheetEntryObj.OwnerSheetSymbol <> nil then
            JsonList.Add('          "ownerSheetSymbolUniqueId": "' + EscapeStr(SheetEntryObj.OwnerSheetSymbol.UniqueId) + '",')
        else
            JsonList.Add('          "ownerSheetSymbolUniqueId": "",');
    ...
    JsonList.Add('      "hierrecthyAnchors": {');
...
begin
    ...
    try
        ...
        begin
            ...
            JsonList.Add('      "bounds": {');
            JsonList.Add('        "x1": ' + JsonFloat(CoordToMMs(Component.BoundingRectangleNoNameCommentForSignals.Left)) + ',');
            JsonList.Add('        "y1": ' + JsonFloat(CoordToMMs(Component.BoundingRectangleNoNameCommentForSignals.Bottom)) + ',');
            JsonList.Add('        "x2": ' + JsonFloat(CoordToMMs(Component.BoundingRectangleNoNameCommentForSignals.Right)) + ',');
            JsonList.Add('        "y2": ' + JsonFloat(CoordToMMs(Component.BoundingRectangleNoNameCommentForSignals.Top)));
            JsonList.Add('      }');
            JsonList.Add('    }');
            Inc(Count);
            Component := Iterator.NextPCBObject;
        ...
        { Arcs on copper, overlay, mechanical and keepout layers. The same native
          IPCB_Arc properties are already used by the proven InteractiveHTMLBOM
          exporter and avoid schematic COM geometry probes entirely. }
        JsonList.Add('  "arcs": [');
        Count := 0;
        Iterator := PcbBoard.BoardIterator_Create;
        ...
        begin
            if Count > 0 then JsonList.Add('    ,');
            JsonList.Add('    {');
            JsonList.Add('      "id": "ARC_' + IntToStr(Count) + '",');
            JsonList.Add('      "layer": "' + Layer2String(Arc.Layer) + '",');
            ...
            if Arc.Net <> nil then
                JsonList.Add('      "net": "' + EscapeStr(Arc.Net.Name) + '"')
            else
                JsonList.Add('      "net": ""');
            JsonList.Add('    }');
            Inc(Count);
        ...
        begin
            ...
            case Pad.TopShape of
                eRounded:
                    if Pad.TopXSize = Pad.TopYSize then
                        JsonList.Add('      "shape": "round",')
                    else
                        JsonList.Add('      "shape": "oval",');
                eRectangular: JsonList.Add('      "shape": "rectangular",');
                eOctagonal: JsonList.Add('      "shape": "octagonal",');
                9: JsonList.Add('      "shape": "rounded-rectangular",');
                else JsonList.Add('      "shape": "rectangular",');
            end;
            if Pad.TopShape = 9 then
                JsonList.Add('      "cornerRadius": ' + JsonFloat(CoordToMMs(Pad.CornerRadius(Pad.Layer))) + ',');
            JsonList.Add('      "holeSize": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
            case Pad.HoleType of
                0:
                    begin
                        JsonList.Add('      "holeSizeX": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
                        JsonList.Add('      "holeSizeY": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
                        JsonList.Add('      "holeShape": "round",');
                    end;
                1:
                    begin
                        JsonList.Add('      "holeSizeX": ' + JsonFloat(CoordToMMs(Pad.HoleWidth)) + ',');
                        JsonList.Add('      "holeSizeY": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
                        JsonList.Add('      "holeShape": "rectangular",');
                    end;
                2:
                    begin
                        JsonList.Add('      "holeSizeX": ' + JsonFloat(CoordToMMs(Pad.HoleWidth)) + ',');
                        JsonList.Add('      "holeSizeY": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
                        JsonList.Add('      "holeShape": "oblong",');
                    end;
                else
                    begin
                        JsonList.Add('      "holeSizeX": ' + JsonFloat(CoordToMMs(Pad.HoleSize)) + ',');
            ...
            JsonList.Add('      "rotation": ' + JsonFloat(Pad.Rotation) + ',');
            if Pad.Name = '1' then
                JsonList.Add('      "pin1": true,')
            else
                JsonList.Add('      "pin1": false,');
        ...
        { Visible PCB text, including overlay designators/comments and mechanical
          annotations. Hidden text is intentionally omitted. }
        JsonList.Add('  "texts": [');
        Count := 0;
        Iterator := PcbBoard.BoardIterator_Create;
        ...
        begin
            if not TextObj.IsHidden then
            begin
                if Count > 0 then JsonList.Add('    ,');
                JsonList.Add('    {');
                JsonList.Add('      "id": "TEXT_' + IntToStr(Count) + '",');
                JsonList.Add('      "layer": "' + Layer2String(TextObj.Layer) + '",');
                JsonList.Add('      "text": "' + EscapeStr(TextObj.Text) + '",');
                if TextObj.IsDesignator then
                    JsonList.Add('      "role": "designator",')
                else if TextObj.IsComment then
                    JsonList.Add('      "role": "comment",')
                else
                    JsonList.Add('      "role": "text",');
                JsonList.Add('      "x": ' + JsonFloat(CoordToMMs((TextObj.BoundingRectangleForSelection.Left + TextObj.BoundingRectangleForSelection.Right) / 2)) + ',');
                JsonList.Add('      "y": ' + JsonFloat(CoordToMMs((TextObj.BoundingRectangleForSelection.Bottom + TextObj.BoundingRectangleForSelection.Top) / 2)) + ',');
                JsonList.Add('      "height": ' + JsonFloat(CoordToMMs(TextObj.Size)) + ',');
                JsonList.Add('      "rotation": ' + JsonFloat(TextObj.Rotation));
                JsonList.Add('    }');
                Inc(Count);
            end;
        ...
        { Copper regions and poured polygon child regions. Each rendered contour
          is flattened to the viewer's polygon schema while preserving layer/net. }
        ...
        while Region <> nil do
        begin
            if (Region.Kind() = eRegionKind_Copper) and (not Region.InPolygon()) then
            begin
                ...
                begin
                    if Count > 0 then JsonList.Add('    ,');
                    JsonList.Add('    {');
                    JsonList.Add('      "id": "REGION_' + IntToStr(Count) + '",');
                    JsonList.Add('      "layer": "' + Layer2String(Region.Layer) + '",');
                    if Region.Net <> nil then
                        JsonList.Add('      "net": "' + EscapeStr(Region.Net.Name) + '",')
                    else
                        JsonList.Add('      "net": "",');
                    JsonList.Add('      "vertices": [');
                    for VertexIndex := 0 to Contour.Count - 1 do
                    begin
                        if VertexIndex > 0 then JsonList.Add('        ,');
                        JsonList.Add('        { "x": ' + JsonFloat(CoordToMMs(Contour.GetState_PointX(VertexIndex))) +
                                     ', "y": ' + JsonFloat(CoordToMMs(Contour.GetState_PointY(VertexIndex))) + ' }');
                    end;
                    JsonList.Add('      ]');
                    JsonList.Add('    }');
                    Inc(Count);
        ...
        Iterator.AddFilter_LayerSet(AllLayers);
        Iterator.AddFilter_Method(eProcessAll);
        PolyPrim := Iterator.FirstPCBObject;
        while PolyPrim <> nil do
        begin
            GroupIterator := PolyPrim.GroupIterator_Create;
            ChildPrim := GroupIterator.FirstPCBObject;
            while ChildPrim <> nil do
            begin
                if ChildPrim.ObjectId = eRegionObject then
                begin
                    Contour := ChildPrim.GetMainContour();
                    if (Contour <> nil) and (Contour.Count >= 3) then
                    begin
                        if Count > 0 then JsonList.Add('    ,');
                        JsonList.Add('    {');
                        JsonList.Add('      "id": "POUR_' + IntToStr(Count) + '",');
                        JsonList.Add('      "layer": "' + Layer2String(PolyPrim.Layer) + '",');
                        if PolyPrim.Net <> nil then
                            JsonList.Add('      "net": "' + EscapeStr(PolyPrim.Net.Name) + '",')
                        else
                            JsonList.Add('      "net": "",');
                        JsonList.Add('      "vertices": [');
                        for VertexIndex := 0 to Contour.Count - 1 do
                ...
                ChildPrim := GroupIterator.NextPCBObject;
            end;
            PolyPrim.GroupIterator_Destroy(GroupIterator);
            PolyPrim := Iterator.NextPCBObject;
        end;
        PcbBoard.BoardIterator_Destroy(Iterator);
        JsonList.Add('  ],');
        ...
        JsonList.Add('    "arcs": "native",');
        JsonList.Add('    "texts": "nativeVisibleObjects",');
        JsonList.Add('    "polygons": "nativeCopperRegionsAndPouredChildRegions",');
        JsonList.Add('    "splitPlanes": "reserved",');
        JsonList.Add('    "regions": "flattenedIntoPolygons",');
        JsonList.Add('    "notes": "V42 keeps the validated native Board.BoardOutline.BoundingRectangle path, exports KeepOut outline edges, and computes rect-aware viewer-ready bounds so the display does not fall back to all-object extents. The viewer should prefer boardOutlineEdges where edgeClass=boardOutlineCandidate for the visible outline, while retaining localKeepoutOrCutoutCandidate for drill/mounting/cutout context."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "exportGoal": "universalLogicalPlusHierrecthyAnchorsAndObjectProbeV42",');
        JsonList.Add('  "compatibilityMode": "v42-safe-sch-hierrecthy-anchors-plus-object-probe",');
        ...
        JsonList.Add('  ],');
        JsonList.Add('  "hierrecthy": {');
        JsonList.Add('    "version": "v42",');
        JsonList.Add('    "status": "viewerResolvableHierrecthyAnchors",');
        JsonList.Add('    "source": "schematicSheetSymbolsSheetEntriesPortsAndProjectDocuments",');
        JsonList.Add('    "safeMode": true,');
        JsonList.Add('    "anchorExport": "perSheetHierrecthyAnchors",');
        ...
        JsonList.Add('      "Use each parent sheet sheetSymbols and sheetEntries coordinates as hierrecthy connection anchors.",');
        ...
        JsonList.Add('    ],');
        JsonList.Add('    "anchors": "see sheets[].hierrecthyAnchors",');
        JsonList.Add('    "edges": [],');
        JsonList.Add('    "notes": "V42 keeps a global hierrecthy contract without reading new risky Altium properties. Existing per-sheet arrays plus sheets[].hierrecthyAnchors contain the raw data needed by the viewer to infer inter-sheet connectivity. Native sheet symbol and sheet entry names remain unavailable in this target API and are therefore not used in the script."');
        JsonList.Add('  }');
        JsonList.Add('}');
...
begin
    ...
    begin
        Result := 'probeAsGraphicalTextLineRectRectangleImageUsingObjectIdFilter';
        Exit;
    end;
...
{ Purpose:
  ...
  designators/comments, symbol graphics and sheet hierrecthy objects. }
...
{ Purpose:
  ...
  components and hierrecthy objects. V45 keeps the same safe rule: ObjectId only.
...
{==============================================================================}
{ 3f. TARGETED NATIVE SYMBOL PRIMITIVE PROBE - v67 ACTIVE COMPONENT CHILD ITERATOR            }
{======================================================================================
 ...
 - v57  : ISch_Line/ISch_Rectangle/ISch_Rectangle avec .Location/.Corner/.Radius -> .Location
 ...
 - v58  : test ciblé ISch_Line.GetState_Vertex(0/1) -> ne compile pas: undeclared GetState_Vertex.
 - v59  : tentative Variant StartLocation/EndLocation -> ne compile pas: undeclared StartLocation.
 - v59b : rollback compile-safe, confirme que les accès Variant dot ne contournent pas le compilateur.
 - v67  : reprend la route V62 sûre après le blocage constaté de la V63 ambitieuse.
          Aucun cast-matrix large n est exécuté; uniquement inventaire enfants ObjectId-only.

 Decision v67 SAFE after v63 hang:
 - on conserve la preuve ISch_Line cast-only sur ObjectId 44,
 - on interdit les accès directs et Variant aux propriétés géométriques inconnues,
 - on active un inventaire enfant objectId-only via Component.SchIterator_Create pour accélérer sans bloquer Altium.
======================================================================================}
...
function V67DecisionForObjectId(ObjectId : Integer) : String;
begin
    Result := 'ignore';
...
function V67ExpectedTextKindForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
...
function V67IsTarget(ObjectId : Integer) : Boolean;
begin
    Result := False;
...
function V67DecodeChildObjectId(ObjectId : Integer) : String;
begin
    Result := 'unknownOrNonTargetChildObject';
    if ObjectId = 4 then Result := 'knownTarget_4_componentLocalPrimitiveOrPinRelatedAnchor'
    else if ObjectId = 5 then Result := 'newChildCandidate_5_possiblePrimitiveOrParameter_needsCorrelation'
    else if ObjectId = 8 then Result := 'newChildCandidate_8_possiblePrimitiveOrParameter_needsCorrelation'
    else if ObjectId = 18 then Result := 'newChildCandidate_18_possiblePrimitiveOrSymbolPart_needsCorrelation'
    else if ObjectId = 22 then Result := 'newChildCandidate_22_possiblePrimitiveOrSymbolPart_needsCorrelation'
    else if ObjectId = 25 then Result := 'knownTarget_25_primaryDisplayTextCandidate'
    else if ObjectId = 27 then Result := 'newChildCandidate_27_possibleSymbolPrimitiveOrPinMetadata_needsCorrelation'
    else if ObjectId = 37 then Result := 'newChildCandidate_37_recurrentChildBeforePrimitives_candidateSymbolInfrastructure'
    else if ObjectId = 44 then Result := 'knownTarget_44_ISchLineCastOK_nativeSymbolGraphicCandidate'
    else if ObjectId = 45 then Result := 'knownTarget_45_designatorDisplayObject'
    else if ObjectId = 46 then Result := 'knownTarget_46_commentDisplayObject';
end;

function V67AppendDecodedChildIdSummary(const ExistingSummary : String; ObjectId : Integer) : String;
var
    Entry : String;
begin
    Entry := IntToStr(ObjectId) + '=' + V67DecodeChildObjectId(ObjectId);
    if ExistingSummary = '' then
        Result := Entry
    else
        Result := ExistingSummary + ',' + Entry;
end;

function V67InterfaceMatrixForObjectId(ObjectId : Integer) : String;
begin
    Result := 'none';
    if ObjectId = 44 then
        Result := 'batchA:ISch_Line/ISch_Rectangle/ISch_Rectangle;batchB:ISch_Ellipse/ISch_Polygon/ISch_Bezier;batchC:componentChildIteratorOrLibrarySymbolRoute'
...
function V67CompileRiskPlanForObjectId(ObjectId : Integer) : String;
begin
...
function V67ChildIteratorInventory(Component : ISch_Component; var ChildCount : Integer; var ChildIds : String; var ChildDecodedRoles : String; var ChildTargetHits : Integer; var RouteStatus : String) : Boolean;
var
    ChildIter : ISch_Iterator;
    ChildObj  : Variant;
    ChildOid  : Integer;
    Guard     : Integer;
begin
    ...
    ChildDecodedRoles := '';
    ChildTargetHits := 0;
    RouteStatus := 'NO_COMPONENT';

    if Component = nil then Exit;

    RouteStatus := 'CHILD_ITERATOR_CREATE_FAILED';
    ChildIter := Component.SchIterator_Create;
    if ChildIter = nil then Exit;

    try
        Result := True;
        RouteStatus := 'CHILD_ITERATOR_ACTIVE_OBJECTID_ONLY';
        Guard := 0;
        ChildObj := ChildIter.FirstSchObject;
        while (ChildObj <> nil) and (Guard < 256) do
        begin
            ChildOid := ProbeTryObjectId(ChildObj);
            if ChildIds = '' then
                ChildIds := IntToStr(ChildOid)
            else
                ChildIds := ChildIds + ',' + IntToStr(ChildOid);
            ChildDecodedRoles := V67AppendDecodedChildIdSummary(ChildDecodedRoles, ChildOid);
            if V67IsTarget(ChildOid) or (ChildOid = 5) or (ChildOid = 8) or (ChildOid = 18) or (ChildOid = 22) or (ChildOid = 27) or (ChildOid = 37) then
                Inc(ChildTargetHits);
            Inc(ChildCount);
            Inc(Guard);
            ChildObj := ChildIter.NextSchObject;
        end;
        if Guard >= 256 then
            RouteStatus := 'CHILD_ITERATOR_ACTIVE_TRUNCATED_256_OBJECTID_ONLY'
        else if ChildCount = 0 then
            RouteStatus := 'CHILD_ITERATOR_ACTIVE_NO_CHILD_OBJECTS'
        else
            RouteStatus := 'CHILD_ITERATOR_ACTIVE_OK_OBJECTID_ONLY';
    finally
        Component.SchIterator_Destroy(ChildIter);
    end;
end;

{ V67 cleanup note:
  The old full child ObjectId decoder report procedure from V62/V66 is intentionally
  removed from the executable script body. Its useful result is preserved in the
  comments/history above and in the smaller grouped mini-probe below. The helper
  decoder function V67DecodeChildObjectId is kept because the V67 CSV uses it. }

{==============================================================================}
{ 3g. V71 CLEAN/STABLE - COM GEOMETRY PROBES STOPPED                          }
{==============================================================================}
{
  V71 decision log:
  - Validated and kept: BOM export, PCB export, SCH JSON export, component text
    rendering, pins, wires, net labels, power ports, sheet symbols/entries and
    hierarchy anchors.
  - Validated ObjectId mapping from prior probes:
      27 = ISch_Parameter
      37 = ISch_Pin
       8 = ISch_NetLabel
      44 = ISch_Line cast-only
  - Known bad / stopped routes:
      * V63 broad COM cast matrix caused Altium hang. Do not re-enable.
      * Generic Variant Location, BoundingRectangle, StartLocation and
        GetState_Vertex routes failed or did not compile.
      * V68 ObjectId18 + ISch_Arc.Location/Radius: no exploitable geometry.
      * V69b ObjectId5 + ISch_Rectangle.Location/Corner: no exploitable geometry.
      * V70b ObjectId44 + ISch_Line: cast OK, but geometry accessors are not
        declared in this Altium API.

  Consequence:
  - No active SCH child-geometry probe is executed in V71.
  - symbolGraphics intentionally remains pinBoundsFallbackUntilNativePrimitives.
  - Next serious route for native symbol bodies is direct SchDoc parsing or a
