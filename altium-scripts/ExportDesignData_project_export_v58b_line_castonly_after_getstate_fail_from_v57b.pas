{==============================================================================}
{ Altium Diff Studio - Exporter Script v58b                                           }
{                                                                              }
...
const
    SCRIPT_VERSION = 'v58b';
    SCHEMA_VERSION = 'ads-json-v58b';
    SCHEMATIC_SCHEMA_VERSION = 'v58b';
...
function V58bTryTypedBatchASymbolPrimitive(Obj : Variant; var X1 : Integer; var Y1 : Integer; var X2 : Integer; var Y2 : Integer; var Status : String) : Boolean;
var
    LineObj : ISch_Line;
...
begin
    ...
    X2 := 0;
    Y2 := 0;
    Status := 'TYPED_BATCH_A_LINE_CASTONLY_NO_MATCH_V58B';

    { v58b historical note / compile-safe rollback:
      v57b proved ObjectId 44 supports ISch_Line on all 378 candidates.
      v58 tried ONE geometry route: ISch_Line.GetState_Vertex(0/1), copied from the
      validated Wire/Bus access pattern. Result in Altium script compiler:
          undeclared identifier: GetState_Vertex on ISch_Line.

      Therefore v58b deliberately DOES NOT call LineObj.GetState_Vertex.
      It keeps the typed cast-only confirmation and records the failure in the CSV/summary.

      Already failed and kept here as a trace:
      ...
      - LineObj.GetState_Vertex(0/1)    -> undeclared identifier GetState_Vertex

      Next candidates for v59 should be one property family at a time:
      StartLocation/EndLocation, Point1/Point2, Vertex1/Vertex2, or library-symbol route. }

    try
        LineObj := Obj;
        if LineObj <> nil then
        begin
            Status := 'ISch_Line_CAST_OK_GETSTATE_VERTEX_UNDECLARED_IN_V58_GEOMETRY_NOT_READ_V58B';
            Result := True;
            Exit;
        end;
    except
        Status := 'ISch_Line_CAST_FAILED_V58B';
    end;
    ...
    try
        ...
        if RectObj <> nil then
        begin
            Status := Status + '|ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V58B';
            Result := True;
            Exit;
        end;
    except
    ...
    try
        ...
        if ArcObj <> nil then
        begin
            Status := Status + '|ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V58B';
            Result := True;
            Exit;
        end;
    except
...
begin
    ...
    begin
        ...
        begin
            ...
            if Oid = 44 then
            begin
                V58bTryTypedBatchASymbolPrimitive(Obj, NativeX1, NativeY1, NativeX2, NativeY2, NativeStatus);
            ...
            begin
                ...
                begin
                    ...
                    DisplayOrient := -1;
                    if (Oid = 44) and ((NativeStatus = 'ISch_Line_CAST_OK_GEOMETRY_NOT_READ_V58') or (NativeStatus = 'ISch_Rectangle_CAST_OK_GEOMETRY_NOT_READ_V58B') or (NativeStatus = 'ISch_Arc_CAST_OK_GEOMETRY_NOT_READ_V58B')) then
                    begin
...
begin
    Workspace := GetWorkSpace;
    if Workspace = nil then begin AppendExportLog('SCH v58b typed ISch_Line cast-only probe after GetState_Vertex compile failure skipped: no workspace'); Exit; end;
    Project := Workspace.DM_FocusedProject;
    if Project = nil then begin AppendExportLog('SCH v58b typed ISch_Line cast-only probe after GetState_Vertex compile failure skipped: no focused project'); Exit; end;

    Csv := TStringList.Create;
    Summary := TStringList.Create;
    Counters := TStringList.Create;
    try
        Csv.Add('sheetId;sheetName;sheetPath;objectIndex;componentClusterId;componentIndex;offsetFromComponent;objectId;knownType;v58bRoleDecision;expectedKindOrPrimitiveRole;componentDesignatorText;componentCommentText;componentLibRef;displayX;displayY;displayOrientation;geometrySource;nativeX1;nativeY1;nativeX2;nativeY2;nativeStatus;nativeSource;probeRouteA_componentContext;probeRouteB_object44TypedAccess;probeRouteC_libRefGrouping;probeRouteD_fallbackQuality;lineGeometryProbe_v58b;batchResultPlan_v58b;recommendedNextAction');
        Summary.Add('Altium schematic v58b typed ISch_Line cast-only probe after GetState_Vertex compile failure embedded in ExportDesignData');
        Summary.Add('Version=' + SCRIPT_VERSION);
        Summary.Add('Schema=' + SCHEMA_VERSION);
        Summary.Add('Project=' + Project.DM_ProjectFullPath);
        Summary.Add('OutputFolder=' + GetProjectPath);
        Summary.Add('Purpose=v58b compile-safe rollback after v58 showed ISch_Line.GetState_Vertex is undeclared; keeps ObjectId 44 typed ISch_Line cast-only confirmation and preserves all failure history.');
        Summary.Add('SafeProperties=ObjectId and iterator order; ObjectId 44 generic properties remain disabled; v58b does not read geometry and records GetState_Vertex as failed on ISch_Line.');
        ...
        Summary.Add('ObjectId 44 generic geometry remains disabled; v58b keeps ISch_Line cast-only and marks GetState_Vertex as undeclared on ISch_Line.');
        Summary.Add('Next step after v58b: try one next ISch_Line property route: StartLocation/EndLocation, Point1/Point2, Vertex1/Vertex2, or library-symbol route.');

        CsvPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v58b_line_castonly_after_getstate_fail_probe.csv');
        SummaryPath := GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v58b_line_castonly_after_getstate_fail_probe_summary.txt');
        Csv.SaveToFile(CsvPath);
        Summary.SaveToFile(SummaryPath);
        AppendExportLog('Saved SCH v58b typed ISch_Line cast-only probe after GetState_Vertex compile failure CSV: ' + CsvPath);
        AppendExportLog('Saved SCH v58b typed ISch_Line cast-only probe after GetState_Vertex compile failure summary: ' + SummaryPath);
    finally
        Csv.Free;
        Summary.Free;
        Counters.Free;
...
begin
    ...
    try
        ...
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v58b_line_castonly_after_getstate_fail_probe.csv'));
        OutputFileNames.Add(GetProjectPath + TargetPrefix + ChangeFileExt(TargetFileName, '_sch_v58b_line_castonly_after_getstate_fail_probe_summary.txt'));
