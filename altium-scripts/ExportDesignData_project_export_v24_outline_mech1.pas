...
begin
    ...
    { V24 outline strategy, kept compatibility-safe:
      1) Prefer a bounding rectangle derived only from Mechanical Layer 1 tracks.
         On many Altium projects this layer carries the routed/manufacturing outline.
      2) Fall back to the previous all-object bounding box if no Mechanical Layer 1
         tracks are present.
      This still is not the native Board Shape API, but it is usually much closer
      to the real board frame than the full-object bounding box and does not rely
      on version-sensitive BoardOutline properties. }
    ...
    while Track <> nil do
    begin
        if Layer2String(Track.Layer) = 'Mechanical Layer 1' then
        begin
            IncludeBoundsPoint(HasMechBounds, MechMinX, MechMinY, MechMaxX, MechMaxY, CoordToMMs(Track.X1), CoordToMMs(Track.Y1));
            IncludeBoundsPoint(HasMechBounds, MechMinX, MechMinY, MechMaxX, MechMaxY, CoordToMMs(Track.X2), CoordToMMs(Track.Y2));
        end;
        Track := Iterator.NextPCBObject;
    end;
    PcbBoard.BoardIterator_Destroy(Iterator);

    if HasMechBounds then
    begin
        MinX := MechMinX;
        MinY := MechMinY;
        MaxX := MechMaxX;
        MaxY := MechMaxY;
        HasBounds := True;
        OutlineSource := 'mechanicalLayer1BoundingBoxFallback';
    end;
...
begin
    ...
    JsonList.Add('    "scriptVersion": "v24",');
    JsonList.Add('    "schemaVersion": "ads-json-v24"');
    JsonList.Add('  },');
end;
...
begin
    ...
    JsonList.Add('      "schematicExportLevel": "logicalPlusFallbackGraphicsV24",');
    ...
    JsonList.Add('        "notes": "V24 keeps the universal schema stable. Native SCH primitive extraction should be enabled object family by object family after API validation in the target Altium version."');
    JsonList.Add('      }');
    JsonList.Add('    }');

    AppendExportLog('Added schematic sheet v24: ' + FileName + ' components=' + IntToStr(CompCount) + ' wires=' + IntToStr(WireCount) + ' netLabels=' + IntToStr(NetCount));
end;
...
begin
    ...
    try
        ...
        JsonList.Add('    "boardOutline": "mechanicalLayer1BoundingBoxFallbackPreferredThenAllObjectFallback",');
        JsonList.Add('    "polygons": "reserved",');
        JsonList.Add('    "splitPlanes": "reserved",');
        JsonList.Add('    "regions": "reserved",');
        JsonList.Add('    "notes": "V24 improves the boardOutline fallback by preferring Mechanical Layer 1 tracks when present. Plane/copper containers remain schema-stable and reserved until native object extraction is validated."');
        JsonList.Add('  },');
...
begin
    ...
    try
        ...
        JsonList.Add('  "type": "schematic",');
        AddExporterMetadata(JsonList);
        JsonList.Add('  "schemaVersion": "v24",');
        JsonList.Add('  "exportGoal": "universalLogicalPlusGraphicsFallbackV24",');
